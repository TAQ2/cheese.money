...
Run this after a PR's code changes are complete and before the final push. Documentation begins **only after all verification gates pass** — never document a broken build. The orchestrator's Stage 5 executes this runbook verbatim, so fill the `{{PLACEHOLDERS}}` with your project's real commands and doc set.
...
```
For any **generated artifact** (CSS/JS bundles, schema dumps, lockfiles), rebuild it and confirm its diff is fully explained by this PR's source changes — never hand-edit generated output.
...
Update your machine-readable reference artifacts (e.g. a function-map, a dependency map, a DB-schema dump) to reflect this PR. If you have generator scripts, run them here; otherwise maintain the maps by hand. Run generators from the repo root (`git rev-parse --show-toplevel`), or from inside the worktree when running under the orchestrator.
...
- New files from this PR get full sections (purpose, imports/imported-by, function/class/symbol docs, design notes).
...
Use the saved verification output from Step 1 — never guess or carry forward stale numbers. Update: the header (commit hash, date, verification mode), the test-run status (results + a PR changelog entry), per-module sections (each new test file: count, scenarios, known gaps), and coverage tables **regenerated from the runner's output**, never hand-edited row by row.

### 5. Commit the docs, stamp the commit hash, push
```bash
cd <repo>
git add {{your doc set: service docs, test docs, function/dependency maps}}
git commit -m "docs(prN): update function maps + service/test docs"
...
- [ ] Build / type-check / lint / tests all pass; generated artifacts rebuilt and their diffs explained by this PR
