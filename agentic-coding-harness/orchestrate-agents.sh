...
run_reduction_pass() {
...

    {
        echo "REMINDER (ORCHESTRATION OVERRIDE): Your instruction file's git branching/commit/push/PR rules are SUSPENDED. Do NOT run 'git commit', 'git push', 'git checkout -b', or 'gh pr create'. Stage changes with 'git add' only. You are in a pre-created worktree."
        echo ""
        echo "SUBTRACTIVE REDUCTION PASS — your implementation is complete and tests are green. This turn has exactly ONE job: make the diff SMALLER without changing what it does."
        echo ""
        ...
        echo ""
        echo "LEGITIMATE TARGETS (subtract — do NOT compress). Remove only:"
        echo "(a) dead branches and unreachable code"
        echo "(b) defensive checks for conditions that cannot occur"
        echo "(c) comments that merely restate the code"
        ...
        echo "(f) docstrings on trivially-named functions beyond the codebase's density"
        ...
        echo ""
        echo "PROCESS:"
        echo "1. Measure current footprint: 'git diff --stat' against the branch point; note the net lines your implementation added."
        ...
        echo ""
        echo "End your response with a REDUCTION REPORT: net lines before -> after (delta) or 'no viable reduction'; what was removed by category (a-h / other), or why nothing more could go without sacrificing clarity or behavior; and confirmation that the full suite is GREEN after reduction."
    } > "$reduce_prompt"
