...
# Code itself (<claude config dir>/projects/<encoded-cwd>/<sid>.jsonl) — when the
...
# (`<claude config dir>/projects/<cwd>/<sid>.jsonl`) and waits for the last
...
spinner() {
    ...
    if [[ -n "$session_file" ]] && [[ -f "$session_file" ]] && [[ -s "$session_file" ]]; then
        ...
        if [[ -n "$sid" ]]; then
            session_jsonl=$(find "$CLAUDE_PROJECTS_DIR" -maxdepth 3 -name "${sid}.jsonl" -print -quit 2>/dev/null || true)
...
# completion polled from <claude config dir>/projects/*/<sid>.jsonl waiting for the
...
tmux_env_prefix() {
    ...
    # The account binding travels with the launch. CLAUDE_CONFIG_DIR is a
    # documented Claude Code variable, not an automation marker, so passing it
    # through does not undo what the stripping is for. Omitted entirely when
    # the default account is selected — that account is the one with NO
    # variable set (see claude_account_resolve).
...
tmux_wait() {
    ...
    while :; do
        ...
        # Announce a switch the moment it happens, even though it is applied
        # only once this turn ends — otherwise the operator sees nothing until
        # the next stage and assumes the switch was ignored.
        if claude_account_change_pending; then
            claude_account_note_change "$CLAUDE_ACCOUNT_CHANGE_SEEN" || true
        fi
...
tmux_oneshot() {
    ...
    tmux -S "$TMUX_SOCK" new-window -t "$TMUX_SESSION:" -n "$role" -d \
        || { rm -f "$pf"; return 1; }
    local env_prefix cmd
    # A fresh window every call, so there is nothing to resume and nothing to
    # lose: a oneshot always starts on whatever CH3 selects right now.
...
invoke_agent() {
    ...
    # A turn boundary: the previous turn is done and the next prompt has not
    # gone out, so this is where a mid-run account switch is safe to apply. The
    # pane is retired first; `tmux_launch` then resumes the same session id on
    # the new account. `if` rather than `&&` — a no-change return must not trip
    # `set -e`.
    if claude_account_refresh; then
        claude_account_rebind_pane "$window" || true
    fi
...
check_prerequisites() {
    ...
    # Bind to the account CH3 has selected before anything can launch claude.
    # Needs jq, hence its position here.
    claude_account_bind
...
prompt_run_config() {
    ...
    if [[ -z "$TPM_CONVERSATION_ID" ]]; then
        ...
        if [[ -z "$tpm_input" ]]; then
            ...
            if ! compgen -G "${CLAUDE_PROJECTS_DIR}/*/${TPM_CONVERSATION_ID}.jsonl" >/dev/null 2>&1; then
...
run_stage_7() {
...

    {
        if [[ "$s7_mode" == "failed" ]]; then
            ...
            echo "against the intended business outcome. This is your Phase 5 gate and you"
            echo "EXECUTE it — you never ask for permission to run it."
            echo ""
            echo "- OUTCOME HOLDS -> land it and push it YOURSELF, in this same reply, unprompted:"
            echo "    git -C ${tpm_repo_root} checkout ${BASE_BRANCH:-main} && git -C ${tpm_repo_root} pull origin ${BASE_BRANCH:-main}"
            echo "    git -C ${tpm_repo_root} merge ${WORKTREE_BRANCH:-<branch>} --squash --no-commit"
            echo "    # review the COMPLETE staged diff, run the project checks, then:"
            echo "    git -C ${tpm_repo_root} commit -C ${WORKTREE_BRANCH:-<branch>}   # -C keeps the Stage 6 body"
            echo "    git -C ${tpm_repo_root} log -1 --format=%B | wc -w                # hundreds of words, not tens"
            echo "    git -C ${tpm_repo_root} push origin ${BASE_BRANCH:-main}"
            echo "  The push is part of the verdict, not a follow-up favour. Then report ONCE:"
            echo "  the SHA now on origin/${BASE_BRANCH:-main}, proof the body survived, and the"
            echo "  DEPLOYMENT INSTRUCTIONS Conrad runs — exact steps in order, with every manual/ops"
            echo "  step (DDL, env vars, dashboards, scheduled jobs, service restarts) and their"
            echo "  ordering. Deploying is the ONLY act here that is his."
            echo "  Do NOT reply asking whether to commit, whether to push, or whether to clean up."
            echo ""
            echo "- WHEN CONRAD CONFIRMS THE DEPLOY -> purge the run scaffolding on your own"
            echo "  initiative, unasked, in that same turn:"
            echo "    git -C ${tpm_repo_root} worktree remove ${WORKTREE_DIR:-<worktree>}"
            echo "    git -C ${tpm_repo_root} branch -D ${WORKTREE_BRANCH:-<branch>}"
            echo "    git -C ${tpm_repo_root} push origin --delete ${WORKTREE_BRANCH:-<branch>}   # only if it was pushed"
            echo "    git -C ${tpm_repo_root} worktree prune"
            echo "  Then state in one line what was removed. A worktree still on disk after a"
            echo "  confirmed deploy is unfinished Phase 5 work."
            echo ""
            echo "- OUTCOME DOES NOT HOLD -> STOP. Do not touch git. Name the ONE specific"
            echo "  business-outcome gap in your reply. Uncertainty resolves to HOLD with a named"
            echo "  gap — never to a question about whether to proceed."
            echo ""
        fi
        echo "Run context:"
    ...
    local session_file
    session_file=$(ls "${CLAUDE_PROJECTS_DIR}"/*/"${TPM_CONVERSATION_ID}.jsonl" 2>/dev/null | head -1 || true)
...
# <claude config dir>/projects/<cwd-with-slashes-and-spaces-as-dashes>/<session-id>.jsonl
...
#   1. Locates the session JSONL on disk (<claude config dir>/projects/*/<sid>.jsonl)
...
print_resume_command() {
    ...
    # Locate the session JSONL wherever it lives under <claude config dir>/projects/
    local jsonl=""
    shopt -s nullglob
    local candidates=("$CLAUDE_PROJECTS_DIR"/*/"${sid}.jsonl")
    shopt -u nullglob
...
copy_sessions_to_worktree() {
    ...
    if [[ -f "${BRAIN_SESSION_FILE:-}" ]] && [[ -s "$BRAIN_SESSION_FILE" ]]; then
        ...
        local brain_jsonl
        brain_jsonl=$(find "$CLAUDE_PROJECTS_DIR" -maxdepth 2 -name "${brain_sid}.jsonl" -print -quit 2>/dev/null || true)
    ...
    if [[ -n "${WORKTREE_BRANCH:-}" ]]; then
        wt_project_dir=$(find "$CLAUDE_PROJECTS_DIR" -maxdepth 1 -type d \
    ...
    if [[ -z "$wt_project_dir" ]] && [[ -n "${WORKTREE_DIR:-}" ]] && [[ -d "$WORKTREE_DIR" ]]; then
        ...
        # `claude` populates <claude config dir>/projects/<encoded-cwd>/ on first launch.
...
select_model_config() {
    ...
    printf "  ${C_BOLD}${C_CYAN}  5${C_RESET}  Brain ${C_BOLD}Opus 5${C_RESET} + Coder ${C_BOLD}Opus 5${C_RESET} + QA ${C_BOLD}Opus→Fable${C_RESET}  ${C_DIM}— cascade: 1st QA Opus, 2nd QA Fable; QA rounds forced to 2${C_RESET}\n"
    printf "  ${C_BOLD}${C_CYAN}  6${C_RESET}  Brain ${C_BOLD}Fable 5${C_RESET} + Coder ${C_BOLD}Opus 5${C_RESET} + QA ${C_BOLD}Opus→Fable${C_RESET}  ${C_DIM}— cascade: 1st QA Opus, 2nd QA Fable; QA rounds forced to 2${C_RESET}\n"
    printf "\n"
    printf "  ${C_BOLD}Select [1-6] (Enter = 3): ${C_RESET}"
    ...
    case "${model_choice:-3}" in
        ...
        5)
            ...
            QA_ROUNDS=2
            QA_ROUNDS_SET=true
            MODEL_CONFIG_LABEL="Brain Opus 5 + Coder Opus 5 + QA Opus→Fable (2 rounds)"
            ;;
        6)
