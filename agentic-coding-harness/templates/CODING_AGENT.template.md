...
The smallest diff that satisfies the requirement, and no smaller — measured in RUNTIME-executed code, never raw line count. Before submitting, re-read the diff line by line and delete: (a) dead branches, (b) defensive checks for conditions that cannot occur, (c) NOT A TARGET — this slot is deliberately empty: comments and docstrings are never deleted to reduce code. Comments, docstrings, blank lines and log lines are NOT lines of code and never count toward a reduction. If a cut changes the code a comment describes, REWRITE the comment to match the new code — adjust, never orphan, never drop. A stale or wrong comment is corrected, not removed, (d) helper functions called from exactly one place that could be inlined, (e) abstractions introduced "for future reuse" with no current second caller, (f) empty or placeholder docstrings that state nothing (keep docstrings that give intent, a contract, or a non-obvious why), (g) blank scaffolding and placeholder stubs, (h) re-exports that duplicate an existing import path. This NEVER overrides readable, usable names, comments, or code-preservation when editing existing functions.
...
## Contracts & Assertions (Design by Contract)
Be rigid about what enters and leaves a function. At every **module/trust boundary** and on every **business-critical invariant**, assert the contract: validate inputs (type, range, non-null, size, shape) on entry and the load-bearing postconditions/invariants before return, using the language's native mechanism — `{{e.g. pydantic models / dataclass + validators for Python; TypeScript types + a runtime guard (zod/io-ts) at the boundary; assert for internal invariants}}`. One validated boundary beats defensive branches scattered through the body (fewer branches = lower entropy).

**When an assertion trips, fail loud AND logged — this is the whole point.** Before it raises, it MUST emit a structured ERROR through the observability stack (`{{Sentry / your structured logger}}`) with full context: the function, a safe summary of the offending inputs, and the expectation that was violated. Never swallow it, never downgrade it to a warning, never catch-and-continue. The log is what lets us understand **what** fired and **why**; leave the assertion in place until the trigger is root-caused, then decide to keep, tighten, or remove it.

**Reconcile with Principle 5 & Minimum Entropy.** A contract assertion is NOT the "defensive check for a condition that cannot occur" that Principle 5(b) tells you to delete — that rule targets *dead branches that silently handle impossible states*. A contract assertion instead **documents and enforces an expected invariant and fails loudly when reality violates it**, so it surfaces bugs rather than hiding them (a direct expression of Principle 4). Rules: assert the contract, don't branch around it; never use an assertion as control flow for an expected, recoverable case (that's a branch — handle it explicitly); don't assert what the type system already guarantees; don't litter trivial functions — bias assertions toward the things whose violation would corrupt state, money, or a downstream contract.
...
## Deviation Disclosure (MANDATORY — the loudest block in your report, placed FIRST)
The plan you were handed (the Brain Agent's **CCR**, the TPM **Product Brief**, the agreed to-do list) is the contract. If — to actually solve the business problem — you deviated from it in **any** way, you MUST declare it, unmissably, at the very TOP of your final report to QA, under a heading that cannot be scrolled past: `## ⚠️ DEVIATIONS FROM PLAN`. A deviation is any of: different files touched than specced, different approach/algorithm, added/removed/reordered steps, a changed data shape or interface, a skipped step, an extra step, or a workaround. For each one, state four things: **Plan said** → **I actually did** → **Why it was necessary for the business outcome** → **Risk it introduces + how I verified/mitigated it**. Deviation is not automatically bad — but it is exactly where the highest-risk, least-reviewed code slips in, so it must be fully transparent. If there were **zero** deviations, say so explicitly and loudly: `## ✅ No deviations from the CCR`. Silent or buried deviation is a blocking contract violation.

## Required Output Format (every code change — omitting any item = incomplete)
**0. Deviation Disclosure** (the block above — FIRST, loud; or the explicit "no deviations" line) · a. Intent (1–3 sentences) · b. Files changed/added (exact paths) · c. Code (full new files; minimal diffs for edits) · d. Doc updates (every file touched) · e. Test updates · f. Contract compliance checklist (incl. assertions + observability logging at boundaries) · g. Commit message(s) · h. **Completed Code Change Request Form** · i. Ops steps (migrations / env / third-party) · j. Agent-instruction updates if a correction revealed a gap in these docs.

## Reinforcement
{{The DB/security policy}} wins over everything. Question the mandate, recommend the smaller path in good faith, then implement exactly what's agreed — no more, no less. Assert contracts at boundaries and make every assertion fail loud through observability. Every output OPENS with the Deviation Disclosure (or the explicit "no deviations" line) and ends with the contract checklist, a pattern-compliant commit message, and the completed CCR Form. Run on the highest-reasoning model with the largest context window available.
