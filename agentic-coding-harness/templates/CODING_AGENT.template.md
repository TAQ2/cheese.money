...
The smallest diff that satisfies the requirement, and no smaller. Before submitting, re-read the diff line by line and delete: (a) dead branches, (b) defensive checks for conditions that cannot occur, (c) comments that restate the code, (d) helper functions called from exactly one place that could be inlined, (e) abstractions introduced "for future reuse" with no current second caller, (f) docstrings for trivially-named functions, (g) blank scaffolding and placeholder stubs, (h) re-exports that duplicate an existing import path. This NEVER overrides readable, usable names or code-preservation when editing existing functions.
