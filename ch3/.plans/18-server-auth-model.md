...
Define the long-term server auth architecture for CH3 before first-class remote environments ship.
...
Every privileged surface of the CH3 server must go through the same auth policy engine:
...
Participants:
  ...
  CH3Server      = local backend child process
...
DesktopMain -> CH3Server : spawn server (--bootstrap-fd ...)
DesktopMain -> CH3Server : send desktop bootstrap envelope
note over CH3Server : policy = DesktopManagedLocalPolicy
note over CH3Server : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> CH3Server : POST /api/auth/bootstrap
CH3Server -> CH3Server : validate desktop bootstrap grant
CH3Server -> CH3Server : create browser session
CH3Server --> Frontend : Set-Cookie: session=...

Frontend -> CH3Server : GET /ws + authenticated cookie
CH3Server -> CH3Server : validate cookie session
CH3Server --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx ch3 launcher
  CH3Server      = standalone local server
  Browser       = browser tab

UserShell -> CH3Server : start server
CH3Server -> CH3Server : getOrCreate("server-signing-key")
note over CH3Server : policy = LoopbackBrowserPolicy

UserShell -> CH3Server : issue one-time pairing token
CH3Server --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> CH3Server : GET /pair?token=...
CH3Server -> CH3Server : validate one-time token
CH3Server -> CH3Server : create browser session
CH3Server --> Browser : Set-Cookie: session=...
CH3Server --> Browser : redirect to app

Browser -> CH3Server : GET /ws + authenticated cookie
CH3Server --> Browser : websocket accepted
```
...
Participants:
  ...
  CH3Server      = CH3 server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> CH3Server : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local CH3 endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> CH3Server : issue one-time pairing token
CH3Server --> DesktopMain : pairing token
...
Tunnel -> CH3Server : forward request
CH3Server -> CH3Server : validate one-time token
CH3Server -> CH3Server : create mobile browser session
CH3Server --> PhoneBrowser : Set-Cookie: session=...
CH3Server --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> CH3Server : forward websocket upgrade
CH3Server --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> CH3Server : enable private-network access
CH3Server -> CH3Server : switch policy to RemoteReachablePolicy
DesktopUser -> CH3Server : issue one-time pairing token
CH3Server --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> CH3Server : route request
CH3Server -> CH3Server : validate one-time token
CH3Server -> CH3Server : create mobile browser session
CH3Server --> PhoneBrowser : Set-Cookie: session=...
CH3Server --> PhoneBrowser : redirect to app
...
PrivateNet -> CH3Server : websocket upgrade
CH3Server --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteT3      = remote CH3 server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify ch3 availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : ch3 remote launch --json
