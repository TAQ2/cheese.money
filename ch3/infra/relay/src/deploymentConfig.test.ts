...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("ch3-relay-traces", "prod")).toBe("ch3-relay-traces-prod");
    expect(relayResourceNameForStage("ch3-relay-traces", "dev_julius")).toBe(
      "ch3-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "ch3relay.com", hash)).toBe(
      "prod-abcdef0123456789.ch3relay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "ch3relay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
