...
import { RELAY_HEALTH_RESPONSE_TYP, RELAY_MINT_RESPONSE_TYP } from "@ch3tools/shared/relayJwt";
...
const settings = RelayConfiguration.RelayConfiguration.of({
  ...
  apns: {
    ...
    bundleId: "com.ch3tools.ch3.dev",
  },
  apnsDeliveryJobSigningSecret: Redacted.make("job-secret"),
  clerkSecretKey: Redacted.make("clerk-secret"),
  clerkPublishableKey: "pk_test_test",
  clerkJwtAudience: "ch3-relay",
...
function signMintResponse(
  ...
  const payload = {
    iss: `ch3-env:${requestProof.environmentId}`,
...
describe("EnvironmentConnector", () => {
  ...
  it.effect("checks linked environment health through the managed endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(seenUrls).toEqual(["https://env.example.test/api/ch3-connect/health"]);
      expect(seenProofs[0]).toMatchObject({
        iss: "https://relay.example.test",
        aud: "ch3-env:env-connector-test",
        sub: "user_123",
        environmentId: "env-connector-test",
  ...
  it.effect("mints a one-time environment credential through the linked endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(seenUrls).toEqual(["https://env.example.test/api/ch3-connect/mint-credential"]);
      expect(seenProofs[0]).toMatchObject({
        iss: "https://relay.example.test",
        aud: "ch3-env:env-connector-test",
        sub: "user_123",
