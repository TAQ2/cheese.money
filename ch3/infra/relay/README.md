...
> [!NOTE]
> Sign in to CH3 Connect from the app under Settings > Connections.

The relay is the hosted control plane for CH3 Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal CH3 traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [CH3 Connect architecture overview](../../docs/internals/ch3-connect-auth-flow.html) for the larger system
design.
...
- Linking CH3 environments to a cloud account.
...
```sh
vp run --filter ch3-relay deploy
```
...
```sh
vp run --filter ch3-relay deploy -- --stage prod
vp run --filter ch3-relay deploy -- --env-file .env.local
```
...
- [CH3 Connect Clerk Setup](../../docs/internals/ch3-connect.md) for Clerk keys, JWT templates, and sign-up restrictions
  setup.
- [Relay Observability](../../docs/operations/relay-observability.md) for deployment tracing and diagnostics.
- [CH3 Connect Architecture Overview](../../docs/internals/ch3-connect-auth-flow.html) for the full link,
