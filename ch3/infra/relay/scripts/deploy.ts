...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    CH3CODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    CH3CODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    CH3CODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    CH3CODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    CH3CODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    CH3CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("CH3CODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("CH3CODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "CH3Relay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the CH3 relay through Alchemy."));
