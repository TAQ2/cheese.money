...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "CH3CODE_CLERK_PUBLISHABLE_KEY=pk_test_example\nCH3CODE_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "ch3-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "ch3-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "CH3CODE_MOBILE_OTLP_TRACES_DATASET=ch3-mobile-traces-dev",
        "CH3CODE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "CH3CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "CH3CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=ch3-relay-client-traces-dev",
        "CH3CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "CH3CODE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "CH3CODE_RELAY_URL=https://old.example.test",
          "CH3CODE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "CH3CODE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "CH3CODE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "CH3CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "CH3CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "CH3CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "CH3CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "CH3CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "CH3CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
