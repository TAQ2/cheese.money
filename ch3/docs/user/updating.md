...
The CH3 web or desktop app and the server it connects to work best when they use the same
version. If they do not match, CH3 shows a warning with the right update option for that server.
...
The available action depends on how that server was started. CH3 does not update connected
...
**Copy update command** gives you `npx ch3@<client-version>`, which relaunches the server directly
at the matching version. Add whatever startup options you normally use.

If the server instead runs as the CH3 background service, update the service on the host and
pin the same version:

```sh
npx ch3@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx ch3@latest service update`
...
See [Running CH3 in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx ch3@<client-version>`, replacing
