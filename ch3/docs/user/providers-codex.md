...
This guide is for people who want to use more than one Codex account in CH3. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install CH3](./install.md).
...
- both accounts can see the same CH3/Codex sessions
...
In CH3 Settings, name it something obvious:
...
In CH3 Settings, add another Codex provider:
...
CH3 shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, CH3 treats it
