...
Use this when you want to connect to a CH3 server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch CH3 over SSH.
...
- HTTPS/WSS-compatible endpoints work from `https://app.ch3.codes`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.ch3.codes/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
backend with the same server-side behavior as `ch3 serve --tailscale-serve`, then the server asks
...
For `https://app.ch3.codes`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
Run the server with `ch3 serve`.

```bash
npx ch3 serve --host "$(tailscale ip -4)"
```

`ch3 serve` starts the server without opening a browser and prints:
...
Use `ch3 serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx ch3 serve --tailscale-serve
```
...
```bash
npx ch3 serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse CH3 on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote CH3 server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual CH3 server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.ch3/ssh-launch/<host-key>/`, starts or reuses a remote CH3 server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. CH3 uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, CH3 first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path CH3 uses:
...
If reconnecting after an app update fails, retry the SSH launch once. The launcher now compares its generated runner script, stops stale launcher-managed remote servers, clears the SSH launch PID/port state, and starts a fresh remote server. You should not normally need to delete `~/.ch3/ssh-launch` or kill `ch3` processes manually.

## Updating a Remote Server

When the CH3 web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: CH3 may
...
see [Keeping CH3 in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running CH3 in the Background](./background-service.md).
...
1. `ch3 serve` issues a one-time owner pairing token.
...
The hosted web app at `https://app.ch3.codes` can save a remote backend in browser local storage from a URL like:

```text
https://app.ch3.codes/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through CH3. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `ch3 auth` to manage access after the initial pairing flow.
...
Use `ch3 auth --help` and the nested subcommand help pages for the full reference.
...
- Use `ch3 auth` to revoke credentials or sessions you no longer trust.
