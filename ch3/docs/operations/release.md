...
- Reads the shared production CH3 Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `ch3`) with OIDC trusted publishing from the same workflow file:
...
```sh
vp run --filter ch3-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `CH3CODE_WEB_ROUTER_URL`: defaults to `https://app.ch3.codes`.
- `CH3CODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.ch3.codes`.
- `CH3CODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.ch3.codes`.

Required Vercel domains:

- `app.ch3.codes`: the router domain users open, updated by stable releases.
- `latest.app.ch3.codes`: channel alias updated by stable releases.
- `nightly.app.ch3.codes`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__ch3/channel?channel=latest` or
`/__ch3/channel?channel=nightly`; the router stores the
`ch3_web_channel` cookie and rewrites future requests on `app.ch3.codes` to
...
through `/__ch3/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.ch3.codes` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `ch3`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `ch3@<version>` package available on
...
For a release smoke test, confirm `npm view ch3@<version> version` returns the expected version, then
...
- Repository slug source:
  - `CH3CODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
script temporarily prepares the `ch3` package, then runs `vp pm publish --filter ch3 ...` from the
repository root so workspace publish configuration is applied correctly.

Checklist:

1. Confirm npm org/user owns package `ch3` (or rename package first if needed).
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `ch3` with npm dist-tag
`latest`, creates a real GitHub Release, aliases the hosted app to `latest.app.ch3.codes` and
`app.ch3.codes`, and can commit a version bump to `main` in the finalize job. Do not push a test tag
...
2. Create an explicit App ID for `com.ch3tools.ch3` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [CH3 Connect Clerk Setup](../internals/ch3-connect.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.ch3tools.ch3` and includes
    Associated Domains.
