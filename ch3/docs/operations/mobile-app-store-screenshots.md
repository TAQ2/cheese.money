...
The screenshot harness runs the real mobile application against three disposable local CH3
...
the software keyboard, and supplies deterministic CH3 Connect discovery rows to the real
...
1. Creates three temporary CH3 base directories and starts a local server for each on an available
   port.
2. Creates CH3, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic CH3 review diff.
...
ARM64 by default for local Apple Silicon development; CI sets `CH3_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @ch3tools/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real CH3 and React fixture
