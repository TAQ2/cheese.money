...
- `ch3-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `ch3-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `ch3-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `ch3-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `ch3-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`ch3-relay-traces-dev-julius` for a personal stage.
