...
CH3 has one server-side observability model:
...
stdout/stderr at `~/.ch3/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.ch3/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.ch3/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.ch3/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.ch3/dev/logs/server.trace.ndjson`.
...
```bash
export CH3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export CH3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export CH3CODE_OTLP_SERVICE_NAME=ch3-local
```

Optional:

```bash
export CH3CODE_TRACE_MIN_LEVEL=Info
export CH3CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `CH3CODE_OTLP_*`.

macOS app bundle example:

```bash
CH3CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
CH3CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
CH3CODE_OTLP_SERVICE_NAME=ch3-desktop \
"/Applications/CH3.app/Contents/MacOS/CH3"
```
...
```bash
TRACE_FILE="${CH3CODE_HOME:-$HOME/.ch3}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.ch3/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.ch3/dev/logs/server.trace.ndjson"
```
...
- service name such as `ch3-local`, `ch3-dev`, or `ch3-desktop`
...
- `ch3_rpc_request_duration`
- `ch3_orchestration_command_duration`
- `ch3_orchestration_command_ack_duration`
- `ch3_provider_turn_duration`
- `ch3_git_command_duration`

Counters tell you volume and failure rate:

- `ch3_rpc_requests_total`
- `ch3_orchestration_commands_total`
- `ch3_provider_turns_total`
- `ch3_git_commands_total`
...
`ch3_orchestration_command_ack_duration` measures:
...
1. Check `ch3_orchestration_command_ack_duration` by `commandType`.
...
- `CH3CODE_OTLP_TRACES_URL` was not set
...
- `CH3CODE_TRACE_FILE`: override trace file path
- `CH3CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `CH3CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `CH3CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `CH3CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `CH3CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `CH3CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `CH3CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `CH3CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `CH3CODE_OTLP_SERVICE_NAME`: service name, default `ch3-server`
