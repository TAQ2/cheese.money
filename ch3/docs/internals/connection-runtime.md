...
The connection runtime is the client-side owner of connectivity,
authentication, retries, transport lifetime, cached environment data, and
environment-scoped operations. The web client is its one consumer today.

The application mounts this runtime once at its root
(`apps/web/src/connection/runtime.ts`) and supplies only the platform layer.
...
- During establishment, `waitForEstablishmentInterrupt` consumes and **ignores**
  ...
  `application-active-reconnect`, which a platform layer may emit after a
  meaningful background suspension; it interrupts establishment and resets the
  retry ladder, because the OS may have silently killed the socket underneath
  the attempt.
...
- Once connected, `monitorConnectedLease` handles plain activation by probing
  the existing session (`lease.session.probe`, with a shorter timeout for an
...
- The application owns its Atom runtime, React hooks, and feature composition.
...
runtime, and selects the domain atom factories required by that platform. An
application may expose its own hooks and features without changing connection
ownership.
