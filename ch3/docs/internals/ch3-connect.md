...
CH3 Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
CH3CODE_CLERK_PUBLISHABLE_KEY=<publishable key>
CH3CODE_CLERK_JWT_TEMPLATE=<JWT template name>
CH3CODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
CH3CODE_RELAY_URL=https://relay.example.com
```
...
should set `CH3CODE_CLERK_PUBLISHABLE_KEY`, `CH3CODE_CLERK_JWT_TEMPLATE`,
`CH3CODE_CLERK_CLI_OAUTH_CLIENT_ID`, and `CH3CODE_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `ch3 connect` command group is
...
The `ch3 connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the CH3 CLI.
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.ch3.codes/connect/callback` for the hosted out-of-band flow. This is
     `connectCallbackUrl(DEFAULT_HOSTED_APP_URL)` from `packages/shared/src/connectAuth.ts`, so a
     custom `CH3CODE_HOSTED_APP_URL` means `$CH3CODE_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `CH3CODE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
ch3 connect            # default: onboarding
ch3 connect login
ch3 connect link       # --publish-only
ch3 connect status     # --json
ch3 connect publish    # --disable
ch3 connect unlink
ch3 connect logout
```

`ch3 serve` is a separate top-level command, not a connect subcommand.

`ch3 connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `ch3 connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running CH3 server. The next `ch3 serve` or `ch3 start` reconciles the relay link and launches the
managed tunnel. `ch3 connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `ch3 connect link` can re-enable exposure without another browser flow. `ch3 connect
...
logout leaves it running; manage it with `ch3 service status`, `install`, `update`, and `uninstall`.
...
Local desktop development uses `ch3-dev://app`, while packaged builds use `ch3://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `ch3-dev://app`; the production Clerk instance should only need
`ch3://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
```dotenv
CH3CODE_APPLE_TEAM_ID=ABC1234567
CH3CODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/ch3.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
CH3CODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `CH3CODE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`CH3CODE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `CH3CODE_PORT` set. Rebuild the signed app
...
CH3CODE_PORT=13773 \
  "/Applications/CH3 (Alpha).app/Contents/MacOS/CH3 (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/CH3 (Alpha).app"
codesign -d --entitlements :- "/Applications/CH3 (Alpha).app"
```
...
Signed-in users manage CH3 Connect under **Connections**. The settings sidebar also has dedicated
controls, rendered by `SettingsSidebarNav.tsx`: `CH3ConnectSidebarSignIn` in the footer shows a
**Sign in to CH3 Connect** button while signed out, and `CH3ConnectSidebarAvatar` shows a Clerk
