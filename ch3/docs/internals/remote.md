...
CH3 has one runtime boundary: a client talks to a CH3 server over HTTP and WebSocket, and the server
...
└───────────────┬──────────────────────────────┘
                │ connects to one CH3 server
┌───────────────▼──────────────────────────────┐
│ Execution environment = one CH3 server        │
...
One running CH3 server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed CH3 Connect relay tunnels.                                       |
...
Tailscale is the first provider, and CH3 manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@ch3tools/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a CH3 server? It does not answer
...
Worker itself. See [ch3-connect.md](./ch3-connect.md).

### Tailscale access

A CH3-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote CH3
...
Launch answers a different question: how does a CH3 server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs CH3 and the client connects directly or through a
  tunnel.
...
- a relay-hosted OAuth callback broker (see [ch3-connect.md](./ch3-connect.md));
