...
CH3 can update a connected server to the exact version of the client that detected version
...
| ----------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `boot-service`    | Linux server running under the CH3-managed systemd user service                               | Call the update RPC; the service unit is replaced and restarted.      |
...
flowchart TD
    ...
    F --> H[Install exact ch3 version in pinned runtime]
    G --> H
    ...
    K -->|boot-service| L[Rewrite and restart CH3 systemd unit]
...
The update service permits one update at a time. It installs `ch3@<version>` under
...
The systemd user service is a host lifecycle concern, not a CH3 Connect resource. The standalone
`ch3 service install`, `uninstall`, `update`, and `status` commands own it. Install and update both
reconcile the unit through `BootService`; running `npx ch3@latest service update` therefore pins and
activates the latest CLI release without requiring a connected client.

The `ch3 connect` onboarding flow may offer service installation, but it calls the same reconciliation
operation as `ch3 service install`. Connect logout only disables cloud access and clears its
authorization; it does not uninstall the host service.

## Process Handoff

For `boot-service`, the server atomically rewrites the CH3-managed user unit to point at the verified
...
The exact client version must exist as the `ch3` npm package before a client carrying that version is
