...
CH3 uses [Vite+](https://viteplus.dev/guide/). Install the global `vp` command, install
...
- `vp run dev --browser`: Auto-opens a browser. Off by default. The dev runner writes
  `CH3CODE_NO_BROWSER` itself from this flag, so setting `CH3CODE_NO_BROWSER=0` in your environment has
...
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/ch3-dev`

### Dev state directories

- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.ch3`, even
  when `CH3CODE_HOME` is set, storing state in `<worktree>/.ch3/userdata`. Pass `--home-dir <path>` to
  choose another isolated directory explicitly. Submodules are not worktrees and keep the normal
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.ch3/dev`, keeping development state
  separate from `~/.ch3/userdata`. An explicit `--home-dir <path>` stores state under
  `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared
  data.

## Build, check, test

- `vp run build`: Fans out over `apps/*`, `packages/*`, `oxlint-plugin-ch3`, and `scripts`.
...
- Desktop production windows load the bundled UI from the `ch3://app/` root URL (not a
  `127.0.0.1` document URL, and not an explicit `index.html` path).
- Desktop packaging includes `apps/server/dist` (the `ch3` backend) and starts it on loopback with an
...
- Signed macOS builds also require `CH3CODE_APPLE_TEAM_ID` and
  `CH3CODE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `CH3CODE_CLERK_PUBLISHABLE_KEY` unless `CH3CODE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
- Example: `CH3CODE_DEV_INSTANCE=branch-a vp run dev:desktop`

Offset resolution, in order:

1. `CH3CODE_PORT_OFFSET`, which must be a non-negative integer. Negative values are rejected.
2. `CH3CODE_DEV_INSTANCE`. An all-digit value is used directly as the offset; any other non-empty
