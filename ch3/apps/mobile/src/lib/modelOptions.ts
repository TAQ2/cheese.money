...
function normalizeSelectionOptions(
  ...
  // Descriptor-less options (the response style) survive normalization: the
  // rebuild is driven by capabilities, which never describe them.
  const options = withDescriptorlessProviderOptionSelections(
    buildProviderOptionSelectionsFromDescriptors(
      getProviderOptionDescriptors({
        caps: capabilities,
...
export function buildModelOptions(
  config: CH3ServerConfig | null | undefined,
