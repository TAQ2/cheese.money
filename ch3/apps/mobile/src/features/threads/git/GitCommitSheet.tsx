      ...
      >
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            ...
            <Text className="text-foreground text-base font-ch3-bold">
        ...
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            <View className="gap-1">
              <Text className="text-foreground text-base font-ch3-bold">Files</Text>
            ...
            <View className="flex-row items-center gap-2">
              {!allSelected && isEditingFiles ? (
                ...
                >
                  <Text className="text-foreground text-2xs font-ch3-bold uppercase">Reset</Text>
                </Pressable>
              ) : null}
              <Pressable
                className="bg-subtle rounded-full px-3 py-2"
              ...
              >
                <Text className="text-foreground text-2xs font-ch3-bold uppercase">
            ...
            <View className="gap-2">
              {selectedFilePreview.map((file) => (
                <View key={file.path} className="flex-row items-center justify-between gap-3">
                  ...
                  <Text className="text-xs font-ch3-bold text-emerald-500">+{file.insertions}</Text>
                  <Text className="text-xs font-ch3-bold text-rose-500">-{file.deletions}</Text>
                </View>
              ))}
                  ...
                  >
                    ...
                    <View className="flex-row items-start justify-between gap-3">
                      <View className="flex-1 gap-1">
                        <Text
                          selectable
                          className={`text-sm font-ch3-bold ${included ? "text-foreground" : "text-foreground-muted"}`}
                        >
                          {file.path}
                        </Text>
                        {!included ? (
                      ...
                      <View className="items-end gap-1">
                        <Text className="text-xs font-ch3-bold text-emerald-500">
                          +{file.insertions}
                        </Text>
                        <Text className="text-xs font-ch3-bold text-rose-500">
                          -{file.deletions}
                        </Text>
                      </View>
                    </View>
        ...
        <View className="gap-2">
          <Text className="text-foreground text-sm font-ch3-bold">Commit message</Text>
          <TextInput
            multiline
