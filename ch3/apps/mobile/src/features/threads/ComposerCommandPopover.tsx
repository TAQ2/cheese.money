...
import type { ComposerTriggerKind } from "@ch3tools/shared/composerTrigger";
import type { ServerProviderSkill, ServerProviderSlashCommand } from "@ch3tools/contracts";
...
const CommandRow = memo(function CommandRow(props: {
  ...
  return (
    ...
    >
      ...
      <Text className="shrink-0 text-base font-ch3-medium text-foreground" numberOfLines={1}>
        {props.item.label}
      </Text>
    ...
    <PopoverSurface isDarkMode={isDarkMode}>
      {label ? (
        <View className="px-3.5 pt-2.5 pb-1">
          <Text className="text-3xs font-ch3-bold tracking-[0.8px] uppercase text-foreground-muted">
            {label}
          </Text>
        </View>
      ) : null}
