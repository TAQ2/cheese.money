...
import type {
  ...
  RuntimeMode,
  ServerConfig as CH3ServerConfig,
...
} from "@ch3tools/shared/composerTrigger";
...
import {
  ...
  scoreQueryMatch,
} from "@ch3tools/shared/searchRanking";
import {
...
export interface ThreadComposerProps {
  ...
  readonly serverConfig: CH3ServerConfig | null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text
          className="max-w-[260px] text-sm font-ch3-bold leading-snug text-foreground"
          numberOfLines={1}
        >
          {props.status.label}
        </Text>
...
export const ThreadComposer = memo(function ThreadComposer(props: ThreadComposerProps) {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      armAgentAwarenessLiveActivityForLocalWork({
        ...
        projectTitle: props.environmentLabel ?? "CH3",
      });
    } finally {
  ...
  function handleOptionsMenuAction(event: string) {
    const providerOptions = applyProviderOptionMenuEvent(
      providerOptionDescriptors,
      event,
            ...
            <View className="flex-row gap-1 pl-1">
              ...
              {props.draftAttachments.length > 3 ? (
                <View className="size-[30px] items-center justify-center rounded-lg bg-subtle-strong">
                  <Text className="text-foreground-muted text-2xs font-ch3-bold">
                    +{props.draftAttachments.length - 3}
                  </Text>
                </View>
              ) : null}
