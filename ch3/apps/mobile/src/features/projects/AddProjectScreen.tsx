...
} from "@ch3tools/client-runtime/operations/projects";
import {
...
} from "@ch3tools/client-runtime/state/projects";
import { CommandId, type EnvironmentId, ProjectId } from "@ch3tools/contracts";
...
function SectionTitle(props: { readonly children: string }) {
  return (
    <Text className="px-1 text-2xs font-ch3-bold tracking-[0.7px] uppercase text-foreground-muted">
      {props.children}
    </Text>
  );
}
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1 gap-0.5">
          <Text className="text-base leading-snug font-ch3-bold">{props.title}</Text>
          {props.subtitle ? (
  ...
  return (
    ...
    >
      {props.loading ? (
        ...
        <Text className="text-base font-ch3-bold text-primary-foreground">{props.label}</Text>
      )}
    </Pressable>
  );
}
...
function EmptyEnvironmentState() {
  ...
  return (
    <View className="items-center gap-3 rounded-2xl bg-card px-5 py-8">
      <Text className="text-center text-lg font-ch3-bold">Environment unavailable</Text>
      ...
      >
        <Text className="text-sm font-ch3-bold text-primary-foreground">Add environment</Text>
      </Pressable>
    </View>
  );
}
    ...
    <AddProjectShell>
      ...
      {repositoryTitle ? (
        <View className="rounded-[24px] bg-card px-4 py-3">
          <Text className="text-base font-ch3-bold">{repositoryTitle}</Text>
