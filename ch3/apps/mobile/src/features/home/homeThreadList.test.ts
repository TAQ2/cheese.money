...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@ch3tools/contracts";
...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      canonicalKey: "github.com/pingdotgg/ch3",
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:pingdotgg/ch3.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("ch3");
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    const environmentId = EnvironmentId.make("environment-1");
    const repositoryIdentity = {
      canonicalKey: "github.com/ch3tools/ch3",
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:ch3tools/ch3.git",
      },
      provider: "github",
      owner: "ch3tools",
      name: "ch3",
      displayName: "CH3",
      rootPath: "/workspaces/ch3",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/ch3/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/ch3/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "CH3" })[0];
