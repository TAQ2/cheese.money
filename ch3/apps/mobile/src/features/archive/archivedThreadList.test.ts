import type { ArchivedSnapshotEntry } from "@ch3tools/client-runtime/state/threads";
import type { OrchestrationProjectShell, OrchestrationThreadShell } from "@ch3tools/contracts";
...
describe("buildArchivedThreadGroups", () => {
  it("groups archived threads by project and sorts newest first", () => {
    const project = makeProject({ id: ProjectId.make("project-1"), title: "CH3" });
  ...
  it("filters by environment and matches project, thread, and branch text", () => {
    ...
    const firstProject = makeProject({ id: ProjectId.make("project-1"), title: "CH3" });
