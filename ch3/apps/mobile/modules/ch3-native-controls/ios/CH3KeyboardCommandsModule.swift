...
public final class CH3KeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("CH3KeyboardCommands")

    View(CH3KeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: CH3KeyboardCommandsView, commands: [String]) in
...
public final class CH3KeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.ch3FirstResponder == nil else { return }
...
private extension UIView {
  var ch3FirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.ch3FirstResponder { return responder }
    }
    return nil
  }
}
