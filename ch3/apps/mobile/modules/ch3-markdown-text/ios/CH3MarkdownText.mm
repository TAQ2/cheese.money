...
#import "CH3MarkdownTextShadowNode.h"
#import "CH3MarkdownTextComponentDescriptor.h"
...
static void CH3MarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<CH3MarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void CH3MarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<CH3MarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = CH3MarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        CH3MarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol CH3MarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface CH3MarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<CH3MarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<CH3MarkdownOutsideTapTarget>)target;

@end

static const void *CH3MarkdownOutsideTapCoordinatorKey =
    &CH3MarkdownOutsideTapCoordinatorKey;

@implementation CH3MarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<CH3MarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<CH3MarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, CH3MarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<CH3MarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static CH3MarkdownOutsideTapCoordinator *
CH3MarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  CH3MarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, CH3MarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[CH3MarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        CH3MarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface CH3MarkdownText () <RCTCH3MarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface CH3MarkdownText () <CH3MarkdownOutsideTapTarget>
@end

@implementation CH3MarkdownText {
  UIView * _view;
  UITextView * _textView;
  CH3MarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<CH3MarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const CH3MarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    CH3MarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, CH3MarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [CH3MarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<CH3MarkdownTextProps const>(_props);
  ...
  CH3MarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  CH3MarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::CH3MarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::CH3MarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<CH3MarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<CH3MarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<CH3MarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == CH3MarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == CH3MarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == CH3MarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == CH3MarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const CH3MarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (CH3MarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[CH3MarkdownTextRun class]]) {
      continue;
    }

    CH3MarkdownTextRun* textChild = (CH3MarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::CH3MarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::CH3MarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> CH3MarkdownTextCls(void)
{
  return CH3MarkdownText.class;
}
