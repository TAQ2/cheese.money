...
import CH3MarkdownTextRunNativeComponent from "./CH3MarkdownTextRunNativeComponent";
import CH3MarkdownTextNativeComponent from "./CH3MarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <CH3MarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <CH3MarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
