...
const ROUTER_HOST = "app.ch3.codes";
const HOSTED_WEB_CHANNEL_COOKIE = "ch3_web_channel";
const LATEST_ORIGIN = "https://latest.app.ch3.codes";
const NIGHTLY_ORIGIN = "https://nightly.app.ch3.codes";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @ch3tools/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@ch3tools/scripts...' --filter '@ch3tools/web...'",
  routes: [
    {
      src: "/__ch3/channel",
