...
import { EnvironmentHttpCommonError, PRIMARY_LOCAL_ENVIRONMENT_ID } from "@ch3tools/contracts";
import type { EnvironmentHttpCommonError as EnvironmentHttpCommonErrorType } from "@ch3tools/contracts";
...
export class PrimaryEnvironmentRequestError extends Schema.TaggedErrorClass<PrimaryEnvironmentRequestError>()(
  "PrimaryEnvironmentRequestError",
  {
    ...
    /**
     * True when the request never got an HTTP response at all — refused,
     * reset, or aborted. `status` still reads 500 for display, but the
     * distinction matters: nothing on the server rejected this, so during
     * bootstrap it is worth waiting for.
     */
    transport: Schema.optional(Schema.Boolean),
  ...
  static fromCause(input: {
    ...
    const transport = isTransportFailure(input.cause);
  ...
  override get message(): string {
    return this.transport === true
      ? `Primary environment request failed during ${this.operation} (no response from the local server).`
...
/**
 * Whether the request never reached the server, as opposed to failing once it
 * got there.
 *
 * Deliberately narrow. "No HTTP status could be read" is far too broad a
 * definition — an encoding failure, a schema mismatch between renderer and
 * server, or a plain programming error all lack one, and none of them
 * improves by being retried for fifteen seconds behind a blank window.
 */
function isTransportFailure(error: unknown): boolean {
  if (error instanceof TypeError) return true;
  if (error instanceof DOMException && error.name === "AbortError") return true;
  // The reason carries the kind: a transport failure never reached the
  // server, while an encode, decode or invalid-url failure is our own bug and
...
export async function retryTransientBootstrap<T>(operation: () => Promise<T>): Promise<T> {
  // The budget starts at the first FAILURE, not at the first attempt. A cold
  // start's first attempt can itself burn more than the whole budget — the
  // local server's first token exchange has been measured at 15 to 39 seconds
  // — and clocking from the start meant that exact case got zero retries and
  // went straight to the error screen, which is the failure this retry exists
  // to absorb.
  ...
  while (true) {
    try {
      ...
      if (deadline === null) {
        deadline = Date.now() + BOOTSTRAP_RETRY_TIMEOUT_MS;
...
function isTransientBootstrapError(error: unknown): boolean {
  if (isPrimaryEnvironmentRequestError(error)) {
    // A request that never reached the server is exactly what a cold start
    // looks like: the window is up before the local server is listening.
    // Without this it was stamped 500 — a status no retry policy treats as
    // worth waiting for — and every launch opened on the error screen until
    // the user hit Reload.
    return error.transport === true || TRANSIENT_BOOTSTRAP_STATUS_CODES.has(error.status);
  }
