...
function installTestBrowser(url: string) {
  ...
  vi.stubGlobal("document", { title: "CH3" });
...
describe("resolveInitialServerAuthGateState", () => {
  ...
  it("derives primary request messages from structural request context", async () => {
    ...
    // A plain Error carries no HTTP status and is not a transport failure
    // either, so it keeps the 500 label and stays fatal — only a genuine
    // "never reached the server" failure is worth waiting out.
    expect(error.transport).toBeUndefined();
    expect(error.message).toBe(
      "Primary environment request failed during list-pairing-links (HTTP 500).",
    );
    // The point of the whole test: the private transport detail never
    // reaches the message.
