...
/**
 * Drives the primary environment's CH3 Connect link. CH3 Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message = cause instanceof Error ? cause.message : "Could not update CH3 Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[ch3-connect] Could not update CH3 Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update CH3 Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn CH3 Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to CH3 Connect before enabling this."));
        return false;
      }
