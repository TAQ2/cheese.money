...
import { EnvironmentRegistry } from "@ch3tools/client-runtime/connection";
import { request, runStream } from "@ch3tools/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@ch3tools/client-runtime/rpc";
import { ManagedRelay } from "@ch3tools/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `CH3 cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `CH3 cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "CH3CODE_RELAY_URL is not configured.",
      });
    }
...
// independently of CH3 Connect while clients reach the environment out of band.
