...
export interface WorkLogEntry {
  ...
  toolCallId?: string;
  /**
   * Model the Task/Agent call asked for ("haiku", "sonnet", ...). Captured
   * here because `toolData` is only carried for MCP tool calls, so the raw
   * agent input is not otherwise reachable downstream.
   */
  agentModel?: string;
  /**
   * The FIRST event's timestamp for a merged tool-lifecycle entry.
   * `createdAt` advances with every update the runtime streams, so elapsed
   * displays (e.g. running subagents) must anchor here, not on `createdAt`.
   */
...
function mergeDerivedWorkLogEntries(
  ...
  // Only the input-carrying update names the model; later updates must not
  // drop it.
  ...
  return {
    ...
    firstCreatedAt: previous.firstCreatedAt ?? previous.createdAt,
  };
}
