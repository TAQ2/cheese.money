...
import {
  ...
  effectiveSnoozed,
  threadWokeAt,
} from "@ch3tools/client-runtime/state/thread-settled";
import type { EnvironmentThreadShell } from "@ch3tools/client-runtime/state/models";
import {
  scopeProjectRef,
  scopeThreadRef,
  scopedThreadKey,
} from "@ch3tools/client-runtime/environment";
import type { ScopedThreadRef, SidebarProjectGroupingMode } from "@ch3tools/contracts";
import {
  AlarmClockIcon,
  AlarmClockOffIcon,
  CheckIcon,
...
/**
 * What an inbox card needs to be dragged. `attributes` is deliberately NOT
 * carried: dnd-kit's accessibility attributes put `role="button"` and a tab
 * stop on whatever they are spread onto, and here that would be the `<li>`
 * inside `role="list"` — invalid, and a second focus target competing with
 * the row's own. Only the pointer sensor is registered, which needs the
 * listeners and the node ref alone.
 */
type SortableThreadHandle = Pick<
  ReturnType<typeof useSortable>,
  "listeners" | "setNodeRef" | "transform" | "transition" | "isDragging"
>;

/**
 * Registers one inbox card with dnd-kit and hands the result to the row,
 * which owns the `<li>` itself (wrapping it in a second element would nest
 * list items and break the list's own spacing).
 *
 * The handle object is memoized so a row whose drag state has not changed
 * keeps identical props and `memo` can still bail out — every active row
 * re-rendering on each streaming shell update is exactly what that memo
 * exists to prevent.
 */
function SortableThreadRow({
  ...
  threadKey: string;
  disabled: boolean;
  children: (handle: SortableThreadHandle) => ReactNode;
}) {
...
const SidebarV2Row = memo(function SidebarV2Row(props: {
  ...
  variant: "card" | "slim";
  /**
   * Set only for cards in the active list, the one section the user can
   * arrange by hand. Slim rows (settled, snoozed) keep their own sorts and
   * never receive it.
   */
  dragHandle?: SortableThreadHandle | null;
  ...
  const isManuallyUnread = useUiStateStore((state) =>
    state.manuallyUnreadThreadKeys.includes(threadKey),
  );
  ...
  // A hand-made mark outranks the visit timestamp: the user said this thread
  // still needs them, and reading it again does not answer that.
  ...
  const handleToggleManualUnreadClick = useCallback(
    (event: ReactMouseEvent) => {
      event.preventDefault();
      event.stopPropagation();
      onToggleManualUnread(threadRef, !isManuallyUnread);
    },
    [isManuallyUnread, onToggleManualUnread, threadRef],
  );
            ...
            {isRegeneratingTitle ? (
              <>
                {/* Renaming shells out to a model, so it is slow enough that a
                    silent row reads as a click that did nothing. */}
                <LoaderIcon
  ...
  const dragHandle = props.dragHandle ?? null;

  return (
    <li
      ref={dragHandle?.setNodeRef}
      // While dragging, the row rides above its neighbours and goes
      // translucent so the gap opening beneath it stays readable.
      style={
        dragHandle
          ? {
              transform: CSS.Translate.toString(dragHandle.transform),
              transition: dragHandle.transition,
              ...(dragHandle.isDragging
                ? { position: "relative" as const, zIndex: 20, opacity: 0.8 }
                : {}),
            }
          : undefined
      }
      ...
      className={cn(
        ...
        // Only while the pointer is down: a resting grab cursor over the
        // whole inbox would bury the fact that a row is primarily a link.
          ...
          <div className="relative z-10 h-[4.875rem] px-2.5 py-2">
              ...
              <span className="group/v2-status-slot relative ml-auto flex h-5 min-w-8 shrink-0 items-stretch justify-end text-xs">
                ...
                <span
                  className={cn(
                    ...
                    // A marked row keeps its envelope visible: that mark is
                    // the reason the row is red, so the way to undo it must
                    // not be hidden behind a hover.
                    isManuallyUnread && "static opacity-100",
                  )}
                >
                  <button
                    type="button"
                    aria-label={isManuallyUnread ? "Clear unread mark" : "Mark unread"}
                    aria-pressed={isManuallyUnread}
                    onClick={handleToggleManualUnreadClick}
                    className={cn(
                      "inline-flex cursor-pointer items-center rounded-md bg-transparent px-1.5 text-xs hover:text-foreground",
                      isManuallyUnread ? "text-foreground" : "text-muted-foreground",
                    )}
                  >
            ...
            <div className="mt-1 flex min-w-0">
              ...
              {isRegeneratingTitle ? (
                <>
                  <LoaderIcon
                    className="text-muted-foreground ml-1 size-3 shrink-0 animate-spin"
                    aria-hidden="true"
                  />
                  <span role="status" className="sr-only">
                    Regenerating title
...
export default function SidebarV2() {
  ...
  const autoArchiveAfterSettledDays = useClientSettings(
    (s) => s.sidebarAutoArchiveAfterSettledDays,
  );
  ...
  const setThreadManuallyUnread = useUiStateStore((s) => s.setThreadManuallyUnread);
  const moveThreadToTopOfOrder = useUiStateStore((s) => s.moveThreadToTopOfOrder);
  const resetThreadOrder = useUiStateStore((s) => s.resetThreadOrder);
  const hasManualThreadOrder = useUiStateStore((s) => s.threadOrder.length > 0);
  const manuallyUnreadThreadKeys = useUiStateStore((s) => s.manuallyUnreadThreadKeys);
  const manuallyUnreadThreadKeysRef = useRef(manuallyUnreadThreadKeys);
  manuallyUnreadThreadKeysRef.current = manuallyUnreadThreadKeys;
  ...
  const threadPartition = useMemo(() => {
    ...
    // Every unarchived thread is classified, in or out of the current
    // project scope: the scope filter belongs to what RENDERS, and the
    // auto-archive sweep must retire the whole settled tail rather than
    // whichever slice of it the user happens to be looking at.
    ...
    const activeAllScopes: EnvironmentThreadShell[] = [];
    const snoozed: EnvironmentThreadShell[] = [];
    const settled: EnvironmentThreadShell[] = [];
    const settledAllScopes: EnvironmentThreadShell[] = [];
    for (const thread of visible) {
      ...
      if (supportsSnooze && effectiveSnoozed(thread, { now: preciseNow })) {
        ...
        settledAllScopes.push(thread);
        if (inScope(thread)) settled.push(thread);
      } else {
        activeAllScopes.push(thread);
        if (inScope(thread)) active.push(thread);
      }
    }
    return {
      activeThreads: sortThreadsForSidebarV2(active),
      // The unscoped inbox. The manual order is global, so anything that
      // WRITES it has to reason about every active thread — writing only the
      // scoped slice would rank those and leave every other project's threads
      // unranked, which sorts them ABOVE the ones just arranged.
      activeThreadsAllScopes: sortThreadsForSidebarV2(activeAllScopes),
      ...
      settledThreadsAllScopes: sortSettledThreadsForSidebarV2(settledAllScopes),
  ...
  const {
    activeThreads: sortedActiveThreads,
    activeThreadsAllScopes,
    snoozedThreads,
    settledThreads,
    settledThreadsAllScopes,
    snoozeNow,
  } = threadPartition;

  // The inbox is the one list the user arranges by hand (drag a card up or
  // down); settled and snoozed keep their own sorts. Applying the manual
  // order HERE — not at the render — means the jump shortcuts, shift-range
  // select and forward-navigation, which all read `orderedThreads` below,
  ...
  // Read by the drag handlers, which must not be rebuilt on every shell
  // update — the same reason the click handler reads its ordered list
  // through a ref.
  const activeThreadKeysRef = useRef(activeThreadKeys);
  activeThreadKeysRef.current = activeThreadKeys;
  // The same list without the project-scope filter, ordered the way the whole
  // inbox would render. Anything that writes the global manual order works
  // from this rather than from what happens to be on screen.
  const allScopesActiveThreadKeys = useMemo(
    () =>
      orderThreadsByManualOrder({
        threads: activeThreadsAllScopes,
        manualOrder: threadOrder,
        getKey: (thread) => scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)),
      }).map((thread) => scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id))),
    [activeThreadsAllScopes, threadOrder],
  );
  ...
  /**
   * Float thread keys to the head of the inbox, last one ending on top.
   *
   * Each float rewrites the whole visible order, so feeding every call the
   * same pre-float list would leave only the last one moved and quietly undo
   * the rest. The running order is carried forward instead — the ref behind
   * it only catches up on the next render.
   */
  const floatThreadKeysToTop = useCallback(
    (threadKeys: readonly string[]) => {
      // The order written is the UNSCOPED inbox. Writing the scoped slice
      // would rank only the project in view, leaving every other project's
      // threads unranked — and unranked sorts above ranked, so the row that
      // just floated would end up buried the moment the scope widened.
      let visibleOrder = allScopesActiveThreadKeysRef.current;
      for (const threadKey of threadKeys) {
        if (!visibleOrder.includes(threadKey)) continue;
  ...
  // the inbox, exactly like the hand-made unread mark does. Without this the
  // static v2 sort leaves the finished row wherever it was opened, which is
  // where it gets missed.
  const threadLastVisitedAtById = useUiStateStore((store) => store.threadLastVisitedAtById);
  const completionFloatStampsRef = useRef<Map<string, string | null>>(new Map());
  useEffect(() => {
    // Never mid-gesture: the float rewrites the order the drop is measured
    // against, so a completion landing while a card is in hand would move the
    // list under the pointer. Skipping is safe — the stamps are not recorded
    // either, so the completion is still pending on the next pass.
    if (threadDragInProgressRef.current) return;
    const decision = selectThreadsToFloatOnCompletion({
      // Every active thread, not just the ones in scope: a thread finishing in
      // another project still has to take its place, and narrowing the scope
      // must not make the tracker forget threads and re-seed them.
      candidates: activeThreadsAllScopes.map((thread) => {
        const threadKey = scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id));
        return {
          threadKey,
          completedAt: thread.latestTurn?.completedAt,
          // The open thread never floats. `isUnread` cannot be trusted for it:
          // this effect runs before the chat view stamps the visit, so on the
          // commit carrying the completion the open row still reads unread.
          isUnread:
      ...
      previousCompletionByKey: completionFloatStampsRef.current,
    });
    completionFloatStampsRef.current = decision.nextCompletionByKey;
    floatThreadKeysToTop(decision.keysToFloat);
  }, [activeThreadsAllScopes, floatThreadKeysToTop, routeThreadKey, threadLastVisitedAtById]);

  // Auto-archive the deep settled tail. Settling has no exit of its own —
  // the projector never archives — so the tail otherwise grows without
  // bound and Delete is the only way out of it in v2. The sweep rides the
  // same minute tick as the partition (day-granular thresholds don't need
  // better) and dispatches one archive at a time so a first run over a long
  // backlog doesn't burst the command channel.
  //
  // Keyed by thread, not cleared on success: the archived thread leaves the
  // partition on its own. A FAILED attempt stays marked too, so a thread the
  // server keeps rejecting is tried once per app start rather than once per
  // minute forever.
  const autoArchiveAttemptedRef = useRef<Set<string>>(new Set());
  const autoArchiveRunningRef = useRef(false);
  useEffect(() => {
    if (autoArchiveRunningRef.current) return;
    const due = selectThreadsToAutoArchive({
      settledThreads: settledThreadsAllScopes,
      now: `${nowMinute}:00.000Z`,
      autoArchiveAfterSettledDays,
      attemptedThreadKeys: autoArchiveAttemptedRef.current,
      routeThreadKey,
      toThreadKey: (thread) => scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)),
    });
    ...
    void (async () => {
      try {
        for (const thread of due) {
          ...
          // Failures stay silent (the archive command already runs with
          // reportFailure: false): this is unattended housekeeping the user
          // never asked for right now, and a toast per stale thread on a
          // flaky connection would bury the sidebar. The thread stays marked
          // as attempted either way, so the sweep moves on.
          await archiveThread(threadRef);
        }
      } catch {
        // A rejected dispatch (transport error) must not surface as an
        // unhandled rejection; the next app start retries.
      } finally {
  ...
  // Collapsed on open, like the snoozed shelf below. Settled threads are
  // finished work: expanded by default they push the live threads — the reason
  // the sidebar is open — down the list on every launch. The open thread is
  // still surfaced through the collapsed shelf (see below), so nothing the user
  // is actually looking at can hide behind it.
  ...
  /**
   * The hand-made unread mark, from the row's envelope or the context menu.
   *
   * Marking also floats the thread to the top of the inbox: an unread mark
   * means "come back to this", and leaving it wherever the sort put it is
   * exactly how it gets forgotten. Clearing only clears — the row keeps the
   * position it has.
   */
  const toggleThreadManualUnread = useCallback(
    (threadRef: ScopedThreadRef, unread: boolean) => {
      const threadKey = scopedThreadKey(threadRef);
      setThreadManuallyUnread(threadKey, unread);
      if (unread) {
        floatThreadKeysToTop([threadKey]);
      }
    },
    [floatThreadKeysToTop, setThreadManuallyUnread],
  );

  const handleThreadClick = useCallback(
    (event: ReactMouseEvent, threadRef: ScopedThreadRef) => {
      // A card that was just dragged is not a card that was clicked.
  ...
  const handleMultiSelectContextMenu = useCallback(
    async (position: { x: number; y: number }) => {
      ...
      if (clicked.value === "mark-unread") {
        // Mark every selected thread, then float them in one pass, bottom-up
        // so the topmost selected row ends up topmost. Floating inside the
        // loop would have each move rewrite the visible order from a list
        // that predates the previous move, leaving only the last one applied.
        const markedKeys: string[] = [];
        for (const threadKey of threadKeys) {
          const thread = threadByKeyRef.current.get(threadKey);
          if (!thread) continue;
          setThreadManuallyUnread(threadKey, true);
          markedKeys.push(threadKey);
        }
        // Bottom-up in VISIBLE order, so the topmost selected row ends up
      ...
      void (async () => {
        ...
        const clicked = await settlePromise(() =>
          api.contextMenu.show(
            [
              ...
              {
                id: "mark-unread",
                label: manuallyUnreadThreadKeysRef.current.includes(threadKey)
                  ? "Clear unread"
                  : "Mark unread",
              },
              ...(hasManualThreadOrder
                ? [{ id: "reset-thread-order", label: "Reset manual order" }]
                : []),
        ...
        switch (clicked.value) {
          ...
          case "regenerate-title": {
            ...
            // The command only reports that the rename was *scheduled*.
            // Generation happens on the server and its failures are logged, not
            // returned, so watch the thread until the pending marker clears and
            // report what actually happened. Without this a failed rename is
            // indistinguishable from a click that did nothing.
            const titleBefore = thread.title;
            const startedAt = Date.now();
            while (Date.now() - startedAt < 60_000) {
              await new Promise((resolve) => setTimeout(resolve, 400));
              const current = threadByKeyRef.current.get(threadKey);
              if (!current) return;
              if (current.titleRegeneration != null) continue;
              if (current.title !== titleBefore) return; // The new name is the feedback.
              toastManager.add(
                stackedThreadToast({
                  type: "error",
                  title: "Rename produced no new name",
                  description:
                    "The text generation model returned nothing usable. Check Settings → Text generation model.",
                }),
              );
              return;
            }
          ...
          case "mark-unread":
            toggleThreadManualUnread(
              scopeThreadRef(thread.environmentId, thread.id),
              !manuallyUnreadThreadKeysRef.current.includes(threadKey),
            );
            return;
          case "reset-thread-order":
            resetThreadOrder();
            return;
          case "copy-path":
  ...
  // Held so a drag can switch it off: auto-animate and dnd-kit both animate
  // the same rows from the same DOM reorder, and run together the dropped
  // card visibly jumps — dnd-kit eases it into its new slot while
  // auto-animate FLIPs it from the old one.
  ...
  const resumeListAutoAnimate = useCallback(() => {
    // Two frames: the reorder lands in a React commit that has not happened
    // yet when this returns, and re-enabling before it would animate the
    // very reorder we are suppressing.
    window.requestAnimationFrame(() => {
      window.requestAnimationFrame(() => listAutoAnimateRef.current?.enable());
    });
  }, []);

  // 6px before a drag starts, exactly as the project list does it: less and
  // a click on a card turns into a drag; more and the drag feels stuck.
  ...
  const handleThreadDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (over && active.id !== over.id) {
        reorderThreads({
          visibleThreadOrder: activeThreadKeysRef.current,
          draggedThreadKeys: [String(active.id)],
          targetThreadKeys: [String(over.id)],
        });
      }
      // After the reorder is dispatched, never before: the resume waits two
      // frames for the commit this call causes, and asking for those frames
      // first only narrows the margin.
      resumeListAutoAnimate();
    },
    [reorderThreads, resumeListAutoAnimate],
  );
  ...
  // The click that follows a drag's pointerup would otherwise open the thread
  // the user was only moving. Cleared on the next press rather than on a
  // timer, so a drag that ends without a click can't eat the click after it.
  const handleThreadListPointerDownCapture = useCallback((event: ReactPointerEvent) => {
    suppressThreadClickAfterDragRef.current = false;
    // A macOS Ctrl+click reports button 0, which is exactly what the drag
    // sensor arms on. Stopping it here — the same guard the project list
    // uses — keeps a context-menu press from starting a reorder whose
    // pointerup may never arrive while the native menu is open.
    if (
      isContextMenuPointerDown({
        button: event.button,
              ...
              >
                <SortableContext items={activeThreadKeys} strategy={verticalListSortingStrategy}>
                  {(() => {
                    ...
                    // Only the inbox is draggable. The wrapper keeps the key the
                    // card used to carry, so a settling thread still unmounts its
                    // card and mounts a slim row rather than one element sliding
