...
import type { EnvironmentId, ScopedThreadRef, ServerProviderSkill } from "@ch3tools/contracts";
import {
...
import {
  ...
  type InlineCodeFileProbe,
} from "./chat/inlineCodeTarget";
...
/**
 * Wide enough that a common basename returns every file that carries it, so
 * "several files answer to this name" is a real answer rather than an
 * artefact of truncation.
 */
const INLINE_CODE_PROBE_SEARCH_LIMIT = 50;

const INLINE_OPEN_BUTTON_CLASS_NAME =
  "inline-flex size-4 shrink-0 translate-y-[0.1em] cursor-pointer select-none items-center justify-center rounded-sm text-muted-foreground/60 transition-colors hover:bg-accent hover:text-foreground";

/**
 * The arrow beside a path or a link: one click, straight out of the app.
 *
 * It sits OUTSIDE the span it belongs to rather than inside it, so selecting
 * and copying the path still yields the path and nothing else — a button
 * nested in the text would come along with it.
 */
function InlineOpenExternalButton({
  label,
  onOpen,
}: {
  label: string;
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        render={
          <button
            ...
            aria-label={label}
            data-markdown-copy=""
            className={INLINE_OPEN_BUTTON_CLASS_NAME}
            onClick={(event) => {
              event.preventDefault();
      ...
      />
      <TooltipPopup side="top" className="max-w-[min(36rem,calc(100vw-2rem))] leading-tight">
        {label}
      </TooltipPopup>
    </Tooltip>
  );
...
/**
 * Opens a web link the way the surrounding app does: through the desktop
 * shell when there is one, and a new tab in the browser build.
 */
function openExternalWebLink(href: string): void {
  ...
  if (api) {
    void (async () => {
      try {
        ...
        reportMarkdownActionFailure({ operation: "open-external-link", target: href }, cause);
      }
    })();
    return;
  }
...
const MarkdownFileLink = memo(function MarkdownFileLink({
  ...
  return (
    <span className="inline-flex max-w-full items-center gap-0.5 align-baseline">
      <Tooltip>
        <TooltipTrigger
          render={
            <a
...
/**
 * An inline code span that might name a project file, rendered as a link only
 * once the project's file index confirms exactly which file it is.
 *
 * The lookup is by BASENAME, so a span carrying no directory at all can still
 * be placed. Until an answer arrives — and forever, if the answer is "no such
 * file" or "several" — the span renders as ordinary code, so nothing ever
 * flickers into an affordance that opens the wrong thing.
 */
function InlineCodeProbedFileLink({
  probe,
  cwd,
  environmentId,
  enabled,
  ...
  enabled: boolean;
  renderResolved: (meta: MarkdownFileLinkMeta) => ReactNode;
  fallback: ReactNode;
}): ReactNode {
  const { entries } = useProjectPathSearch(
    {
      environmentId: enabled ? environmentId : null,
      cwd: enabled ? cwd : null,
      query: enabled ? probe.fileName : null,
      kind: "file",
    },
    INLINE_CODE_PROBE_SEARCH_LIMIT,
  );
  const meta = useMemo(() => {
    if (entries.length === 0) return null;
    const resolvedPath = resolveInlineCodeProbeEntry(probe, entries);
    if (!resolvedPath) return null;
    return buildVerifiedFileLinkMeta(resolvedPath, cwd, { line: probe.line, column: probe.column });
  }, [cwd, entries, probe]);

  return meta ? renderResolved(meta) : fallback;
}
    ...
    return {
      ...
      code({ node, children, className, ...props }) {
        if (node?.properties?.dataInlineCode != null) {
          ...
          // A link the model wrote inside backticks never became an anchor,
          // so until now the only way to follow it was to select and paste
          // it by hand.
          const urlTarget = detectInlineCodeUrl(codeText);
          if (urlTarget) {
            return (
              <span className="inline-flex max-w-full items-center gap-0.5 align-baseline">
                {plainCode}
                <InlineOpenExternalButton
                  label={`Open ${urlTarget.href}`}
                  onOpen={() => openExternalWebLink(urlTarget.href)}
                />
              </span>
            );
          }
          // Left over: spans that could name a file but do not prove it —
          // a bare filename, or a path through directories with spaces in
          // them. Only the project can settle those, and asking it while
          // the message is still streaming would query a moving target.
          const probe = cwd ? detectInlineCodeFileProbe(codeText) : null;
          if (probe && cwd) {
            return (
              <InlineCodeProbedFileLink
                probe={probe}
                cwd={cwd}
                environmentId={threadRef?.environmentId ?? environmentId}
                enabled={isStreaming !== true}
                renderResolved={(meta) => fileLinkChip(meta, `\`${codeText}\``)}
                fallback={plainCode}
              />
            );
          }
