...
export interface CompletionFloatCandidate {
  readonly threadKey: string;
  readonly completedAt: string | null | undefined;
  readonly isUnread: boolean;
}

export interface CompletionFloatDecision {
  /** Thread keys to float, oldest completion first, so the newest ends up on top. */
  readonly keysToFloat: string[];
  /**
   * What each thread's completion looked like this pass — `null` for a thread
   * that has not completed. Threads are recorded whether or not they float,
   * because a key's PRESENCE is what marks it as already seen.
   */
  readonly nextCompletionByKey: Map<string, string | null>;
}

/**
 * Decide which threads just finished and should jump to the head of the inbox.
 *
 * The v2 sort is static creation order — a row holds its position from open
 * until settled — so a turn finishing moves nothing on its own. A finished
 * thread that the user has not read is the definition of "needs attention",
 * which is what the top of the inbox is for, so it floats there the same way
 * a hand-made unread mark does.
 *
 * Seeding is PER THREAD, not one flag for the whole list: a thread floats only
 * if it was already known on an earlier pass AND its completion stamp has
 * since changed. A thread seen for the first time is recorded and left alone.
 *
 * That distinction is what makes this safe against the list arriving late or
 * partially, which it always does — the shell snapshot starts empty, each
 * environment streams in separately, a reconnect empties it again, and
 * switching project scope narrows it. A single "have I run once?" flag would
 * treat the whole first real batch as freshly completed and float all of it,
 * reshuffling the inbox on every launch around threads that finished days ago
 * — the precise thing this is meant to avoid.
 *
 * Callers must also exclude the thread the user currently has open. Relying on
 * `isUnread` alone would be a race: the sidebar's effects run before the chat
 * view stamps the visit, so on the commit that carries the completion the open
 * thread still reads as unread.
 */
export function selectThreadsToFloatOnCompletion(input: {
  candidates: readonly CompletionFloatCandidate[];
  previousCompletionByKey: ReadonlyMap<string, string | null>;
}): CompletionFloatDecision {
  const { candidates, previousCompletionByKey } = input;
  const nextCompletionByKey = new Map<string, string | null>();
  const keysToFloat: { threadKey: string; completedAtMs: number }[] = [];

  for (const candidate of candidates) {
    const completedAt = candidate.completedAt ?? null;
    const completedAtMs = completedAt === null ? Number.NaN : Date.parse(completedAt);
    // An unparseable stamp is recorded as "not completed" rather than skipped,
    // so the thread still counts as seen and cannot float later on a stamp
    // nobody can compare.
    const normalizedCompletedAt = Number.isNaN(completedAtMs) ? null : completedAt;

    const seenBefore = previousCompletionByKey.has(candidate.threadKey);
    const previousCompletedAt = previousCompletionByKey.get(candidate.threadKey) ?? null;
    nextCompletionByKey.set(candidate.threadKey, normalizedCompletedAt);

    if (!seenBefore) continue;
    if (normalizedCompletedAt === null) continue;
    if (previousCompletedAt === normalizedCompletedAt) continue;
    if (!candidate.isUnread) continue;
    keysToFloat.push({ threadKey: candidate.threadKey, completedAtMs });
  }

  return {
    keysToFloat: keysToFloat
...
/**
 * Applies the user's manual inbox order on top of the v2 sort.
 *
 * A thread the manual order has never heard of stays ABOVE the arranged
 * block rather than sinking below it, which is the opposite of
 * `orderItemsByPreferredIds`. The inbox is a queue of what needs attention:
 * a thread created after the last drag must arrive where new threads always
 * arrive — at the top — instead of hiding under rows that were placed by
 * hand weeks ago. Arranged rows keep their exact relative order among
 * themselves, and unknown rows keep the sort they came in with.
 *
 * Dragging anything writes the whole visible list into the manual order, so
 * "unknown" means precisely "created since the last drag".
 */
export function orderThreadsByManualOrder<T>(input: {
  threads: readonly T[];
  manualOrder: readonly string[];
  getKey: (thread: T) => string;
}): T[] {
  const { getKey, manualOrder, threads } = input;
  if (manualOrder.length === 0) {
    return [...threads];
  }
  const rankByKey = new Map(manualOrder.map((key, index) => [key, index] as const));
  const unranked: T[] = [];
  const ranked: { thread: T; rank: number }[] = [];
  for (const thread of threads) {
    const rank = rankByKey.get(getKey(thread));
    if (rank === undefined) {
      unranked.push(thread);
    } else {
  ...
  return [
    ...unranked,
    ...ranked.toSorted((left, right) => left.rank - right.rank).map((entry) => entry.thread),
  ];
}
...
/** Settled threads the auto-archive sweep should retire. Settling is a
    lifecycle state with no exit of its own — nothing in the projector ever
    archives a thread — so without a sweep the tail grows forever and Delete
    is the only way out of it in v2. Measured from resolveSettledTimestamp,
    the same instant the row's "settled N ago" label reports, so what the
    sweep acts on is what the user was told.

    Archiving is reversible (Settings → Archived un-archives), which is why
    this can run unattended; deleting on a timer never could. */
export function selectThreadsToAutoArchive<
...
>(input: {
  ...
  readonly now: string;
  readonly autoArchiveAfterSettledDays: number | null;
  /** Threads already dispatched this session: archive is not idempotent
      server-side (thread.archive rejects an archived thread), and a failed
      attempt must not retry every minute tick. */
  readonly attemptedThreadKeys: ReadonlySet<string>;
  /** The open thread is never swept: archiving it navigates the user to a
      fresh draft mid-read. It retires once they move off it. */
  readonly routeThreadKey: string | null;
  readonly toThreadKey: (thread: T) => string;
}): T[] {
  const { autoArchiveAfterSettledDays } = input;
  if (autoArchiveAfterSettledDays === null) return [];
  const nowMs = Date.parse(input.now);
  // A malformed clock never archives anything.
  if (Number.isNaN(nowMs)) return [];
  const cutoffMs = nowMs - autoArchiveAfterSettledDays * DAY_MS;

  return input.settledThreads.filter((thread) => {
    const key = input.toThreadKey(thread);
    if (key === input.routeThreadKey) return false;
    if (input.attemptedThreadKeys.has(key)) return false;
    const settledAt = resolveSettledTimestamp(thread);
    // No resolvable settle time means no age to measure against: leave it.
    if (settledAt === null) return false;
    const settledAtMs = Date.parse(settledAt);
    if (Number.isNaN(settledAtMs)) return false;
    return settledAtMs < cutoffMs;
  });
}
