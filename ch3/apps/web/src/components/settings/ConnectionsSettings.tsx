...
import { connectionStatusText } from "@ch3tools/client-runtime/connection";
import {
...
function SavedBackendListRow({
  ...
  const metadataBits = [
    ...
    environment.relayManaged ? "CH3 Connect" : null,
  ].filter((value): value is string => value !== null);
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          {versionMismatch ? (
            <p className="flex items-center gap-1 text-warning text-xs">
              ...
              {versionMismatch.serverVersion}. {versionMismatch.hint}
            </p>
          ) : null}
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to CH3 Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage CH3 Connect access."
      : null;
    ...
    if (ok) {
      ...
      toastManager.add({
        ...
        title: enabled
          ? "CH3 Connect linked"
          : publishAgentActivity
            ? "CH3 Connect tunnel disabled"
            : "CH3 Connect unlinked",
        description: enabled
          ? "This environment is available through CH3 Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through CH3 Connect.",
      });
    }
    setIsUpdating(false);
  };
  ...
  return (
    <>
      <SettingsRow
        ...
        description={
          managedTunnelActive
            ? "This environment is available to your other devices through CH3 Connect."
            : "Make this environment available to your other devices through CH3 Connect."
        }
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without a CH3 Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from CH3 Connect."
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. CH3 restarts when you change this."
              ...
              <SettingsRow
                ...
                description={
                  <span className="flex items-center gap-1 text-warning">
                    ...
                    {primaryVersionMismatch.serverVersion}. {primaryVersionMismatch.hint}
                  </span>
                }
              />
            ) : null}
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "CH3 will restart to expose this environment over the network."
                    : "CH3 will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "CH3 will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in CH3 until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "CH3 will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "CH3 will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "CH3 will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  CH3 will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  CH3 will restart the local backend with Tailscale Serve enabled and ask Tailscale
