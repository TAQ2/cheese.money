...
import { safeErrorLogAttributes } from "@ch3tools/client-runtime/errors";
import {
...
import {
  ...
  MAX_GLASS_OPACITY,
  MIN_GLASS_OPACITY,
} from "@ch3tools/contracts/settings";
import {
  getBackgroundActivityBaseProfile,
  getBackgroundActivityPresetSettings,
  resolveServerBackgroundActivitySettings,
} from "@ch3tools/shared/backgroundActivitySettings";
...
import { describeSpeechVoice, SPEECH_SPANISH_VOICES, SPEECH_VOICES } from "./speechVoices";

const DEFAULT_SPEECH_VOICE_ID = "en-GB-RyanNeural";
const DEFAULT_SPEECH_SPANISH_VOICE_ID = "es-MX-JorgeNeural";
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="Appearance">
        <SettingsRow
          title="Theme"
          description="Choose how CH3 looks across the app."
          resetAction={
            theme !== "system" ? (
...
/**
 * Read-aloud settings, on their own tab.
 *
 * The voice list only appears when the server this client is attached to
 * actually has a speech engine; without one the tab explains that rather than
 * offering a control that cannot work.
 *
 * The voice is a per-device preference held in local UI state, not a synced
 * setting: which voice you want depends on the speakers you are sitting in
 * front of.
 */
/**
 * Claude account management on its own tab.
 *
 * The same manager the provider card hosts in a popover, given a permanent
 * home: which account is active, its plan headroom, adding and switching
 * accounts, and the automatic hand-over toggle. The popover stays for quick
 * switches; this is where accounts LIVE.
 */
export function AccountsSettingsPanel() {
  ...
  // The same instance the automatic hand-over acts on: the default Claude
  // slot. The explicit entry exists once anything was ever changed; before
  // that the legacy provider block is the instance in effect.
  ...
  const legacyConfig = settings.providers.claudeAgent;
  ...
  const updateConfigValue = (key: string, value: unknown) => {
    updateSettings(
      buildProviderInstanceUpdatePatch({
        settings,
        instanceId,
        instance: { ...instance, config: { ...config, [key]: value } },
        driver: ProviderDriverKind.make("claudeAgent"),
        isDefault: true,
      }),
    );
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="Claude accounts">
        <div className="grid gap-2 px-3 py-2">
          <p className="text-xs leading-snug text-muted-foreground">
            Each account is its own config directory, so switching keeps every account signed in and
            is instantly reversible. Conversations are shared across accounts — a thread started on
            one keeps working after you switch.
          </p>
          {environmentId ? (
            <ClaudeAccountsManager
              ...
              onSelectHomePath={(nextHomePath) => updateConfigValue("homePath", nextHomePath)}
              failoverEnabled={failoverEnabled}
              onFailoverEnabledChange={(enabled) =>
                updateConfigValue("accountFailoverEnabled", enabled)
              }
            ...
            <p className="text-xs text-muted-foreground">
              Accounts appear once a server connection is established.
            </p>
          )}
        </div>
      </SettingsSection>
...
export function SpeechSettingsPanel() {
  const speechAvailable =
    useAtomValue(primaryServerConfigAtom)?.environment.capabilities.speech === true;
  const speechVoice = useUiStateStore((state) => state.speechVoice);
  const setSpeechVoice = useUiStateStore((state) => state.setSpeechVoice);
  const speechVoiceSpanish = useUiStateStore((state) => state.speechVoiceSpanish);
  const setSpeechVoiceSpanish = useUiStateStore((state) => state.setSpeechVoiceSpanish);
  const speechLanguageMode = useUiStateStore((state) => state.speechLanguageMode);
  const setSpeechLanguageMode = useUiStateStore((state) => state.setSpeechLanguageMode);

  const selected = SPEECH_VOICES.find((voice) => voice.id === speechVoice);
  const selectedSpanish = SPEECH_SPANISH_VOICES.find((voice) => voice.id === speechVoiceSpanish);
      ...
      <SettingsPageContainer>
        <SettingsSection title="Text to speech">
          <SettingsRow
            ...
            description="This server does not offer speech synthesis. Current servers always do — an older server needs updating before the voices appear here."
            control={null}
          />
        </SettingsSection>
      </SettingsPageContainer>
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="Text to speech">
        <SettingsRow
          title="Voice"
          description="Reads replies written in English. Each reply's language is detected on the server, which then picks between this voice and the Spanish one."
          resetAction={
            speechVoice !== DEFAULT_SPEECH_VOICE_ID ? (
              <SettingResetButton
                label="voice"
                onClick={() => setSpeechVoice(DEFAULT_SPEECH_VOICE_ID)}
              />
            ) : null
          }
          control={
            <Select value={speechVoice} onValueChange={(value) => value && setSpeechVoice(value)}>
              <SelectTrigger className="w-full sm:w-72" aria-label="Speech voice">
                <SelectValue>{selected ? describeSpeechVoice(selected) : speechVoice}</SelectValue>
              </SelectTrigger>
              <SelectPopup align="end" alignItemWithTrigger={false}>
                {SPEECH_VOICES.map((voice) => (
                  <SelectItem hideIndicator key={voice.id} value={voice.id}>
                    {describeSpeechVoice(voice)}
                  </SelectItem>
                ))}
              </SelectPopup>
            </Select>
        ...
        <SettingsRow
          title="Language"
          description="Detect routes each reply by its own language. Pin a language if detection ever guesses wrong for your replies."
          resetAction={
            speechLanguageMode !== "detect" ? (
              <SettingResetButton
                label="language handling"
                onClick={() => setSpeechLanguageMode("detect")}
              />
            ) : null
          }
          control={
            <Select
              ...
              onValueChange={(value) =>
                (value === "detect" || value === "english" || value === "spanish") &&
                setSpeechLanguageMode(value)
              }
            >
              <SelectTrigger className="w-full sm:w-72" aria-label="Speech language handling">
                <SelectValue>
                  {speechLanguageMode === "detect"
                    ? "Detect per reply"
                    : speechLanguageMode === "english"
                      ? "Always English voice"
                      : "Always Spanish voice"}
                </SelectValue>
              </SelectTrigger>
              <SelectPopup align="end" alignItemWithTrigger={false}>
                <SelectItem hideIndicator value="detect">
                  Detect per reply
                </SelectItem>
                <SelectItem hideIndicator value="english">
                  Always English voice
                </SelectItem>
                <SelectItem hideIndicator value="spanish">
                  Always Spanish voice
                </SelectItem>
              </SelectPopup>
            </Select>
        ...
        <SettingsRow
          title="Spanish voice"
          description="Reads replies detected as Spanish."
          resetAction={
            speechVoiceSpanish !== DEFAULT_SPEECH_SPANISH_VOICE_ID ? (
              <SettingResetButton
                label="Spanish voice"
                onClick={() => setSpeechVoiceSpanish(DEFAULT_SPEECH_SPANISH_VOICE_ID)}
              />
            ) : null
          }
          control={
            <Select
              value={speechVoiceSpanish}
              onValueChange={(value) => value && setSpeechVoiceSpanish(value)}
            >
              <SelectTrigger className="w-full sm:w-72" aria-label="Spanish speech voice">
                <SelectValue>
                  {selectedSpanish ? describeSpeechVoice(selectedSpanish) : speechVoiceSpanish}
                </SelectValue>
              </SelectTrigger>
              <SelectPopup align="end" alignItemWithTrigger={false}>
                {SPEECH_SPANISH_VOICES.map((voice) => (
                  <SelectItem hideIndicator key={voice.id} value={voice.id}>
                    {describeSpeechVoice(voice)}
                  </SelectItem>
                ))}
