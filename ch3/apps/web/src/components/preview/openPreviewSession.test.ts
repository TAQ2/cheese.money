import type { PreviewOpenInput, PreviewSessionSnapshot, ScopedThreadRef } from "@ch3tools/contracts";
...
describe("openPreviewSession", () => {
  ...
  it("applies the RPC response without waiting for a preview event", async () => {
    ...
    expect(open).toHaveBeenCalledWith({ threadId: "thread-1", url: "ch3.chat" });
    expect(readThreadPreviewState(threadRef).snapshot).toEqual(snapshot);
    expect(readThreadPreviewState(threadRef).recentlySeenUrls).toEqual(["https://ch3.chat/"]);
  });
