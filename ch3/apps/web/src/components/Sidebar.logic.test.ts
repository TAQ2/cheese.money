...
describe("getSidebarThreadIdsToPrewarm", () => {
  it("returns only the first visible thread ids up to the prewarm limit", () => {
    expect(getSidebarThreadIdsToPrewarm(["t1", "t2", "ch3"], 2)).toEqual(["t1", "t2"]);
  });
...
describe("orderThreadsByManualOrder", () => {
  const order = (threads: readonly string[], manualOrder: readonly string[]) =>
    orderThreadsByManualOrder({
  ...
  it("leaves the incoming sort alone when nothing was ever dragged", () => {
    expect(order(["a", "b", "c"], [])).toEqual(["a", "b", "c"]);
  });

  it("arranges known threads by the manual order", () => {
    expect(order(["a", "b", "c"], ["c", "a", "b"])).toEqual(["c", "a", "b"]);
  });

  it("keeps threads created since the last drag at the top", () => {
    // "new" is not in the manual order: it stays where the v2 sort put it,
    // above the arranged block, instead of sinking to the bottom.
    expect(order(["new", "a", "b"], ["b", "a"])).toEqual(["new", "b", "a"]);
  });

  it("ignores manual entries for threads that are not in this list", () => {
    expect(order(["a", "b"], ["gone", "b", "elsewhere", "a"])).toEqual(["b", "a"]);
  });
});
...
describe("selectThreadsToAutoArchive", () => {
  ...
  const settled = (input: {
    ...
    latestUserMessageAt: input.latestUserMessageAt ?? null,
    latestTurn: null,
    updatedAt: input.updatedAt ?? "2026-03-20T12:00:00.000Z",
  });
  const select = (
    settledThreads: ReadonlyArray<ReturnType<typeof settled>>,
    overrides: {
      autoArchiveAfterSettledDays?: number | null;
      attemptedThreadKeys?: ReadonlySet<string>;
    ...
    selectThreadsToAutoArchive({
      settledThreads,
      now,
      // `?? 4` would swallow an explicit null (the "off" case).
      autoArchiveAfterSettledDays:
        overrides.autoArchiveAfterSettledDays === undefined
          ? 4
          : overrides.autoArchiveAfterSettledDays,
      attemptedThreadKeys: overrides.attemptedThreadKeys ?? new Set<string>(),
      routeThreadKey: overrides.routeThreadKey ?? null,
      toThreadKey: (thread) => thread.id,
    }).map((thread) => thread.id);

  it("archives threads settled longer than the window", () => {
    expect(
      select([
        settled({ id: "old", settledAt: "2026-03-15T11:00:00.000Z" }),
        settled({ id: "just-inside", settledAt: "2026-03-16T13:00:00.000Z" }),
      ]),
    ).toEqual(["old"]);
  });

  it("leaves a thread settled exactly the window ago", () => {
    expect(select([settled({ id: "boundary", settledAt: "2026-03-16T12:00:00.000Z" })])).toEqual(
      [],
    );
  });

  it("measures auto-settled threads from their last activity, not settledAt", () => {
    // No settledAt stamp: the same fallback the row's label uses.
    expect(
      select([
        settled({ id: "auto-old", latestUserMessageAt: "2026-03-10T12:00:00.000Z" }),
        settled({ id: "auto-recent", latestUserMessageAt: "2026-03-19T12:00:00.000Z" }),
      ]),
    ).toEqual(["auto-old"]);
  });

  it("never archives the open thread", () => {
    expect(
      select([settled({ id: "open", settledAt: "2026-03-01T12:00:00.000Z" })], {
        routeThreadKey: "open",
      }),
    ).toEqual([]);
  });

  it("skips threads already attempted this session", () => {
    expect(
      select([settled({ id: "retry", settledAt: "2026-03-01T12:00:00.000Z" })], {
        attemptedThreadKeys: new Set(["retry"]),
      }),
    ).toEqual([]);
  });

  it("archives nothing when the setting is off", () => {
    expect(
      select([settled({ id: "ancient", settledAt: "2025-01-01T12:00:00.000Z" })], {
        autoArchiveAfterSettledDays: null,
      }),
    ).toEqual([]);
  });

  it("archives nothing on an unparseable clock", () => {
    expect(
      selectThreadsToAutoArchive({
        settledThreads: [settled({ id: "old", settledAt: "2026-03-01T12:00:00.000Z" })],
        now: "not-a-date",
        autoArchiveAfterSettledDays: 4,
        attemptedThreadKeys: new Set<string>(),
        routeThreadKey: null,
        toThreadKey: (thread) => thread.id,
      }),
    ).toEqual([]);
...
describe("selectThreadsToFloatOnCompletion", () => {
  const pass = (
    candidates: Parameters<typeof selectThreadsToFloatOnCompletion>[0]["candidates"],
    previousCompletionByKey: ReadonlyMap<string, string | null> = new Map(),
  ) => selectThreadsToFloatOnCompletion({ candidates, previousCompletionByKey });

  it("records a thread the first time it is seen without moving it", () => {
    // The shell list arrives empty, then in batches, and empties again on a
    // reconnect. Anything already finished when a thread first appears is
    // recorded, never floated — otherwise every launch reshuffles the inbox.
    const decision = pass([
      { threadKey: "a", completedAt: "2026-08-05T10:00:00.000Z", isUnread: true },
      { threadKey: "b", completedAt: null, isUnread: false },
    ]);

    expect(decision.keysToFloat).toEqual([]);
    // Both are recorded — presence is what marks a thread as already seen, so
    // an unfinished one must be remembered too or its first completion is
    // mistaken for a launch.
    expect([...decision.nextCompletionByKey]).toEqual([
      ["a", "2026-08-05T10:00:00.000Z"],
      ["b", null],
    ]);
  });

  it("floats a known thread when its completion stamp changes", () => {
    const seeded = pass([
      { threadKey: "a", completedAt: null, isUnread: false },
      { threadKey: "b", completedAt: "2026-08-05T10:00:00.000Z", isUnread: true },
    ]);

    const decision = pass(
      [
        { threadKey: "a", completedAt: "2026-08-05T10:05:00.000Z", isUnread: true },
        { threadKey: "b", completedAt: "2026-08-05T10:00:00.000Z", isUnread: true },
      ],
      seeded.nextCompletionByKey,
    );

    // Only `a` finished since the last pass; `b` already had its turn.
    expect(decision.keysToFloat).toEqual(["a"]);
  });

  it("does not float a whole batch that arrives after an empty first pass", () => {
    // The regression that mattered: mount runs before any shell has loaded.
    const mountPass = pass([]);
    const firstRealPass = pass(
      [
        { threadKey: "a", completedAt: "2026-08-01T10:00:00.000Z", isUnread: true },
        { threadKey: "b", completedAt: "2026-08-02T10:00:00.000Z", isUnread: true },
      ],
      mountPass.nextCompletionByKey,
    );

    expect(firstRealPass.keysToFloat).toEqual([]);
  });

  it("leaves a finished thread alone once it has been floated", () => {
    const seeded = pass([{ threadKey: "a", completedAt: null, isUnread: false }]);
    const candidates = [
      { threadKey: "a", completedAt: "2026-08-05T10:05:00.000Z", isUnread: true },
    ];

    const first = pass(candidates, seeded.nextCompletionByKey);
    const second = pass(candidates, first.nextCompletionByKey);

    expect(first.keysToFloat).toEqual(["a"]);
    expect(second.keysToFloat).toEqual([]);
  });

  it("does not float a thread reported as read", () => {
    // The caller reports the open thread as read, so the row being looked at
    // never jumps out from under the reader.
    expect(
      pass(
        [{ threadKey: "a", completedAt: "2026-08-05T10:05:00.000Z", isUnread: false }],
        new Map([["a", "2026-08-05T09:00:00.000Z"]]),
      ).keysToFloat,
    ).toEqual([]);
  });

  it("orders a batch oldest first so the newest completion ends up on top", () => {
    ...
    expect(
      pass(
        [
          { threadKey: "newest", completedAt: "2026-08-05T10:05:00.000Z", isUnread: true },
          { threadKey: "oldest", completedAt: "2026-08-05T10:01:00.000Z", isUnread: true },
          { threadKey: "middle", completedAt: "2026-08-05T10:03:00.000Z", isUnread: true },
        ],
        seeded,
      ).keysToFloat,
  ...
  it("records an unparseable stamp as unfinished rather than dropping the thread", () => {
    const decision = pass(
      [
        { threadKey: "running", completedAt: null, isUnread: true },
        { threadKey: "corrupt", completedAt: "not-a-date", isUnread: true },
      ],
      new Map([
        ["running", null],
        ["corrupt", null],
