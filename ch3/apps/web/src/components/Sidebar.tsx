          ...
          {renamingThreadKey === threadKey ? (
            ...
            <Tooltip>
                ...
                render={
                  <span
                    className={cn(
                      ...
                      // A rename shells out to a model, so it is slow enough to
                      // need saying so — otherwise the click looks ignored.
                      isRegeneratingTitle && "opacity-60",
                    )}
              ...
              <TooltipPopup side="top" className="max-w-80 whitespace-normal leading-tight">
                {isRegeneratingTitle ? "Renaming…" : thread.title}
              </TooltipPopup>
            </Tooltip>
          )}
          {isRegeneratingTitle && renamingThreadKey !== threadKey ? (
            <LoaderIcon
    ...
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      const clicked = await api.contextMenu.show(
        [
          ...
          {
            id: "regenerate-title",
            label: thread.titleRegeneration != null ? "Renaming…" : "Smart rename",
            disabled: thread.titleRegeneration != null,
          },
          { id: "mark-unread", label: "Mark unread" },
          { id: "copy-path", label: "Copy Path" },
          { id: "copy-thread-id", label: "Copy Thread ID" },
          { id: "copy-conversation-id", label: "Copy Conversation ID" },
      ...
      if (clicked === "regenerate-title") {
        // Same path the periodic refresh uses, so an in-flight refresh and a
        // manual one cannot both land — the newer request supersedes.
        ...
        // The command only reports that the rename was *scheduled*. Generation
        // happens on the server and its failures are logged, not sent back, so
        // watch the thread until the pending marker clears and report what
        // actually happened. Without this a failed rename is indistinguishable
        ...
        while (Date.now() - startedAt < deadlineMs) {
          ...
          const current = sidebarThreadByKeyRef.current.get(threadKey);
          if (!current) return;
