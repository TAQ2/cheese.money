import { findErrorTraceId } from "@ch3tools/client-runtime/errors";
import {
...
import type { RelayClientEnvironmentRecord } from "@ch3tools/contracts/relay";
...
/**
 * The user's CH3 Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through CH3 Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the CH3 Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[ch3-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
