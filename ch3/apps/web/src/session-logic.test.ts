...
describe("deriveWorkLogEntries", () => {
  ...
  it("captures the delegated agent's model from the Task call payload", () => {
    // Exactly what the activity store persists for a subagent delegation:
    // itemType collab_agent_tool_call, the model inside data.input. This is
    // NOT reachable via toolData, which the derivation only carries for MCP
    // tool calls — reading it from there silently produced no model at all.
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "agent-updated",
        createdAt: "2026-02-23T00:00:01.000Z",
        ...
        payload: {
          ...
          data: {
            toolName: "Agent",
            input: { description: "Recite pi very long", model: "haiku" },
          },
        },
      }),
      makeActivity({
    ...
    const entries = deriveWorkLogEntries(activities);
    const agentEntry = entries.find((entry) => entry.itemType === "collab_agent_tool_call");
    expect(agentEntry).toBeDefined();
    // Survives the merge with a later update that names no model.
    expect(agentEntry?.agentModel).toBe("haiku");
  });
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "ch3 · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "ch3 · preview_status",
          data: { item },
        },
      }),
    ];
