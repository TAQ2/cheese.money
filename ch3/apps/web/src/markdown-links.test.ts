...
describe("rewriteMarkdownFileUriHref", () => {
  ...
  it("normalizes file uri hrefs for windows drive paths", () => {
    expect(
      rewriteMarkdownFileUriHref(
        "file:///D:/Programme/ch3/apps/web/src/components/chat/OpenInPicker.tsx#L69",
      ),
    ).toBe("D:/Programme/ch3/apps/web/src/components/chat/OpenInPicker.tsx#L69");
  });

  it("unwraps angle-bracketed file uri hrefs", () => {
    expect(
      rewriteMarkdownFileUriHref(" <file:///D:/Programme/ch3/apps/web/src/markdown-links.ts> "),
    ).toBe("D:/Programme/ch3/apps/web/src/markdown-links.ts");
  });
});
...
describe("resolveMarkdownFileLinkTarget", () => {
  ...
  it("formats tooltip display paths relative to the cwd when possible", () => {
    expect(
      resolveMarkdownFileLinkMeta(
        "file:///C:/Users/mike/dev-stuff/ch3/apps/web/src/session-logic.ts#L501",
        "C:/Users/mike/dev-stuff/ch3",
      ),
    ).toMatchObject({
      displayPath: "ch3/apps/web/src/session-logic.ts:501",
  ...
  it("formats tooltip display paths relative to the cwd for slash-prefixed windows paths", () => {
    expect(
      resolveMarkdownFileLinkMeta(
        "/C:/Users/mike/dev-stuff/ch3/apps/web/src/components/chat/MessagesTimeline.virtualization.browser.tsx",
        "C:/Users/mike/dev-stuff/ch3",
      ),
    ).toMatchObject({
      displayPath: "ch3/apps/web/src/components/chat/MessagesTimeline.virtualization.browser.tsx",
  ...
  it("normalizes slash-prefixed windows drive paths before resolving", () => {
    expect(
      resolveMarkdownFileLinkTarget(
        "/D:/Programme/ch3/apps/web/src/components/chat/OpenInPicker.tsx#L69",
      ),
    ).toBe("D:/Programme/ch3/apps/web/src/components/chat/OpenInPicker.tsx:69");
  });

  it("resolves angle-bracketed windows drive paths", () => {
    expect(
      resolveMarkdownFileLinkTarget(
        "</D:/Programme/ch3/apps/web/src/components/ChatMarkdown.tsx:1>",
      ),
    ).toBe("D:/Programme/ch3/apps/web/src/components/ChatMarkdown.tsx:1");
  });
...
describe("buildVerifiedFileLinkMeta", () => {
  const cwd = "/Users/julius/project";

  it("builds meta for a path the file index confirmed, spaces and all", () => {
    // The shape checks reject whitespace outright, so this path can only ever
    // be linked through a confirmed index entry.
    expect(buildVerifiedFileLinkMeta("change requests/2026-08-04-notes.md", cwd)).toEqual({
      filePath: "/Users/julius/project/change requests/2026-08-04-notes.md",
      targetPath: "/Users/julius/project/change requests/2026-08-04-notes.md",
      displayPath: "project/change requests/2026-08-04-notes.md",
      workspaceRelativePath: "change requests/2026-08-04-notes.md",
      basename: "2026-08-04-notes.md",
    });
  });

  it("carries a stated line and column into the target", () => {
    const meta = buildVerifiedFileLinkMeta("src/main.rs", cwd, { line: 12, column: 5 });

    expect(meta.targetPath).toBe("/Users/julius/project/src/main.rs:12:5");
    expect(meta.filePath).toBe("/Users/julius/project/src/main.rs");
    expect(meta.line).toBe(12);
    expect(meta.column).toBe(5);
  });

  it("omits a column that was never stated", () => {
    const meta = buildVerifiedFileLinkMeta("schema.sql", cwd, { line: 42 });
