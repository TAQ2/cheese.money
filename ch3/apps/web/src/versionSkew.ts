import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@ch3tools/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "ch3:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same CH3 version.",
  };
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the CH3 desktop app on its machine — update the desktop app there to sync them.`;
    default:
