...
describe("shouldInterceptComposerBuiltin", () => {
  it("lets a provider-advertised command through", () => {
    // The Claude runtime advertises /clear and really executes it.
    expect(
      shouldInterceptComposerBuiltin({
        builtin: "clear",
        providerCommandNames: ["clear", "compact", "usage"],
      }),
    ).toBe(false);
  });
  ...
  it("still intercepts a command the runtime does not advertise", () => {
    expect(
      shouldInterceptComposerBuiltin({
        builtin: "login",
        providerCommandNames: ["clear", "compact"],
      }),
    ).toBe(true);
  });

  it("keeps CH3's native surfaces even when advertised", () => {
    for (const builtin of ["mcp", "rewind", "resume"] as const) {
      expect(
        shouldInterceptComposerBuiltin({
          builtin,
          providerCommandNames: ["mcp", "rewind", "resume"],
        }),
      ).toBe(true);
    }
  });
...
describe("parseComposerInteractiveBuiltin", () => {
  it("parses /mcp", () => {
    expect(parseComposerInteractiveBuiltin(" /mcp ")).toBe("mcp");
    expect(parseComposerInteractiveBuiltin("/MCP")).toBe("mcp");
  });

  it("parses guarded built-ins", () => {
    expect(parseComposerInteractiveBuiltin("/rewind")).toBe("rewind");
    expect(parseComposerInteractiveBuiltin("/resume")).toBe("resume");
    expect(parseComposerInteractiveBuiltin("/login")).toBe("login");
    expect(parseComposerInteractiveBuiltin("/terminal-setup")).toBe("terminal-setup");
  });

  it("intercepts guarded built-ins even with arguments", () => {
    // These never work as prompt text with or without arguments.
    expect(parseComposerInteractiveBuiltin("/resume abc123")).toBe("resume");
    expect(parseComposerInteractiveBuiltin("/mcp list servers")).toBe("mcp");
  });

  it("ignores plain prose and unknown or text-capable commands", () => {
    expect(parseComposerInteractiveBuiltin("run /mcp for me")).toBeNull();
    expect(parseComposerInteractiveBuiltin("/mcpx")).toBeNull();
    // Text-capable commands must pass through to the runtime untouched.
    expect(parseComposerInteractiveBuiltin("/compact")).toBeNull();
    expect(parseComposerInteractiveBuiltin("/usage")).toBeNull();
    expect(parseComposerInteractiveBuiltin("/init")).toBeNull();
  });

  it("has notice copy for every guarded built-in except the native /mcp view", () => {
