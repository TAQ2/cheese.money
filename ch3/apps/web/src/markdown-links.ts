...
/**
 * Meta for a path the project's own file index has already confirmed, which
 * skips every "is this really a path?" check above.
 *
 * Those checks exist to keep prose and identifiers from linking, and a path
 * that came back from the index is neither. It is also the only way a
 * legitimate path with spaces in it — a real directory name — can be linked
 * at all, since the shape checks reject whitespace outright.
 */
export function buildVerifiedFileLinkMeta(
  ...
  const withPosition =
    line === undefined
      ? projectRelativePath
      : `${projectRelativePath}:${line}${column === undefined ? "" : `:${column}`}`;
  return buildFileLinkMetaFromTarget(resolvePathLinkTarget(withPosition, cwd), cwd);
}
