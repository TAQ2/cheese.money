...
describe("requestLatencyState", () => {
  ...
  it("exempts long-running methods using the tag shape production sends", () => {
    // Production tags are "method · environmentId" — an exemption that only
    // matches the bare method name silently exempts nothing.
    trackRpcRequestSent("s1", `${WS_METHODS.speechSynthesize} · env_1`);
    trackRpcRequestSent("s2", `${WS_METHODS.claudeAwaitAccountLogin} · env_1`);
    trackRpcRequestSent("s3", `${WS_METHODS.claudeStartAccountLogin} · env_1`);
    vi.advanceTimersByTime(SLOW_RPC_ACK_THRESHOLD_MS * 30);
    expect(getSlowRpcAckRequests()).toEqual([]);
  });

  it("still tracks ordinary methods carrying the production tag shape", () => {
    trackRpcRequestSent("t1", "server.getConfig · env_1");
