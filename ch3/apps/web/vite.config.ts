...
import { DEV_PROXIED_PATH_PREFIXES } from "@ch3tools/shared/devProxy";
...
const isSingleOriginDev = process.env.CH3CODE_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_CH3CODE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.CH3CODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: CH3CODE_BUNDLED_DEV=1 pnpm dev:web
const bundledDevEnv = process.env.CH3CODE_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // CH3CODE_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.CH3CODE_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.CH3CODE_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_CH3CODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
