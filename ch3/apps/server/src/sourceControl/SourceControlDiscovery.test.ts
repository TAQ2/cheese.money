...
import { VcsProcessSpawnError } from "@ch3tools/contracts";
    ...
    Layer.provide(
      Layer.mergeAll(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "ch3-source-control-registry-test-",
...
it.effect("reports implemented tools separately from locally available executables", () => {
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "ch3-source-control-discovery-",
      }),
    ),
...
`),
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "ch3-source-control-auth-discovery-",
      }),
    ),
    Layer.provide(Layer.mock(VcsProcess.VcsProcess)(processMock)),
    Layer.provide(
