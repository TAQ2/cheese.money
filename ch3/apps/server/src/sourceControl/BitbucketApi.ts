...
import { sanitizeBranchFragment } from "@ch3tools/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@ch3tools/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("CH3CODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("CH3CODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("CH3CODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("CH3CODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("ch3/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `ch3/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set CH3CODE_BITBUCKET_EMAIL and CH3CODE_BITBUCKET_API_TOKEN, or CH3CODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
