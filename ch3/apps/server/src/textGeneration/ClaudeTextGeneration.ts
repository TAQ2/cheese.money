...
import { type ClaudeSettings, type ModelSelection } from "@ch3tools/contracts";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@ch3tools/shared/git";
...
export const makeClaudeTextGeneration = Effect.fn("makeClaudeTextGeneration")(function* (
  ...
  // Text generation never uses tools, but without explicit isolation every
  // spawn boots the user's configured MCP servers and ships their tool
  // schemas as prompt tokens. Same isolation as the capability probe:
  // connected claude.ai MCP servers are discovered outside filesystem config,
