...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(CH3ProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "ch3.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("CH3ProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid ch3.json", () =>
      Effect.gen(function* () {
        const loader = yield* CH3ProjectFileLoader.CH3ProjectFileLoader;
    ...
    it.effect("returns none when ch3.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* CH3ProjectFileLoader.CH3ProjectFileLoader;
        const cwd = yield* makeTempDir;
