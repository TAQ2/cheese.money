 ...
 */
import { ClaudeSettings, ProviderDriverKind, type ServerProvider } from "@ch3tools/contracts";
...
export const ClaudeDriver: ProviderDriver<ClaudeSettings, ClaudeDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      // Runs on every instance build, so a Claude account switch — which
      // rebuilds this instance — lands with conversations already shared and
      // resumable under the newly selected account.
      yield* ensureSharedClaudeTranscriptStore(effectiveConfig);
      // Same moment, same reason: a profile must also carry the MCP servers
      // the user added at user scope, which otherwise live only in the
      // default home's config and are invisible to every thread run here.
      yield* ensureSharedClaudeMcpServers(effectiveConfig);
