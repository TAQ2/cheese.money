...
import { normalizeCustomModelSlug } from "@ch3tools/shared/model";
...
export function buildServerProvider(input: {
  ...
  return {
    ...
    skills: [...(input.skills ?? [])],
    ...(input.outputStyles && input.outputStyles.length > 0
      ? { outputStyles: [...input.outputStyles] }
      : {}),
    ...(input.activeOutputStyle ? { activeOutputStyle: input.activeOutputStyle } : {}),
