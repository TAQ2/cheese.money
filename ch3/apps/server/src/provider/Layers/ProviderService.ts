...
import { causeErrorTag } from "@ch3tools/shared/observability";
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in CH3 settings.`,
          );
        }
  ...
  const getMcpStatus: ProviderServiceMethod<"getMcpStatus"> = Effect.fn("getMcpStatus")(
    function* (input) {
      const routed = yield* resolveRoutableSession({
        threadId: input.threadId,
      ...
      // Optional adapter capability: adapters without an MCP status control
      // (Codex, Cursor, ...) report unsupported instead of guessing.
      const readMcpStatus = routed.adapter.mcpServerStatus;
      if (!readMcpStatus) {
        return yield* Effect.fail(
          new ProviderUnsupportedError({ provider: routed.adapter.provider }),
        );
      }
      return yield* readMcpStatus(routed.threadId);
    },
  );

  const mcpServerAction: ProviderServiceMethod<"mcpServerAction"> = Effect.fn("mcpServerAction")(
    function* (input) {
      const routed = yield* resolveRoutableSession({
        threadId: input.threadId,
        operation: "ProviderService.mcpServerAction",
        allowRecovery: false,
      });
      yield* Effect.annotateCurrentSpan({
        "provider.operation": "mcp-action",
        "provider.kind": routed.adapter.provider,
        "provider.thread_id": input.threadId,
        "provider.mcp_server": input.serverName,
        "provider.mcp_action": input.action,
      });
      if (!routed.isActive) {
        return yield* Effect.fail(new ProviderSessionNotFoundError({ threadId: input.threadId }));
      }
      const runMcpAction = routed.adapter.mcpServerAction;
      if (!runMcpAction) {
        return yield* Effect.fail(
          new ProviderUnsupportedError({ provider: routed.adapter.provider }),
        );
      }
      return yield* runMcpAction(routed.threadId, input.serverName, input.action);
    },
  );

  const listRewindTargets: ProviderServiceMethod<"listRewindTargets"> = Effect.fn(
    "listRewindTargets",
  )(function* (input) {
    const routed = yield* resolveRoutableSession({
      threadId: input.threadId,
      operation: "ProviderService.listRewindTargets",
      allowRecovery: false,
    });
    yield* Effect.annotateCurrentSpan({
      "provider.operation": "rewind-targets",
      "provider.kind": routed.adapter.provider,
      "provider.thread_id": input.threadId,
    });
    if (!routed.isActive) {
    ...
    const readTargets = routed.adapter.listRewindTargets;
    if (!readTargets) {
      return yield* Effect.fail(
        new ProviderUnsupportedError({ provider: routed.adapter.provider }),
      );
    }
    return yield* readTargets(routed.threadId);
  });

  const rewindFiles: ProviderServiceMethod<"rewindFiles"> = Effect.fn("rewindFiles")(
    function* (input) {
      const routed = yield* resolveRoutableSession({
        threadId: input.threadId,
      ...
      yield* Effect.annotateCurrentSpan({
        ...
        "provider.kind": routed.adapter.provider,
        "provider.thread_id": input.threadId,
        "provider.rewind_dry_run": input.dryRun,
      });
      if (!routed.isActive) {
        return yield* Effect.fail(new ProviderSessionNotFoundError({ threadId: input.threadId }));
      }
      const runRewind = routed.adapter.rewindFiles;
      if (!runRewind) {
        return yield* Effect.fail(
          new ProviderUnsupportedError({ provider: routed.adapter.provider }),
        );
      }
      return yield* runRewind(routed.threadId, input.userMessageId, { dryRun: input.dryRun });
    },
  );
