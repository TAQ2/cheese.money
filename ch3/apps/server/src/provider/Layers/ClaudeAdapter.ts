...
import {
  ...
  type McpServerStatus as ClaudeMcpServerStatus,
  type SDKMessage,
...
/** A user message the live session can rewind tracked files to. */
interface ClaudeRewindTarget {
  readonly id: string;
...
interface ClaudeSessionContext {
   ...
   * only spends a control request when the user actually changed it, and so a
   * cleared style can be pushed back down as `null`.
   */
  currentOutputStyle: string | undefined;
   ...
   * picker's source. Lost on session restart (the CLI re-emits nothing on
   * resume), so targets only cover the current process's lifetime.
   */
  readonly rewindTargets: ClaudeRewindTarget[];
...
interface ClaudeQueryRuntime extends AsyncIterable<SDKMessage> {
  ...
  /**
   * Merges settings into the CLI's flag-settings layer mid-session — the same
   * layer the inline `settings` query option seeds at startup. Optional
   * because a CLI older than the control request does not expose it; a `null`
   * value clears the key and falls back to the user's own settings files.
   */
  readonly applyFlagSettings?: (settings: Record<string, string | boolean | null>) => Promise<void>;
  readonly getContextUsage?: () => Promise<SDKControlGetContextUsageResponse>;
  // A local control request over the subprocess's stdio — never an
  // Anthropic API call, so it consumes no tokens.
  readonly mcpServerStatus?: () => Promise<ClaudeMcpServerStatus[]>;
  ...
  /**
   * Present on the runtime Query class but absent from the SDK's public
   * type: starts the MCP server's OAuth flow over the same control channel.
   * Feature-detected at call time; response shape is version-dependent, so
   * callers must treat it as unknown.
   */
  readonly mcpAuthenticate?: (serverName: string, redirectUri?: string) => Promise<unknown>;
  /** Same undocumented runtime surface as mcpAuthenticate. */
  readonly mcpClearAuth?: (serverName: string) => Promise<unknown>;
  /** Requires the session to run with enableFileCheckpointing. */
...
const MAX_REWIND_TARGETS = 100;

/**
 * Best-effort URL extraction from the undocumented mcp_authenticate control
 * response. Different CLI versions shape it differently; anything http(s)
 * found in the common fields is handed to the client to open.
 */
function extractMcpAuthUrl(response: unknown): string | undefined {
  ...
  for (const key of ["authUrl", "url", "authorizationUrl", "oauthUrl"]) {
...
/**
 * First line of a user message's text content, truncated for the rewind
 * picker. Null when the message carries no text (e.g. tool_result-only).
 */
function claudeUserMessagePreview(content: unknown): string | null {
  const text =
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleUserMessage = Effect.fn("handleUserMessage")(function* (
    ...
    // A prompt echo (runtime uuid + non-tool-result content) marks a file
    // checkpoint the session can rewind to. Tool-result user messages and
    // synthetic messages are not rewind boundaries.
    const runtimeUuid =
      "uuid" in message && typeof message.uuid === "string" ? message.uuid : undefined;
    if (runtimeUuid && message.parent_tool_use_id === null && message.isSynthetic !== true) {
      const preview = claudeUserMessagePreview(message.message.content);
      if (preview !== null && !context.rewindTargets.some((t) => t.id === runtimeUuid)) {
        context.rewindTargets.push({
          id: runtimeUuid,
          createdAt: yield* nowIso,
          preview,
        });
        if (context.rewindTargets.length > MAX_REWIND_TARGETS) {
          context.rewindTargets.shift();
        }
      }
    }
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      case "notification":
        ...
        return;
      // Inner protocol/UX details with no CH3 surface today — consumed
  ...
  const handleSdkMessage = Effect.fn("handleSdkMessage")(function* (
    ...
    switch (message.type) {
      ...
      case "rate_limit_event":
        ...
        return;
      // Composer prompt suggestions have no CH3 surface; consumed deliberately.
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const handleAskUserQuestion = Effect.fn("handleAskUserQuestion")(function* (
        ...
        // expected lookup key. See https://github.com/pingdotgg/ch3/issues/2388
      ...
      const queryOptions: ClaudeQueryOptions = {
        ...
        // File snapshots per user message, kept CLI-side; costs nothing per
        // turn and makes the rewind control available.
  ...
  const sendTurn: ClaudeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    // The response style is a CLI setting, not an API parameter, so it moves
    // through the flag-settings layer the same way the inline `settings`
    // option seeded it at session start. Only pushed when it actually
    // changed; `null` clears it so the user's own settings files take over
    // again. A CLI without the control request keeps whatever style it
    // started with — the picker still records the choice for the next
    // session, so nothing is silently lost.
    if (modelSelection) {
      const nextOutputStyle = getOutputStyleSelection(modelSelection.options);
      if (nextOutputStyle !== context.currentOutputStyle) {
        const applyFlagSettings = context.query.applyFlagSettings;
        if (applyFlagSettings) {
          yield* Effect.tryPromise({
            try: () =>
              applyFlagSettings.call(context.query, { outputStyle: nextOutputStyle ?? null }),
            catch: (cause) => toRequestError(input.threadId, "turn/applyFlagSettings", cause),
          });
          context.currentOutputStyle = nextOutputStyle;
        }
      }
    }
  ...
  const mcpServerStatus: NonNullable<ClaudeAdapterShape["mcpServerStatus"]> = Effect.fn(
    "mcpServerStatus",
  )(function* (threadId) {
    const context = yield* requireSession(threadId);
    // Local control request over the subprocess's stdio (same channel as
    // interrupt/setModel) — never an Anthropic API call, never billed.
    if (!context.query.mcpServerStatus) {
      return yield* Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "mcpServerStatus",
          detail: "This Claude runtime does not expose MCP server status.",
        }),
      );
    }
    const statuses = yield* Effect.tryPromise({
      // Invoked as a method: the SDK Query is a class instance, and a
      // detached reference loses `this` and crashes inside the SDK.
      try: () => context.query.mcpServerStatus!(),
      catch: (cause) =>
        new ProviderAdapterProcessError({
          provider: PROVIDER,
          threadId,
          detail: "Failed to read MCP server status from the Claude session.",
          cause,
        }),
    });
    return statuses.flatMap((server) => {
      const name = server.name.trim();
      if (!name) return [];
      // config is a transport union; read url/type defensively so an
      // unknown newer shape degrades to omitted fields, not a crash.
      const config = server.config as { url?: unknown; type?: unknown } | undefined;
      const url = typeof config?.url === "string" ? config.url : undefined;
      const transport = typeof config?.type === "string" ? config.type : undefined;
      ...
      return [
        {
          ...
          ...(server.serverInfo
            ? { serverVersion: `${server.serverInfo.name} ${server.serverInfo.version}`.trim() }
            : {}),
          ...(server.scope ? { scope: server.scope } : {}),
          ...(server.tools ? { toolCount: server.tools.length } : {}),
          ...(url ? { url } : {}),
          ...(transport ? { transport } : {}),
          ...(tools && tools.length > 0 ? { tools: tools.slice(0, 200) } : {}),
        },
      ];
    });
  });

  const mcpServerAction: NonNullable<ClaudeAdapterShape["mcpServerAction"]> = Effect.fn(
    "mcpServerAction",
  )(function* (threadId, serverName, action) {
    const context = yield* requireSession(threadId);
    // Same local stdio control channel as mcpServerStatus — never an
    // Anthropic API call, never billed.
    const unsupported = (method: string) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method,
          detail: `This Claude runtime does not support the "${action}" MCP action.`,
        }),
      );
    const run = <A>(operation: () => Promise<A>, detail: string) =>
      Effect.tryPromise({
        try: operation,
        catch: (cause) =>
          new ProviderAdapterProcessError({
            provider: PROVIDER,
            threadId,
            detail: `${detail} ${toMessage(cause, "")}`.trim(),
            cause,
          }),
      });
    let authUrl: string | undefined;
    if (action === "reconnect") {
      if (!context.query.reconnectMcpServer) {
        return yield* unsupported("reconnectMcpServer");
      }
      // Invoked as methods throughout: the SDK Query is a class instance and
      // a detached reference loses `this` (proven live on mcpServerStatus).
      yield* run(
        () => context.query.reconnectMcpServer!(serverName),
        `Failed to reconnect MCP server "${serverName}".`,
      );
    } else if (action === "enable" || action === "disable") {
      if (!context.query.toggleMcpServer) {
        return yield* unsupported("toggleMcpServer");
      }
      yield* run(
        () => context.query.toggleMcpServer!(serverName, action === "enable"),
        `Failed to ${action} MCP server "${serverName}".`,
      );
    } else if (action === "clear-auth") {
      if (!context.query.mcpClearAuth) {
        return yield* unsupported("mcpClearAuth");
      }
      yield* run(
        () => context.query.mcpClearAuth!(serverName),
        `Failed to clear authentication for MCP server "${serverName}".`,
      );
    } else {
      if (!context.query.mcpAuthenticate) {
        return yield* unsupported("mcpAuthenticate");
      }
      const response = yield* run(
        () => context.query.mcpAuthenticate!(serverName),
        `Failed to start authentication for MCP server "${serverName}".`,
      );
      authUrl = extractMcpAuthUrl(response);
    }
    const servers = yield* mcpServerStatus(threadId);
    return { servers, ...(authUrl ? { authUrl } : {}) };
  });

  const listRewindTargets: NonNullable<ClaudeAdapterShape["listRewindTargets"]> = Effect.fn(
    "listRewindTargets",
  )(function* (threadId) {
    const context = yield* requireSession(threadId);
    // Newest first for the picker.
    return context.rewindTargets.toReversed();
  });

  const rewindFiles: NonNullable<ClaudeAdapterShape["rewindFiles"]> = Effect.fn("rewindFiles")(
    function* (threadId, userMessageId, options) {
      const context = yield* requireSession(threadId);
      if (!context.query.rewindFiles) {
        return yield* Effect.fail(
          new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "rewindFiles",
            detail: "This Claude runtime does not expose file rewind.",
          }),
        );
      }
      const result = yield* Effect.tryPromise({
        // Attached method call — see mcpServerStatus.
        try: () => context.query.rewindFiles!(userMessageId, { dryRun: options.dryRun }),
        catch: (cause) =>
          new ProviderAdapterProcessError({
            provider: PROVIDER,
            threadId,
            detail: "Failed to rewind files in the Claude session.",
            cause,
          }),
      });
      return {
