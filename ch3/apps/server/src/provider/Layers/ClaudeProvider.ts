 ...
 * Claude Code setting that pairs with `xhigh`, and `ultrathink` is filtered out
 * because it is a prompt-prefix mode. No per-model downgrade remains: every
 * model in the catalog accepts the full effort range.
 */
...
type ClaudeCapabilitiesProbe = {
  ...
  readonly activeOutputStyle?: string | undefined;
};
 ...
 */
function parseClaudeOutputStyles(styles: unknown): ReadonlyArray<string> {
  ...
  return Effect.gen(function* () {
    ...
    return yield* Effect.tryPromise(async () => {
      ...
      return {
        ...
        activeOutputStyle:
          typeof init.output_style === "string"
            ? nonEmptyProbeString(init.output_style)
            : undefined,
...
> {
  ...
  if (!claudeSettings.enabled) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth: { status: "unknown" },
        message: "Claude is disabled in CH3 settings.",
      },
    });
  }
  ...
  if (!capabilities) {
    return buildServerProvider({
      ...
      skills,
      outputStyles,
      ...(activeOutputStyle ? { activeOutputStyle } : {}),
      probe: {
        installed: true,
        version: parsedVersion,
        status: "warning",
