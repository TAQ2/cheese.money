...
import { GrokSettings } from "@ch3tools/contracts";
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "ch3-grok-version-" });
    ...
    Effect.gen(function* () {
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "ch3-grok-success-" });
          const grokPath = path.join(dir, "grok");
          yield* fs.writeFileString(
            grokPath,
