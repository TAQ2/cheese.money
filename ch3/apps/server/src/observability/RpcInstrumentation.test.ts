...
describe("RpcInstrumentation", () => {
  it.effect("records success metrics for unary RPC handlers", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "ch3_rpc_requests_total", {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "ch3_rpc_request_duration", {
          method: "rpc.instrumentation.success",
        }),
        true,
      );
  ...
  it.effect("records direct stream durations from nanosecond clock readings", () =>
    Effect.gen(function* () {
      ...
      const snapshots = yield* Metric.snapshot;
      const snapshot = findHistogramSnapshot(snapshots, "ch3_rpc_request_duration", {
