...
import { HostProcessArguments } from "@ch3tools/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `ch3` command.
 */
...
/**
 * The `ch3` package spec to suggest. The literal spec the user typed (e.g.
 * `ch3@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "ch3@nightly" : "ch3";
}

/**
 * Render a `ch3 <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx ch3 connect` suggests
 * `npx ch3 serve`, a global install suggests `ch3 serve`, and a nightly build
