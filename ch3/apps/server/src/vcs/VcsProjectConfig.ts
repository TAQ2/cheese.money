...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@ch3tools/contracts";
import { fromLenientJson } from "@ch3tools/shared/schemaJson";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".ch3", "vcs.json");
