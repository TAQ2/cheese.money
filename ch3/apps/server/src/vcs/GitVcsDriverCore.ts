...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@ch3tools/shared/git";
import { compactTraceAttributes } from "@ch3tools/shared/observability";
import { decodeJsonResult } from "@ch3tools/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `ch3-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const pullCurrentBranch: GitVcsDriver.GitVcsDriver["Service"]["pullCurrentBranch"] = Effect.fn(
    ...
    if (options?.discardConflictingLocalChanges) {
      // The user explicitly chose to drop local edits that block the
      // fast-forward. Scoped restore, never `reset --hard`: only files that
      // are BOTH locally modified AND changed by the incoming commits are
      // restored to HEAD (the pull then overwrites them with the remote
      // version). Unrelated local work and untracked files are untouched.
      const incoming = yield* runGitStdout("GitVcsDriver.pullCurrentBranch.incomingFiles", cwd, [
        "diff",
        "--name-only",
        "HEAD",
        "@{upstream}",
      ]).pipe(Effect.map((stdout) => stdout.split("\n").filter((line) => line.length > 0)));
      const dirty = yield* runGitStdout("GitVcsDriver.pullCurrentBranch.dirtyFiles", cwd, [
        "diff",
        "--name-only",
        "HEAD",
      ]).pipe(Effect.map((stdout) => stdout.split("\n").filter((line) => line.length > 0)));
      const incomingSet = new Set(incoming);
      const overlap = dirty.filter((file) => incomingSet.has(file));
      if (overlap.length > 0) {
        yield* runGit("GitVcsDriver.pullCurrentBranch.discardConflicting", cwd, [
          "checkout",
          "HEAD",
          "--",
          ...overlap,
    ...
    // allowNonZeroExit so the error can carry git's actual stderr — "Your
    // local changes to the following files would be overwritten" is the
    // difference between a self-explanatory failure and a useless one. The
    // stable-diagnostics variant pins LC_ALL=C: the client matches that
    // sentence to offer the discard-and-retry dialog, and a localized git
    // would otherwise reduce it to a dead-end toast.
    const pullExecution = yield* executeGitWithStableDiagnostics(
    ...
    if (pullExecution.exitCode !== 0) {
      const stderrExcerpt = pullExecution.stderr.trim().slice(0, 500);
      return yield* new GitCommandError({
        ...gitCommandContext({
          operation: "GitVcsDriver.pullCurrentBranch",
          cwd,
        ...
        detail: stderrExcerpt.length > 0 ? stderrExcerpt : "git pull failed",
        ...(pullExecution.exitCode === null ? {} : { exitCode: pullExecution.exitCode }),
        stdoutLength: pullExecution.stdout.length,
        stderrLength: pullExecution.stderr.length,
      });
    }
  ...
  return GitVcsDriver.GitVcsDriver.of({
    ...
    pullCurrentBranch: (cwd, options) =>
      withListRefsInvalidation(cwd, pullCurrentBranch(cwd, options)),
    readRangeContext,
