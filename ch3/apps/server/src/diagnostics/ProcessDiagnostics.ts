...
>()("ch3/diagnostics/ProcessDiagnostics") {}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the CH3 server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some(`Process ${input.pid} is not a signalable CH3 backend descendant.`),
        };
      }
