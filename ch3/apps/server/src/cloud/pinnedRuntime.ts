...
/**
 * A pinned runtime is an exact `ch3@<version>` npm-installed into
 ...
 * switching over — never `npx ch3`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "ch3", "dist", "bin.mjs"),
...
/**
 * Installs `ch3@<version>` into the pinned runtime directory unless a complete
  ...
  function* (input: {
    ...
    return yield* pinnedRuntimeInstallLock.withPermit(
      Effect.gen(function* () {
        ...
        const installStep = "installing the pinned ch3 runtime (this can take a few minutes)";
        yield* runner
          .run({
            command: "npm",
            args: [
