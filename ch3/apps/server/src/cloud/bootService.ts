...
/**
 * Installs CH3 as a per-user boot service. Linux-only for now: systemd
 * user unit + loginctl enable-linger. The service runs a stable or pinned
 * runtime — never an ephemeral `npx ch3` cache whose eviction could break
 * startup.
 */

const BOOT_SERVICE_NAME = "ch3";

export const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
export const BOOT_SERVICE_UNIT_ENV = "CH3_BOOT_SERVICE_UNIT";
...
/**
 * `npx ch3` (and pnpm dlx / bunx) run out of ephemeral package-manager
...
export interface BootServicePlan {
  ...
  /** Absolute path of the pinned ch3 entry point the unit will run. */
  readonly ch3EntryPath: string;
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=CH3CODE_HOME=${quoteSystemdValue(plan.baseDir)}`,
    `Environment=${BOOT_SERVICE_UNIT_ENV}=${BOOT_SERVICE_UNIT_FILE}`,
    `ExecStart=${quoteSystemdValue(plan.nodePath)} ${quoteSystemdValue(plan.ch3EntryPath)} serve`,
    "Restart=always",
    "RestartSec=5",
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the CH3 background service.";
  }
}
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
   ...
   * of bin.mjs) because ch3 ships native deps like node-pty.
   */
  ...
  const status: BootService["Service"]["status"] = Effect.gen(function* () {
    ...
    // references still exists (a pinned runtime under ~/.ch3 can be deleted to
