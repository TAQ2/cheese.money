...
/**
 * Lets a connected client replace this server with another published `ch3`
...
export function isPublishedCliEntry(entryPath: string): boolean {
  return normalizeEntryPath(entryPath).includes("/node_modules/ch3/dist/");
}

/**
 * The update path this process can offer, or null when only a manual
 * relaunch works. "desktop-managed" — the CH3 desktop app spawned this
...
export const resolveServerSelfUpdateCapability = Effect.fn(
  ...
  if (platform === "linux" && homeDir !== "") {
    ...
    // explicit marker written into ch3.service identifies this unit as the
...
>()("ch3/cloud/selfUpdate/ServerSelfUpdate") {}
  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      return yield* failWith(
        "This server is managed by the CH3 desktop app on its machine; update the desktop app to update it.",
      );
    }
    if (capability === null) {
      return yield* failWith(
    ...
    if (!EXACT_VERSION_PATTERN.test(targetVersion)) {
      return yield* failWith(`'${targetVersion}' is not an exact ch3 version.`);
    }
    ...
    return yield* Effect.gen(function* () {
      ...
      const runtimePaths = yield* ensurePinnedRuntimeInstalled({
        ...
        Effect.mapError((error) => failWith("Could not install the requested ch3 version.", error)),
      );
      ...
      const preflight = yield* runner
        ...
        .pipe(
          Effect.mapError((cause) =>
            failWith(`Could not verify the installed ch3@${targetVersion}.`, cause),
          ),
        );
      ...
      if (preflight.code !== 0 || reportedVersion !== targetVersion) {
        ...
        yield* removePinnedRuntimeInstallation({
          ...
          Effect.mapError((error) =>
            failWith(`Could not remove the failed ch3@${targetVersion} installation.`, error),
          ),
        );
        return yield* failWith(
          preflight.code !== 0
            ? `The installed ch3@${targetVersion} failed its version check (exit code ${String(preflight.code)}).`
            : `The installed runtime did not report the requested ch3@${targetVersion} version.`,
        );
      }
      ...
      if (activeMethod === "boot-service") {
        ...
        const unit = renderBootServiceUnit({
          ...
          ch3EntryPath: runtimePaths.entryPath,
          baseDir: serverConfig.baseDir,
        ...
        yield* host
          ...
          .pipe(
            Effect.mapError((cause) =>
              failWith("Could not start the replacement ch3 process.", cause),
            ),
          );
        ...
        yield* scheduleRestart(
          Effect.try({
            ...
            catch: (cause) => failWith("Could not exit the replaced ch3 process.", cause),
          }).pipe(
            Effect.catch((error) =>
