...
import { RelayClientTracer } from "@ch3tools/shared/relayTracing";
import * as EnvironmentAuth from "../auth/EnvironmentAuth.ts";
import * as ServerSecretStore from "../auth/ServerSecretStore.ts";
import * as ServerEnvironment from "../environment/ServerEnvironment.ts";
import { CLOUD_CLI_DESIRED_LINK_SECRET } from "./CliState.ts";
import * as CliTokenManager from "./CliTokenManager.ts";
import type { RelayLinkProofRequest } from "@ch3tools/contracts/relay";
  ...
  it.effect("does not accept an unauthenticated request trace parent", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("https://environment.example.test/api/ch3-cloud/mint-credential", {
          headers: {
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not ch3_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("ch3_relay"))).toBe(false);
  });
