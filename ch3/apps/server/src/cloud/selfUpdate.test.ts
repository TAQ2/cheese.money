        ...
        Effect.sync(() => {
          ...
          return {
            stdout:
              ...
              (versionFromPath === undefined ? "" : `ch3 v${versionFromPath}\n`),
...
it("recognizes published npm artifacts as swappable entry points", () => {
  assert.isTrue(SelfUpdate.isPublishedCliEntry("/usr/local/lib/node_modules/ch3/dist/bin.mjs"));
  assert.isTrue(
    SelfUpdate.isPublishedCliEntry("/home/theo/.npm/_npx/abc123/node_modules/ch3/dist/bin.mjs"),
  );
  assert.isTrue(
    SelfUpdate.isPublishedCliEntry(
      "C:\\Users\\theo\\AppData\\Roaming\\npm\\node_modules\\ch3\\dist\\bin.mjs",
    ),
  );
  // Dev checkouts and the desktop bundle run apps/server/dist directly.
  assert.isFalse(SelfUpdate.isPublishedCliEntry("/home/theo/dev/ch3/apps/server/dist/bin.mjs"));
...
it.layer(NodeServices.layer)("resolveServerSelfUpdateCapability", (it) => {
  const makeHome = Effect.fn("test.makeHome")(function* () {
    ...
    const home = yield* fs.makeTempDirectoryScoped({ prefix: "ch3-self-update-test-" });
    return { fs, path, home };
  });
  ...
  const writeUnitReferencing = Effect.fn("test.writeUnitReferencing")(function* (
    ...
    yield* fs.writeFileString(
      ...
      renderBootServiceUnit({
        nodePath: NODE_PATH,
        ch3EntryPath: entryPath,
        baseDir: path.join(home, ".ch3"),
        logPath: path.join(home, ".ch3", "userdata", "logs", "boot-service.log"),
        unitPath: path.join(unitDir, "ch3.service"),
      }),
    );
  });
  ...
  it.effect("reports boot-service for the systemd-spawned unit process", () =>
    Effect.gen(function* () {
      ...
      const entryPath = path.join(home, ".ch3/runtime/versions/0.0.28/node_modules/ch3/dist/bin.mjs");
    ...
    Effect.gen(function* () {
      ...
      const method = yield* SelfUpdate.resolveServerSelfUpdateCapability({
        ...
        provideHostRefs({
          platform: "darwin",
          env: { HOME: home },
          entryPath: `${home}/.npm/_npx/abc123/node_modules/ch3/dist/bin.mjs`,
        }),
      );
      assert.equal(method, "respawn");
    }),
    ...
    Effect.gen(function* () {
      ...
      const devMethod = yield* SelfUpdate.resolveServerSelfUpdateCapability({
        ...
        provideHostRefs({
          platform: "darwin",
          env: { HOME: home },
          entryPath: `${home}/dev/ch3/apps/server/dist/bin.mjs`,
        }),
      );
...
it.layer(NodeServices.layer)("ServerSelfUpdate.update", (it) => {
  ...
  const makeContext = Effect.fn("test.makeContext")(function* (options?: {
    ...
    const path = yield* Path.Path;
    const home = yield* fs.makeTempDirectoryScoped({ prefix: "ch3-self-update-test-" });
    const baseDir = path.join(home, ".ch3");
    const entryPath =
      options?.entryPath ??
      path.join(home, ".ch3/runtime/versions/0.0.28/node_modules/ch3/dist/bin.mjs");
  ...
  it.effect("rejects dist-tags and other non-exact versions", () =>
    Effect.gen(function* () {
      ...
      assert.include(error.reason, "not an exact ch3 version");
  ...
  it.effect("fails without touching anything when no update method applies", () =>
    Effect.gen(function* () {
      const context = yield* makeContext({
        entryPath: "/home/theo/dev/ch3/apps/server/dist/bin.mjs",
      });
      const error = yield* context.service.update({ targetVersion: "0.0.29" }).pipe(Effect.flip);
  ...
  it.effect("surfaces a failed npm install and never schedules a restart", () =>
    Effect.gen(function* () {
      ...
      const error = yield* context.service.update({ targetVersion: "0.0.29" }).pipe(Effect.flip);
      assert.equal(error.reason, "Could not install the requested ch3 version.");
  ...
  it.effect("reinstalls the same version after a failed preflight", () =>
    Effect.gen(function* () {
      ...
      const entryPath = context.path.join(versionDir, "node_modules", "ch3", "dist", "bin.mjs");
    ...
    Effect.gen(function* () {
      const context = yield* makeContext({
        stdoutFor: (command, args) =>
          command === NODE_PATH && args[1] === "--version" ? "ch3 v0.0.28\n" : undefined,
      });
      const versionDir = context.path.join(context.baseDir, "runtime", "versions", "0.0.29");
  ...
  it.effect("installs, preflights, and respawns a foreground server", () =>
    Effect.gen(function* () {
      ...
      const pinnedEntry = context.path.join(
        context.baseDir,
        "runtime/versions/0.0.29/node_modules/ch3/dist/bin.mjs",
      );
      assert.deepEqual(
        context.commands.map((entry) => [entry.command, ...entry.args].join(" ")),
        [
          `npm install --prefix ${context.path.join(context.baseDir, "runtime/versions/0.0.29")} --no-fund --no-audit ch3@0.0.29`,
  ...
  it.effect("rewrites the systemd unit and restarts the boot service", () =>
    Effect.gen(function* () {
      ...
      const unit = yield* context.fs.readFileString(
        context.path.join(context.home, ".config", "systemd", "user", "ch3.service"),
      );
