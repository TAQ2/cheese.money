...
import {
  ...
  HostProcessPlatform,
} from "@ch3tools/shared/hostProcess";
...
const makeTestContext = Effect.fn("test.makeTestContext")(function* () {
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "ch3-boot-service-test-" });
  ...
  return {
    ...
    dirs: {
      home: root,
      baseDir: path.join(root, ".ch3"),
      logsDir: path.join(root, ".ch3", "userdata", "logs"),
      stableEntry,
    },
  };
});
...
it("renders a systemd unit with absolute paths and append-mode logging", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    ch3EntryPath: "/home/theo/.ch3/runtime/versions/0.0.27/node_modules/ch3/dist/bin.mjs",
    baseDir: "/home/theo/.ch3",
    logPath: "/home/theo/.ch3/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/ch3.service",
  });
  ...
  assert.equal(
    ...
    [
      ...
      "Type=simple",
      "WorkingDirectory=%h",
      "Environment=CH3CODE_HOME=/home/theo/.ch3",
      "Environment=CH3_BOOT_SERVICE_UNIT=ch3.service",
      "ExecStart=/usr/local/bin/node /home/theo/.ch3/runtime/versions/0.0.27/node_modules/ch3/dist/bin.mjs serve",
      "Restart=always",
      "RestartSec=5",
      "StandardOutput=append:/home/theo/.ch3/userdata/logs/boot-service.log",
      "StandardError=append:/home/theo/.ch3/userdata/logs/boot-service.log",
      "",
      "[Install]",
...
it("quotes systemd values containing spaces and escapes percent specifiers", () => {
  ...
  assert.equal(BootService.quoteSystemdValue("/home/me/CH3 Data"), '"/home/me/CH3 Data"');
  assert.equal(BootService.quoteSystemdValue("/opt/100%cpu"), "/opt/100%%cpu");

  const unit = BootService.renderBootServiceUnit({
    nodePath: "/home/me/my tools/node",
    ch3EntryPath: "/home/me/CH3 Data/bin.mjs",
    baseDir: "/home/me/CH3 Data",
    logPath: "/home/me/100%logs/boot.log",
    unitPath: "/home/me/.config/systemd/user/ch3.service",
  });
  assert.include(unit, 'ExecStart="/home/me/my tools/node" "/home/me/CH3 Data/bin.mjs" serve');
  assert.include(unit, 'Environment=CH3CODE_HOME="/home/me/CH3 Data"');
...
it("flags package-manager cache entry points as ephemeral", () => {
  assert.isTrue(
    BootService.isEphemeralCacheEntry("/home/theo/.npm/_npx/abc123/node_modules/ch3/dist/bin.mjs"),
  );
  assert.isTrue(
  ...
  assert.isTrue(
    BootService.isEphemeralCacheEntry(
      "/home/theo/.cache/pnpm/dlx/abc/node_modules/ch3/dist/bin.mjs",
    ),
  );
  assert.isTrue(
    BootService.isEphemeralCacheEntry("/home/theo/.bun/install/cache/ch3@0.0.27/dist/bin.mjs"),
  );
  assert.isFalse(BootService.isEphemeralCacheEntry("/usr/local/lib/node_modules/ch3/dist/bin.mjs"));
  assert.isFalse(
    BootService.isEphemeralCacheEntry(
      "/home/theo/dev/pnpm/dlx-tools/ch3/node_modules/ch3/dist/bin.mjs",
    ),
  );
  assert.isFalse(
    BootService.isEphemeralCacheEntry(
      "/home/theo/.ch3/runtime/versions/0.0.27/node_modules/ch3/dist/bin.mjs",
    ),
  );
});
...
it.layer(NodeServices.layer)("BootService", (it) => {
  ...
  it.effect("pins a runtime via npm install when running from the npx cache", () =>
    Effect.gen(function* () {
      ...
      const service = yield* BootService.make({
        ...
        logsDir: dirs.logsDir,
        cliVersion: "0.0.27",
        host: makeHost("/home/theo/.npm/_npx/abc/node_modules/ch3/dist/bin.mjs"),
      }).pipe(Effect.provide(makeRecordingRunnerLayer(commands)), provideHostRefs(dirs.home));
      ...
      assert.equal(
        plan.ch3EntryPath,
        path.join(runtimeDir, "node_modules", "ch3", "dist", "bin.mjs"),
      );
      assert.deepEqual(commands[0], {
        command: "npm",
        args: ["install", "--prefix", runtimeDir, "--no-fund", "--no-audit", "ch3@0.0.27"],
      });
  ...
  it.effect("reinstalls a pinned runtime when its entry point is missing", () =>
    Effect.gen(function* () {
      ...
      const plan = yield* service.install;
      yield* fs.makeDirectory(path.dirname(plan.ch3EntryPath), { recursive: true });
      yield* fs.writeFileString(plan.ch3EntryPath, "#!/usr/bin/env node\n");
      yield* fs.remove(plan.ch3EntryPath);
      commands.length = 0;
  ...
  it.effect("reports an installed-but-stale unit so the lifecycle can offer a repair", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(
        path.join(unitDir, "ch3.service"),
        "[Service]\nExecStart=/old/node /old/ch3 serve\n",
      );
  ...
  it.effect("fails on non-Linux platforms without touching the filesystem", () =>
    Effect.gen(function* () {
      ...
      const service = yield* BootService.make({
        ...
        logsDir: dirs.logsDir,
        cliVersion: "0.0.27",
        host: makeHost("/usr/local/lib/node_modules/ch3/dist/bin.mjs"),
      }).pipe(
        Effect.provide(makeRecordingRunnerLayer(commands)),
      ...
      assert.isFalse(
        yield* fs.exists(path.join(dirs.home, ".config", "systemd", "user", "ch3.service")),
      );
  ...
  it.effect("removes the unit file when an activation step fails", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        commands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user disable --now ch3.service",
        ),
      );
    }),
  );
  ...
  it.effect("restores the previous unit when a repair cannot activate", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        repairCommands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user restart ch3.service",
        ),
      );
    }),
  );
