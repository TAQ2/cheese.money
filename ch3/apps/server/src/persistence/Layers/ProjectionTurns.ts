import { OrchestrationCheckpointFile } from "@ch3tools/contracts";
...
const makeProjectionTurnRepository = Effect.gen(function* () {
  ...
  /**
   * Turns that are mid-flight anywhere. Used to hold back a Claude account
   * hand-over: switching accounts rebuilds the provider instance and would
   * tear down a session in the middle of streaming a reply.
   */
  const countRunningProjectionTurns = SqlSchema.findAll({
    Request: Schema.Struct({}),
    Result: Schema.Struct({ running: Schema.Number }),
    execute: () =>
      sql`
  ...
  const countRunningProjectionTurnsSince = SqlSchema.findAll({
  ...
  const countErrorProjectionTurnsSince = SqlSchema.findAll({
    Request: Schema.Struct({ since: Schema.String }),
    Result: Schema.Struct({ errored: Schema.Number }),
    execute: ({ since }) =>
      sql`
        SELECT COUNT(*) AS "errored"
        FROM projection_turns
        WHERE state = 'error'
          AND COALESCE(completed_at, requested_at) >= ${since}
      `,
  });
  ...
  const countRunning: ProjectionTurnRepositoryShape["countRunning"] = () =>
    countRunningProjectionTurns({}).pipe(
      Effect.mapError(
        toPersistenceSqlOrDecodeError(
          "ProjectionTurnRepository.countRunning:query",
          "ProjectionTurnRepository.countRunning:decodeRows",
        ),
      ),
      Effect.map((rows) => rows[0]?.running ?? 0),
    );

  const countRunningSince: ProjectionTurnRepositoryShape["countRunningSince"] = (since) =>
    countRunningProjectionTurnsSince({ since }).pipe(
      Effect.mapError(
        toPersistenceSqlOrDecodeError(
          "ProjectionTurnRepository.countRunningSince:query",
          "ProjectionTurnRepository.countRunningSince:decodeRows",
        ),
      ),
      Effect.map((rows) => rows[0]?.running ?? 0),
    );

  const countErrorsSince: ProjectionTurnRepositoryShape["countErrorsSince"] = (since) =>
    countErrorProjectionTurnsSince({ since }).pipe(
      Effect.mapError(
        toPersistenceSqlOrDecodeError(
          "ProjectionTurnRepository.countErrorsSince:query",
          "ProjectionTurnRepository.countErrorsSince:decodeRows",
        ),
      ),
      Effect.map((rows) => rows[0]?.errored ?? 0),
    );
