...
import * as RelayClient from "@ch3tools/shared/relayClient";
import { synthesizeSpeech } from "./speech/SpeechEngine.ts";
import {
  DEFAULT_SPEECH_RATE,
  DEFAULT_SPEECH_SPANISH_VOICE,
...
const decodeTranscriptLine = Schema.decodeUnknownExit(
  Schema.fromJsonString(Schema.Struct({ cwd: Schema.optional(Schema.String) })),
);

/**
 * Locates `<claude home>/projects/<slug>/<sessionId>.jsonl` and reads the
 * session's recorded working directory from it. Local file reads only.
 */
const resolveClaudeExternalSession = Effect.fn("resolveClaudeExternalSession")(function* (input: {
  ...
  const home =
    input.homePath.trim().length > 0
      ? input.homePath
      : pathService.join(NodeOS.homedir(), ".claude");
  const projectsDir = pathService.join(home, "projects");
  const slugs = yield* fs.readDirectory(projectsDir).pipe(
    Effect.mapError(
      () =>
        new ClaudeExternalSessionError({
          reason: "not-found",
          detail: `No Claude Code transcript store at ${projectsDir}.`,
        }),
    ),
  );
  const fileName = `${input.sessionId}.jsonl`;
  for (const slug of slugs) {
    const candidate = pathService.join(projectsDir, slug, fileName);
    ...
    const content = yield* fs.readFileString(candidate).pipe(
      Effect.mapError(
        () =>
          new ClaudeExternalSessionError({
            reason: "unreadable",
            detail: `Could not read the transcript at ${candidate}.`,
          }),
      ),
    );
    // The cwd is stamped on most transcript lines; the first stamped line
    // wins. 256KB covers any realistic head segment.
    for (const line of content.slice(0, 262144).split("\n")) {
      if (!line.includes('"cwd"')) {
        continue;
      }
      const parsed = decodeTranscriptLine(line);
      // Partial trailing lines fail decoding — keep scanning.
      if (
        parsed._tag === "Success" &&
        parsed.value.cwd !== undefined &&
        parsed.value.cwd.length > 0
      ) {
        return { cwd: parsed.value.cwd, transcriptPath: candidate };
      }
    }
    return yield* Effect.fail(
      new ClaudeExternalSessionError({
        reason: "unreadable",
        detail: `Transcript found at ${candidate} but no working directory is recorded in it.`,
      }),
    );
  }
  return yield* Effect.fail(
    new ClaudeExternalSessionError({
      reason: "not-found",
      detail: `No transcript named ${fileName} under ${projectsDir}. Check the session id.`,
    }),
  );
});
...
const decodeRewindTranscriptLine = Schema.decodeUnknownExit(
  Schema.fromJsonString(
    Schema.Struct({
      ...
      message: Schema.optional(Schema.Struct({ content: Schema.optional(Schema.Unknown) })),
    }),
  ),
);

/** First line of a transcript user message's text; null for tool-result-only
 * or synthetic content (command wrappers, system reminders). */
function rewindTargetPreview(content: unknown): string | null {
  const text =
  ...
  if (firstLine.length === 0 || firstLine.startsWith("<") || firstLine.startsWith("Caveat:")) {
    return null;
  }
  return firstLine.length > 120 ? `${firstLine.slice(0, 119)}\u2026` : firstLine;
}

/** Full text of a transcript message; null when tool-only or synthetic. */
/** The CLI's own marker for a turn the user stopped, e.g.
    "[Request interrupted by user]". */
const TRANSCRIPT_INTERRUPT_PREFIX = "[Request interrupted";

function transcriptMessageText(content: unknown, role: "user" | "assistant"): string | null {
  const text =
    typeof content === "string"
      ? content
      : Array.isArray(content)
  ...
  // Synthetic user payloads (command wrappers, caveats, reminders) are not
  // conversation the user wrote.
  if (role === "user" && (trimmed.startsWith("<") || trimmed.startsWith("Caveat:"))) {
    return null;
  }
  return trimmed.length > 20000 ? `${trimmed.slice(0, 19999)}\u2026` : trimmed;
}

/**
 * Ordered user/assistant text messages from a session transcript, for
 * seeding an imported thread's timeline. Message ids reuse the transcript
 * uuids, so a re-import updates rows instead of duplicating them.
 */
const listClaudeTranscriptMessages = Effect.fn("listClaudeTranscriptMessages")(function* (input: {
  readonly homePath: string;
  readonly sessionId: string;
}) {
  const fs = yield* FileSystem.FileSystem;
  ...
  const home =
    input.homePath.trim().length > 0
      ...
      : pathService.join(NodeOS.homedir(), ".claude");
  const projectsDir = pathService.join(home, "projects");
  const slugs = yield* fs.readDirectory(projectsDir).pipe(Effect.orElseSucceed(() => []));
  const fileName = `${input.sessionId}.jsonl`;
  for (const slug of slugs) {
    const candidate = pathService.join(projectsDir, slug, fileName);
    const exists = yield* fs.exists(candidate).pipe(Effect.orElseSucceed(() => false));
    ...
    const content = yield* fs.readFileString(candidate).pipe(Effect.orElseSucceed(() => ""));
    ...
    for (const line of content.split("\n")) {
      ...
      const parsed = decodeRewindTranscriptLine(line);
      ...
      const text = transcriptMessageText(entry.message?.content, entry.type);
      if (text === null) {
        continue;
      }
      seen.add(entry.uuid);
    ...
    // Long conversations: keep the newest span the timeline can usefully show.
...
/**
 * All transcript entries for a session in order (uuid + role), used to
 * anchor a conversation rewind at the right point.
 */
const listClaudeTranscriptEntries = Effect.fn("listClaudeTranscriptEntries")(function* (input: {
  readonly homePath: string;
  readonly sessionId: string;
}) {
  const fs = yield* FileSystem.FileSystem;
  ...
  for (const slug of slugs) {
    ...
    const content = yield* fs.readFileString(candidate).pipe(Effect.orElseSucceed(() => ""));
    const entries: Array<{ uuid: string; role: "user" | "assistant"; isPrompt: boolean }> = [];
    const seen = new Set<string>();
    for (const line of content.split("\n")) {
      if (!line.includes('"type":"user"') && !line.includes('"type":"assistant"')) {
        continue;
      ...
      seen.add(entry.uuid);
      const promptText =
        entry.type === "user" ? transcriptMessageText(entry.message?.content, "user") : null;
      entries.push({
        uuid: entry.uuid,
        role: entry.type,
        // A real typed prompt, as opposed to a tool-result or synthetic
        // user payload — only these correspond to CH3's user messages.
        //
        // An interrupt is written as an ordinary user text entry, but CH3
        // records it as turn state and creates no user message for it. Left
        // in, it shifts every position counted from the end: rewinding to
        // the input before an interrupted turn would anchor on the
        // interrupt instead, and the model would keep the turn the user
        // just asked to drop.
        isPrompt: promptText !== null && !promptText.startsWith(TRANSCRIPT_INTERRUPT_PREFIX),
      });
    }
    return entries;
  }
...
/**
 * Rewind targets straight from the CLI's own transcript for a session —
 * the same store its terminal rewind menu reads. Survives session
 * restarts, unlike anything captured from the live stream.
 */
const listClaudeRewindTargets = Effect.fn("listClaudeRewindTargets")(function* (input: {
  readonly homePath: string;
  readonly sessionId: string;
}) {
  const fs = yield* FileSystem.FileSystem;
  ...
  for (const slug of slugs) {
    ...
    const content = yield* fs.readFileString(candidate).pipe(Effect.orElseSucceed(() => ""));
    const targets: Array<{ id: string; createdAt: string; preview: string }> = [];
    const seen = new Set<string>();
    for (const line of content.split("\n")) {
      ...
      const preview = rewindTargetPreview(entry.message?.content);
      if (preview === null) {
        continue;
      }
      seen.add(entry.uuid);
    ...
    // Newest first, capped for the picker.
    return targets.toReversed().slice(0, 100);
  }
  return [];
});

/** Maps provider errors to the client-facing rewind error discriminant. */
function mapRewindError(error: unknown): ProviderRewindError {
  ...
  const reason =
    ...
    tag === "ProviderValidationError"
      ? ("no-session" as const)
      : tag === "ProviderUnsupportedError" || tag === "ProviderAdapterRequestError"
        ? ("unsupported" as const)
        : ("failed" as const);
  return new ProviderRewindError({
    reason,
    detail:
      reason === "no-session"
        ? "No active provider session for this thread."
        : reason === "unsupported"
          ? "This provider does not support file rewind."
          : ((error as { readonly detail?: string }).detail ?? "File rewind failed."),
    cause: error,
  });
}
...
> {
  return (
    ...
    event.type === "thread.messages-rewound" ||
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      // await calls.
      const pendingClaudeLogins = new Map<string, PendingClaudeLogin>();
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.speechSynthesize]: (input) =>
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              const paths = yield* ServerConfig.deriveServerPaths(config.baseDir, config.devUrl);
              const now = yield* DateTime.now.pipe(Effect.map(DateTime.toEpochMillis));
              // The server picks the voice, not the client: the language of the
              // text decides whether the English or the Spanish voice reads it.
              const resolved = resolveSpeechVoice({
                text: input.text,
                voice: input.voice,
                spanishVoice: input.spanishVoice,
                defaultVoice: DEFAULT_SPEECH_VOICE,
                defaultSpanishVoice: DEFAULT_SPEECH_SPANISH_VOICE,
              });
              const spoken = yield* synthesizeSpeech({
                cacheDir: paths.speechCacheDir,
                text: input.text,
              ...
              return {
                ...
                byteLength: spoken.audio.length,
        ...
        [WS_METHODS.threadsGetMcpStatus]: (input) =>
          observeRpcEffect(
            WS_METHODS.threadsGetMcpStatus,
            // A control read from the provider CLI's local session — no model
            // request is made, so this consumes no tokens.
            providerService.getMcpStatus({ threadId: input.threadId }).pipe(
                ...
                const reason =
                  ...
                  tag === "ProviderValidationError"
                    ...
                    : tag === "ProviderUnsupportedError" || tag === "ProviderAdapterRequestError"
                      ? ("unsupported" as const)
                      : ("failed" as const);
                return new ProviderMcpStatusError({
                  reason,
                  detail:
                    reason === "no-session"
                      ? "No active provider session for this thread."
                      : reason === "unsupported"
                        ? "This provider does not report MCP server status."
                        : "Could not read MCP server status.",
                  cause: error,
                });
              }),
            ),
        ...
        [WS_METHODS.threadsMcpServerAction]: (input) =>
          observeRpcEffect(
            WS_METHODS.threadsMcpServerAction,
            // A control write to the provider CLI's local session — no model
            // request is made, so this consumes no tokens.
            providerService
              .mcpServerAction({
                threadId: input.threadId,
                  ...
                  return new ProviderMcpStatusError({
                    ...
                    detail:
                      reason === "no-session"
                        ? "No active provider session for this thread."
                        : reason === "unsupported"
                          ? "This provider does not support MCP server actions."
        ...
        [WS_METHODS.threadsListRewindTargets]: (input) =>
          observeRpcEffect(
            WS_METHODS.threadsListRewindTargets,
            // Read straight from the CLI's transcript store (the same files
            // its terminal rewind menu uses): needs no live session and
            // survives session restarts. Local file reads only.
            Effect.gen(function* () {
              const binding = yield* providerSessionDirectory.getBinding(input.threadId).pipe(
                Effect.mapError(
                  (cause) =>
                    new ProviderRewindError({
                      reason: "failed",
                      detail: "Could not read this thread's provider binding.",
                      cause,
                    }),
                ),
              );
              const sessionId = Option.match(binding, {
                onNone: () => null,
                onSome: (value) => readProviderSessionIdFromCursor(value.resumeCursor ?? null),
              });
              if (sessionId === null || !CLAUDE_SESSION_ID_PATTERN.test(sessionId)) {
                return yield* Effect.fail(
                  new ProviderRewindError({
                    reason: "no-session",
                    detail: "No provider session recorded for this thread yet.",
                  }),
                );
              }
              const homePath = yield* serverSettings.getSettings.pipe(
                Effect.map((settings) => settings.providers.claudeAgent.homePath),
                Effect.orElseSucceed(() => ""),
              );
              const targets = yield* listClaudeRewindTargets({ homePath, sessionId }).pipe(
                Effect.mapError(
                  (cause) =>
                    new ProviderRewindError({
                      reason: "failed",
                      detail: "Could not read the session transcript.",
                      cause,
                    }),
                ),
              );
        ...
        [WS_METHODS.threadsRewindFiles]: (input) =>
          observeRpcEffect(
            WS_METHODS.threadsRewindFiles,
            // File restore over the CLI's local checkpoint store — the model
            // is never involved and no tokens are consumed.
            providerService
              .rewindFiles({
                threadId: input.threadId,
              ...
              .pipe(Effect.mapError((error) => mapRewindError(error))),
            {
              "rpc.aggregate": "thread",
            },
          ),
        [WS_METHODS.claudeResolveExternalSession]: (input) =>
          observeRpcEffect(
            WS_METHODS.claudeResolveExternalSession,
            // Local transcript reads only — nothing reaches a model.
            Effect.gen(function* () {
              ...
              if (!CLAUDE_SESSION_ID_PATTERN.test(sessionId)) {
                return yield* Effect.fail(
                  new ClaudeExternalSessionError({
                    reason: "invalid",
                    detail: "That does not look like a Claude session id (a UUID).",
                  }),
                );
              }
              const homePath = yield* serverSettings.getSettings.pipe(
              ...
              const resolved = yield* resolveClaudeExternalSession({ homePath, sessionId });
              return {
                sessionId,
                cwd: resolved.cwd,
        ...
        [WS_METHODS.threadsAdoptClaudeSession]: (input) =>
          observeRpcEffect(
            WS_METHODS.threadsAdoptClaudeSession,
            // Persists a provider binding whose cursor carries the external
            // session id; the thread's next turn starts the CLI with
            // `resume: <sessionId>` and continues that conversation.
            Effect.gen(function* () {
              const sessionId = input.sessionId.trim().toLowerCase();
              if (!CLAUDE_SESSION_ID_PATTERN.test(sessionId)) {
                return yield* Effect.fail(
              ...
              yield* providerSessionDirectory
                .upsert({
                  ...
                  // Must match the thread's resolved instance: the
                  // session-start fallback ignores cursors from bindings
                  // under any other instance id (proven live via trace —
                  // "generated-session" with an empty resume).
                  providerInstanceId:
                ...
                .pipe(
                  Effect.mapError(
                    (cause) =>
                      new ClaudeExternalSessionError({
                        reason: "failed",
                        detail: "Could not bind the session id to this thread.",
                        cause,
                      }),
                  ),
                );
              // Seed the timeline with the conversation as recorded in the
              // transcript, so the imported thread shows its history instead
              // of starting visually empty.
              const homePath = yield* serverSettings.getSettings.pipe(
                Effect.map((settings) => settings.providers.claudeAgent.homePath),
                Effect.orElseSucceed(() => ""),
              );
              const importedMessages = yield* listClaudeTranscriptMessages({
                homePath,
                sessionId,
              ...
              if (importedMessages.length > 0) {
                const importFailed = (cause: unknown) =>
                  new ClaudeExternalSessionError({
                    reason: "failed",
                    detail:
                      "The session was bound, but its history could not be imported into the timeline.",
                    cause,
                  });
                // serverCommandId's error channel is the dispatch error type;
                // both it and the dispatch itself map into this RPC's error.
                const commandId = yield* serverCommandId("claude-session-import").pipe(
        ...
        [WS_METHODS.threadsRewindToInput]: (input) =>
          observeRpcEffect(
            WS_METHODS.threadsRewindToInput,
            // Two independent steps, both local: truncate CH3's own
            // conversation (no checkpoint, no git, no live session needed),
            // then reposition the provider conversation by pointing its
            // resume cursor at the transcript entry before that input.
            Effect.gen(function* () {
              const rewindFailed = (detail: string) => (cause: unknown) =>
                new ProviderRewindError({ reason: "failed", detail, cause });
              const commandId = yield* serverCommandId("rewind-to-input").pipe(
                Effect.mapError(rewindFailed("Could not start the rewind.")),
              );
              const createdAt = DateTime.formatIso(yield* DateTime.now);
              yield* orchestrationEngine
                .dispatch({
                  type: "thread.messages.rewind",
                  commandId,
                  threadId: input.threadId,
                  fromMessageId: MessageId.make(input.fromMessageId),
                  createdAt,
                })
                .pipe(Effect.mapError(rewindFailed("Could not rewind the conversation.")));

              const binding = yield* providerSessionDirectory
                .getBinding(input.threadId)
                .pipe(Effect.mapError(rewindFailed("Could not read the provider binding.")));
              const bindingValue = Option.getOrUndefined(binding);
              const sessionId =
                bindingValue === undefined
                  ? null
                  : readProviderSessionIdFromCursor(bindingValue.resumeCursor ?? null);
              if (bindingValue === undefined || sessionId === null) {
                return {
                  conversationRepositioned: false,
                  detail:
                    "Messages were rewound. No provider session is recorded for this thread yet, so nothing needed repositioning.",
                };
              }
              const homePath = yield* serverSettings.getSettings.pipe(
                Effect.map((settings) => settings.providers.claudeAgent.homePath),
              ...
              const entries = yield* listClaudeTranscriptEntries({ homePath, sessionId }).pipe(
                Effect.orElseSucceed(() => []),
              );
              // Count typed prompts from the end: identical texts repeat, so
              // position is the only sound way to find the target.
              const promptIndexes = entries.flatMap((entry, index) =>
                entry.isPrompt ? [index] : [],
              );
              const targetIndex = promptIndexes.at(-1 - input.userMessagesAfter);
              const anchorUuid =
                targetIndex === undefined || targetIndex === 0
                  ? null
                  : (entries[targetIndex - 1]?.uuid ?? null);
              if (anchorUuid === null) {
                return {
                  conversationRepositioned: false,
                  detail:
                    "Messages were rewound, but that input was not found in the session transcript, so the model still holds the newer turns.",
                };
              }
              // Stop first: the stop path writes the session's own cursor
              // back to the binding and would clobber ours.
              ...
              yield* providerSessionDirectory
                .upsert({
                  threadId: input.threadId,
                  provider: bindingValue.provider,
                  ...(bindingValue.providerInstanceId !== undefined
                    ? { providerInstanceId: bindingValue.providerInstanceId }
                    : {}),
                  resumeCursor: {
                    threadId: input.threadId,
                    resume: sessionId,
                ...
                .pipe(Effect.mapError(rewindFailed("Could not reposition the provider session.")));
              return {
                conversationRepositioned: true,
                detail:
                  "Rewound. The next message resumes from that point with the earlier history intact.",
              };
            }),
            {
              "rpc.aggregate": "thread",
        ...
        [WS_METHODS.claudeListAccountProfiles]: (input) =>
          observeRpcEffect(
            WS_METHODS.claudeListAccountProfiles,
            // Directory reads plus the CLI's own init probe per profile —
            // local only, no model request, no token cost.
            Effect.gen(function* () {
                ...
                Effect.mapError(
                  (cause) =>
                    new ClaudeAccountError({
                      reason: "failed",
                      detail: "Could not read the Claude account profiles.",
                      cause,
                    }),
                ),
              );
        ...
        [WS_METHODS.claudeStartAccountLogin]: (input) =>
          observeRpcEffect(
              ...
              const settings = yield* serverSettings.getSettings.pipe(
                Effect.mapError(
                  (cause) =>
                    new ClaudeAccountError({
                      reason: "failed",
                      detail: "Could not read CH3 settings.",
                      cause,
                    }),
                ),
              );
              const started = yield* startClaudeAccountLogin({
                    ...
                    new ClaudeAccountError({
                      ...
                      detail:
                        cause instanceof Error
                          ? cause.message
                          : "Could not start the Claude sign-in flow.",
                      cause,
                    }),
                ),
              );
              const loginId = yield* randomUUID.pipe(
                Effect.mapError(
                  (cause) =>
                    new ClaudeAccountError({
                      reason: "failed",
                      detail: "Could not start the Claude sign-in flow.",
                      cause,
                    }),
                ),
              );
              pendingClaudeLogins.set(loginId, started.pending);
              return {
                loginId,
        ...
        [WS_METHODS.claudeAwaitAccountLogin]: (input) =>
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              if (!pending) {
                return yield* Effect.fail(
                  new ClaudeAccountError({
                    reason: "not-found",
                    detail: "That sign-in is no longer in progress. Start it again.",
                  }),
                );
              }
              ...
              const profile = yield* probeClaudeProfile({
                ...
                Effect.mapError(
                  (cause) =>
                    new ClaudeAccountError({
                      reason: "failed",
                      detail: "Signed in, but the account could not be read back.",
                      cause,
                    }),
                ),
              );
        ...
        [WS_METHODS.claudeStatusLineRender]: (input) =>
          observeRpcEffect(
            WS_METHODS.claudeStatusLineRender,
            // Settings are read per call so a changed CLAUDE_CONFIG_DIR takes
            // effect without a reconnect. An unreadable settings file must not
            // fail the status line: fall back to the default Claude home.
            //
            // Resolved from the provider INSTANCE, which is where the account
            // switcher writes `homePath`. Reading the legacy
            // `providers.claudeAgent` block instead handed the status line an
            // empty path on every call, so a script reporting plan usage kept
            // showing the previously selected account's numbers.
            serverSettings.getSettings.pipe(
              Effect.map((settings) =>
        ...
        [WS_METHODS.vcsFetchRemote]: (input) =>
          observeRpcEffect(
            WS_METHODS.vcsFetchRemote,
            // Straight to the driver rather than through status refresh, whose
            // fetch is coalesced on a 15s timer: an explicit click must always
            // reach the network, otherwise the spinner lies.
            gitWorkflow
              .fetchRemote({
                cwd: input.cwd,
          ...
          observeRpcEffect(
            ...
            gitWorkflow
              .pullCurrentBranch(input.cwd, {
                discardConflictingLocalChanges: input.discardConflictingLocalChanges === true,
              })
              .pipe(
