import type { OrchestrationEvent, OrchestrationReadModel, ThreadId } from "@ch3tools/contracts";
import {
    ...
    case "thread.messages-rewound":
      return decodeForEvent(
        ...
        Effect.map((payload) => {
          ...
          const cutIndex = thread.messages.findIndex(
            (message) => message.id === payload.fromMessageId,
          );
          if (cutIndex < 0) {
            return nextBase;
          }
          const messages = thread.messages.slice(0, cutIndex);
          // Anything belonging to a dropped turn goes with it; entries with
          // no turn (imported history, system notes) are kept.
          ...
          const droppedTurnIds = new Set(
            thread.messages
              ...
              .filter((turnId) => !retainedTurnIds.has(turnId)),
          );
          const activities = thread.activities.filter(
            (activity) => activity.turnId === null || !droppedTurnIds.has(activity.turnId),
          );
          const proposedPlans = thread.proposedPlans.filter(
            (plan) => plan.turnId === null || !droppedTurnIds.has(plan.turnId),
          );
          const checkpoints = thread.checkpoints.filter(
            (checkpoint) => !droppedTurnIds.has(checkpoint.turnId),
          );
          const latestTurnDropped =
            thread.latestTurn !== null && droppedTurnIds.has(thread.latestTurn.turnId);
          return {
            ...nextBase,
            threads: updateThread(nextBase.threads, payload.threadId, {
              messages,
              ...
              proposedPlans,
              checkpoints,
              ...(latestTurnDropped ? { latestTurn: null } : {}),
              updatedAt: event.occurredAt,
            }),
          };
        }),
