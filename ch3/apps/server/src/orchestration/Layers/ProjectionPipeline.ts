      ...
      switch (event.type) {
        ...
        case "thread.session-set": {
          ...
          yield* projectionThreadRepository.upsert({
            ...
            // thread has no latest turn. Writing the null through erased the
            // pointer to the turn that had just completed, and with it every
            // signal the client derives from `latestTurn`: the "Done" badge,
            // the unread mark, and the inbox float. Whether a thread kept its
            // pointer then came down to whether some later event (a turn diff
            // completing) happened to set it again — which is why the badge
            // appeared for some finished threads and not others.
