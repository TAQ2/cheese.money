...
const BaseTestLayer = makeProjectionPipelinePrefixedTestLayer("ch3-projection-pipeline-test-");
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("ch3-base-")))(
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("ch3-projection-attachments-safe-")))(
  "OrchestrationProjectionPipeline",
  (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("ch3-projection-attachments-overwrite-")),
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("ch3-projection-attachments-rollback-")),
)("OrchestrationProjectionPipeline", (it) => {
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.make("refs/ch3/checkpoints/thread-revert-files/turn/1"),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.make("refs/ch3/checkpoints/thread-revert-files/turn/2"),
          status: "ready",
          files: [],
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("ch3-projection-attachments-revert-")))(
  "OrchestrationProjectionPipeline",
  (it) => {
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("ch3-projection-attachments-delete-")))(
  "OrchestrationProjectionPipeline",
  (it) => {
...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("keeps the turn running across interim assistant messages until the session ends", () =>
    Effect.gen(function* () {
      ...
      // The thread must still POINT at the turn that just finished. A session
      // with no active turn means the turn ended, not that the thread has no
      // latest turn — and every "this finished while you were away" signal the
      // client shows (the Done badge, the unread mark, the inbox float) is
      // derived from `latestTurn`, so clearing the pointer here silently
      // removes all three.
      ...
      Effect.gen(function* () {
        ...
        yield* appendAndProject({
          ...
          payload: {
            ...
            turnId: TurnId.make("turn-completed"),
            checkpointTurnCount: 1,
            checkpointRef: CheckpointRef.make("refs/ch3/checkpoints/thread-conflict/turn/1"),
            status: "ready",
            files: [],
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 1,
          checkpointRef: CheckpointRef.make("refs/ch3/checkpoints/thread-revert/turn/1"),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 2,
          checkpointRef: CheckpointRef.make("refs/ch3/checkpoints/thread-revert/turn/2"),
          status: "ready",
          files: [],
...
it.layer(makeProjectionPipelinePrefixedTestLayer("ch3-pending-turn-terminal-test-"))(
    ...
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "ch3-projection-pipeline-restart-",
        }),
        NodeServices.layer,
      ),
    ),
...
const engineLayer = it.layer(
  OrchestrationEngineLive.pipe(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "ch3-projection-pipeline-engine-dispatch-",
      }),
    ),
    Layer.provideMerge(NodeServices.layer),
  ),
