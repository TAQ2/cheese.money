...
import * as BackgroundShellPidLookup from "../src/resourceTelemetry/BackgroundShellPidLookup.ts";
...
import { KanbanClassificationReactor } from "../src/orchestration/Services/KanbanClassificationReactor.ts";
...
export const makeOrchestrationIntegrationHarness = (
  ...
  Effect.gen(function* () {
    ...
    const rootDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "ch3-orchestration-integration-",
    });
    ...
    const orchestrationReactorLayer = OrchestrationReactorLive.pipe(
      ...
      Layer.provideMerge(
        Layer.succeed(KanbanClassificationReactor, {
          start: () => Effect.void,
          drain: Effect.void,
        }),
      ),
    ...
    const layer = Layer.empty.pipe(
      ...
      // The harness runs no process sampler, so no task row names a pid.
      Layer.provideMerge(BackgroundShellPidLookup.layerUnavailable),
