// This file mostly exists because we want dev mode to say "CH3 (Dev)" instead of "electron"
...
// oxlint-disable-next-line ch3/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["CH3CODE_PORT", environment.CH3CODE_PORT],
    ["CH3CODE_HOME", environment.CH3CODE_HOME],
    ["CH3CODE_COMMIT_HASH", environment.CH3CODE_COMMIT_HASH],
    ["CH3CODE_OTLP_TRACES_URL", environment.CH3CODE_OTLP_TRACES_URL],
    ["CH3CODE_OTLP_EXPORT_INTERVAL_MS", environment.CH3CODE_OTLP_EXPORT_INTERVAL_MS],
    ["CH3CODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --ch3-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the CH3 name.
