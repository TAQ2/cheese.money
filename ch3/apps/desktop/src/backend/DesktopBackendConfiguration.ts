...
>()("@ch3tools/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  ...
  "CH3CODE_NO_BROWSER",
  "CH3CODE_HOST",
  "CH3CODE_DESKTOP_WS_URL",
  "CH3CODE_DESKTOP_LAN_ACCESS",
  "CH3CODE_DESKTOP_LAN_HOST",
  "CH3CODE_DESKTOP_HTTPS_ENDPOINTS",
  "CH3CODE_TAILSCALE_SERVE",
  "CH3CODE_TAILSCALE_SERVE_PORT",
] as const;
  ...
  > {
    ...
    return {
      ...
      env: {
        ...
        // The server's Node runtime defaults to the bundled Mozilla roots and
        // ignores proxy variables. On managed company Macs both are wrong:
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit ch3Home so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus CH3CODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.ch3 via
  // /mnt/c, which means both backends read/write the same database and
  // their env-ids collide).
  const parentEnvWithoutCH3Home: Record<string, string | undefined> = {};
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "CH3CODE_HOME") continue;
    parentEnvWithoutCH3Home[key] = value;
  }
  const wslEnv = mergeWslEnv(parentEnvWithoutCH3Home.WSLENV, forwardedEnvNames);
  ...
  const baseConfig = {
    ...
    cwd: environment.backendCwd,
    env: {
      ...parentEnvWithoutCH3Home,
      ...backendChildEnvPatch(),
      ...forwardedEnv,
      ...(wslEnv !== undefined ? { WSLENV: wslEnv } : {}),
    },
    // env is already a complete process.env minus CH3CODE_HOME; pass it
