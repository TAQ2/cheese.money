...
import * as SshAuth from "@ch3tools/ssh/auth";
import { discoverSshHosts } from "@ch3tools/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@ch3tools/ssh/errors";
import * as SshTunnel from "@ch3tools/ssh/tunnel";
...
>()("@ch3tools/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteCH3RunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "CH3 window is not available for SSH authentication.";
      break;
