import type { DesktopSshPasswordPromptRequest } from "@ch3tools/contracts";
import { DesktopSshPasswordPromptResolutionInputSchema } from "@ch3tools/contracts";
import type { SshPasswordRequest } from "@ch3tools/ssh/auth";
...
export class DesktopSshPromptWindowUnavailableError extends Schema.TaggedErrorClass<DesktopSshPromptWindowUnavailableError>()(
  ...
  override get message(): string {
    ...
    return `CH3 window is unavailable during ${this.stage} for SSH authentication to ${this.destination} (request: ${request}).`;
  }
}
...
>()("@ch3tools/desktop/ssh/DesktopSshPasswordPrompts") {}
