...
describe("ElectronProtocol", () => {
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* protocol.registerDesktopProtocol({
            ...
            clerkFrontendApiHostname: "clerk.ch3.codes",
          });
          assert.isDefined(handler);

          const response = yield* Effect.promise(() =>
            handler!(
              new Request("ch3-dev://app/api/health?verbose=1", {
                headers: {
          ...
          assert.include(
            ...
            "script-src 'self' 'unsafe-inline' https://clerk.ch3.codes https://challenges.cloudflare.com",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' ch3-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' ch3-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["ch3-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("ch3://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("ch3-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "ch3-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(error.message, 'Failed to register Electron protocol scheme "ch3-dev".');
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "ch3");
        assert.strictEqual(error.cause, cause);
        assert.equal(error.message, 'Failed to unregister Electron protocol scheme "ch3".');
      }
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["font-src"], ["'self'", "ch3:", "data:"]);
    // Read-aloud plays from an object URL; without `blob:` here the media
    // element is silently blocked and the whole feature produces no sound.
    assert.deepEqual(directives["media-src"], ["'self'", "blob:", "data:"]);
  });
});
