...
export const DESKTOP_PRODUCTION_SCHEME = "ch3";
export const DESKTOP_DEVELOPMENT_SCHEME = "ch3-dev";
...
>()("@ch3tools/desktop/electron/ElectronProtocol") {}
...
export function makeDesktopContentSecurityPolicy(input: DesktopProtocolRegistrationInput): string {
  ...
  return [
    ...
    // Read-aloud hands the browser an mp3 as an object URL, so media has to
    // allow `blob:` explicitly. Without a media-src directive it falls back to
    // `default-src 'self'`, which blocks the source silently: the audio element
    // simply never loads and `play()` rejects.
