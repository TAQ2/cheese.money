...
>()("@ch3tools/desktop/shell/DesktopShellEnvironment") {}
...
const LOGIN_SHELL_ENV_NAMES = [
  ...
  "XDG_CONFIG_HOME",
  "XDG_DATA_HOME",
  // Corporate networks reach the world through a proxy, and a Finder launch
  // carries none of the shell's proxy settings. Without these the server's
  // own outbound calls (speech synthesis, plan usage) fail only when the app
  // was opened from the Dock — the least debuggable way to differ.
  "HTTP_PROXY",
  "HTTPS_PROXY",
  "NO_PROXY",
  "ALL_PROXY",
...
const installPosixEnvironment = Effect.fn("desktop.shellEnvironment.installPosixEnvironment")(
  function* (
    ...
    for (const name of [
      ...
      "XDG_CONFIG_HOME",
      "XDG_DATA_HOME",
      // Capturing the proxy settings from the login shell only matters if
      // they are then INSTALLED — a Finder launch has none of its own, and
      // the backend child inherits this process's environment. Without this
      // write-back the earlier capture was a no-op and every outbound server
      // call still failed on proxy-only networks.
      "HTTP_PROXY",
      "HTTPS_PROXY",
      "NO_PROXY",
      "ALL_PROXY",
