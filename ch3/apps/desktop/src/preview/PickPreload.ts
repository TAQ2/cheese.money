...
const OVERLAY_ATTRIBUTE = "data-ch3-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--ch3-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--ch3-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--ch3-radius": theme.radius,
    "--ch3-background": theme.background,
    "--ch3-foreground": theme.foreground,
    "--ch3-popover": theme.popover,
    "--ch3-popover-foreground": theme.popoverForeground,
    "--ch3-primary": theme.primary,
    "--ch3-primary-foreground": theme.primaryForeground,
    "--ch3-muted": theme.muted,
    "--ch3-muted-foreground": theme.mutedForeground,
    "--ch3-accent": theme.accent,
    "--ch3-accent-foreground": theme.accentForeground,
    "--ch3-border": theme.border,
    "--ch3-input": theme.input,
    "--ch3-ring": theme.ring,
    "--ch3-font-sans": theme.fontSans,
    "--ch3-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-ch3-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--ch3-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-ch3-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
