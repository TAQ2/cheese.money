...
const AppPackageMetadata = Schema.Struct({
  ch3CommitHash: Schema.optional(Schema.String),
});
...
>()("@ch3tools/desktop/app/DesktopAppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.ch3CommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
