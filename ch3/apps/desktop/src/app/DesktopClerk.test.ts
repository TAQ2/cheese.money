...
describe("DesktopClerk", () => {
  ...
  it("derives the Clerk Frontend API hostname used by the desktop CSP", () => {
    const publishableKey = `pk_test_${btoa("clerk.ch3.codes$")}`;
        ...
        [
          {
            ...
            passkeys: true,
            renderer: { scheme: "ch3-dev", host: "app" },
          },
        ],
      ]);
  ...
  it.effect("preserves bridge initialization failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(error.stateDir, "/tmp/ch3-state");
      assert.equal(error.isDevelopment, true);
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to initialize the desktop Clerk bridge for state directory "/tmp/ch3-state" (development: true).',
      );
    });
  });
  ...
  it.effect("preserves bridge cleanup failures", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(
          error.message,
          'Failed to clean up the desktop Clerk bridge for state directory "/tmp/ch3-state" (development: false).',
        );
      }
    });
  });
  ...
  it.each([
    { isDevelopment: true, scheme: "ch3-dev" },
    { isDevelopment: false, scheme: "ch3" },
    ...
    assert.equal(DesktopClerk.createDesktopClerkBridge("/tmp/ch3-state", isDevelopment), bridge);
    assert.deepEqual(storageMock.mock.calls, [[{ path: "/tmp/ch3-state" }]]);
    assert.deepEqual(createClerkBridgeMock.mock.calls, [
      [
        {
          storage: storageAdapter,
