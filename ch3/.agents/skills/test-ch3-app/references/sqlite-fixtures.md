...
Load this reference only when inspecting or seeding local CH3 state directly.
...
```bash
node apps/server/scripts/ch3-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/ch3-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/ch3-seed.sql
```
...
For a substantial current example, inspect `seedDatabase` in `scripts/showcase-environment.ts`. Adapt its column set to the target database instead of assuming copied SQL remains current.
