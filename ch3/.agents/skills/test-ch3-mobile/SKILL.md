---
name: test-ch3-mobile
description: Launch and test CH3 Mobile on an iOS Simulator or Android Emulator against disposable local CH3 environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test CH3 Mobile

Run one focused, end-to-end mobile verification pass against disposable CH3 state. Use the sibling [`test-ch3-app`](../test-ch3-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `CH3 Dev`
- Bundle/package identifier: `com.ch3tools.ch3.dev`
- URL scheme: `ch3-dev`
...
## Start one disposable CH3 environment

Run backend commands from the repository root. Use the ignored, worktree-local `.ch3` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.ch3` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-ch3-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `ch3-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/CH3Dev.xcworkspace`
- Scheme: `CH3Dev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.ch3tools.ch3.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.ch3tools.ch3.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.ch3tools.ch3.dev
...
```bash
CH3CODE_PORT=<server-port> node apps/server/src/bin.ts auth pairing create \
  --base-dir <base-dir> \
...
In PowerShell, set `$env:CH3CODE_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'ch3-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'ch3-dev://connections/new' \
  com.ch3tools.ch3.dev
```

Run only the command for the selected platform.

In CH3 Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in CH3 when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from CH3 Dev.
