  ...
  switch (event.type) {
    ...
    case "thread.messages-rewound": {
      const cutIndex = thread.messages.findIndex(
        (message) => message.id === event.payload.fromMessageId,
      );
      if (cutIndex < 0) {
        return { kind: "unchanged" };
      }
      ...
      // Turn-less entries (imported history, system notes) are never dropped.
      const activities = pipe(
        thread.activities,
        Arr.filter((activity) => activity.turnId === null || !droppedTurnIds.has(activity.turnId)),
      );
      const proposedPlans = pipe(
        thread.proposedPlans,
        Arr.filter((plan) => plan.turnId === null || !droppedTurnIds.has(plan.turnId)),
      );
      const checkpoints = pipe(
        thread.checkpoints,
        Arr.filter((checkpoint) => !droppedTurnIds.has(checkpoint.turnId)),
      );
