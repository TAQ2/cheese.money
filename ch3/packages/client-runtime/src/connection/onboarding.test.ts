import { AuthStandardClientScopes, EnvironmentId } from "@ch3tools/contracts";
...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/ch3/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/ch3/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("CH3 Test");
    }),
  );
