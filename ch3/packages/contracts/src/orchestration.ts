...
/**
 * Server-internal: seeds a thread's timeline with messages imported from an
 * external provider transcript (the /resume session import). Emits plain
 * thread.message-sent events — no turn, no provider side effects.
 */
...
/** Server-internal: drops this message and everything after it. */
const ThreadMessagesRewindCommand = Schema.Struct({
  type: Schema.Literal("thread.messages.rewind"),
  commandId: CommandId,
  threadId: ThreadId,
...
/**
 * Conversation rewind to an earlier input: every message from
 * `fromMessageId` onward is dropped, everything before is kept. Unlike
 * `thread.reverted` this touches no files and needs no checkpoint, so it
 * works on chat-only turns too.
 */
export const ThreadMessagesRewoundPayload = Schema.Struct({
  threadId: ThreadId,
  fromMessageId: MessageId,
});
...
export const OrchestrationEvent = Schema.Union([
  ...
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.messages-rewound"),
    payload: ThreadMessagesRewoundPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
