...
/**
 * Explicit, user-initiated `git fetch`. Distinct from the fetch that backs
 * status refresh, which is coalesced on a timer: a person clicking "fetch" is
 * asking for network I/O now, not for a cached answer.
 */
export const VcsFetchRemoteInput = Schema.Struct({
  cwd: TrimmedNonEmptyStringSchema,
  remoteName: Schema.optionalKey(TrimmedNonEmptyStringSchema),
});
export type VcsFetchRemoteInput = typeof VcsFetchRemoteInput.Type;
export type VcsStatusInput = typeof VcsStatusInput.Type;

export const VcsPullInput = Schema.Struct({
  cwd: TrimmedNonEmptyStringSchema,
  /**
   * Explicit user consent to discard local modifications to files the
   * incoming commits also touch, so a fast-forward blocked by "your local
   * changes would be overwritten" can proceed. Scoped to the overlapping
   * files only — unrelated local work is never touched. The client must
   * gate this behind its own destructive-action confirmation.
   */
  discardConflictingLocalChanges: Schema.optionalKey(Schema.Boolean),
});
