...
export const ServerProvider = Schema.Struct({
  ...
  // Response styles the driver reports for the account it runs as. Claude
  // populates this from the CLI's own initialization result; every other
  // driver omits it, which is how the composer decides whether to offer the
  // picker at all. Optional rather than defaulted so the ~90 existing
  // snapshot fixtures and every non-Claude producer stay valid untouched.
  outputStyles: Schema.optionalKey(Schema.Array(TrimmedNonEmptyString)),
  // The style the driver resolves on its own from the user's settings files,
  // used as the composer's label before anyone picks one so the chip never
  // claims a style that is not actually in force.
  activeOutputStyle: Schema.optionalKey(TrimmedNonEmptyString),
...
export const ServerSelfUpdateInput = Schema.Struct({
  /** Exact npm version of the `ch3` package to install (never a dist-tag, so
