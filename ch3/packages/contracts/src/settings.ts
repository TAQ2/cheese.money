...
export const MIN_SIDEBAR_AUTO_ARCHIVE_AFTER_SETTLED_DAYS = 1;
export const MAX_SIDEBAR_AUTO_ARCHIVE_AFTER_SETTLED_DAYS = 365;
export const SidebarAutoArchiveAfterSettledDays = Schema.Number.check(
  Schema.isBetween({
    minimum: MIN_SIDEBAR_AUTO_ARCHIVE_AFTER_SETTLED_DAYS,
    maximum: MAX_SIDEBAR_AUTO_ARCHIVE_AFTER_SETTLED_DAYS,
  }),
);
export type SidebarAutoArchiveAfterSettledDays = typeof SidebarAutoArchiveAfterSettledDays.Type;
export const DEFAULT_SIDEBAR_AUTO_ARCHIVE_AFTER_SETTLED_DAYS: SidebarAutoArchiveAfterSettledDays = 4;
...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Whether the native Claude usage band under the composer is collapsed. A
  // single global toggle so hiding it on one thread hides it on all of them.
  usageBandHidden: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
  ...
  sidebarAutoArchiveAfterSettledDays: Schema.NullOr(SidebarAutoArchiveAfterSettledDays).pipe(
    Schema.withDecodingDefault(Effect.succeed(DEFAULT_SIDEBAR_AUTO_ARCHIVE_AFTER_SETTLED_DAYS)),
  ),
...
export const CodexSettings = makeProviderSettingsSchema(
  {
    ...
    shadowHomePath: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: {
          placeholder: "~/.codex-ch3/personal",
          clearWhenEmpty: "omit",
        },
      }),
    ),
...
export const ClaudeSettings = makeProviderSettingsSchema(
  {
    ...
    /**
     * Percentage at which this instance hands over to another signed-in
     * account. Both the 5-hour session window and the 7-day window count.
     * 98 everywhere: the reactor's own fallback uses the same number, so an
     * instance authored through this schema behaves identically to a bare
     * one.
     */
    accountFailoverThresholdPercent: Schema.Number.pipe(
      Schema.withDecodingDefault(Effect.succeed(98)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let CH3 spawn the server when needed.",
        providerSettingsForm: {
...
export const ClientSettingsPatch = Schema.Struct({
  ...
  sidebarAutoArchiveAfterSettledDays: Schema.optionalKey(
    Schema.NullOr(SidebarAutoArchiveAfterSettledDays),
  ),
