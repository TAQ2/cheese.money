...
}).annotate({ description: "Links an authenticated cloud user to a CH3 environment." });
...
>()("@ch3tools/contracts/relay/RelayClientPrincipal") {}
...
>()("@ch3tools/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in CH3 Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["ch3-mobile", "ch3-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "ch3-mobile" as const;
export const RelayWebClientId = "ch3-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "CH3 Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking CH3 environments, connecting authorized clients, and publishing agent activity.",
  );
