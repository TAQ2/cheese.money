...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "ch3-ssh-askpass-test-" });
      ...
      assert.equal(env.CH3_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "ch3");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\ch3-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\ch3-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
