...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@ch3tools/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "ch3-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when CH3 re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# CH3_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${CH3_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$CH3_SSH_AUTH_SECRET"
  exit 0
fi
printf 'CH3 ssh-askpass invoked without CH3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when CH3 re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if CH3_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:CH3_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:CH3_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("CH3 ssh-askpass invoked without CH3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "ch3-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined ? {} : { CH3_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "ch3" }),
  };
});
