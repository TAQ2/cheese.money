...
} from "@ch3tools/shared/httpReadiness";
import * as NetService from "@ch3tools/shared/Net";
import { extractJsonObject, fromLenientJson } from "@ch3tools/shared/schemaJson";
import { satisfiesSemverRange } from "@ch3tools/shared/semver";
...
export interface RemoteCH3RunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteCH3RunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteCH3RunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  CH3_NODE_ENGINE_RANGE=@@CH3_NODE_ENGINE_RANGE@@
  if [ -z "$CH3_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$CH3_NODE_ENGINE_RANGE" <<'NODE'
@@CH3_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for CH3_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$CH3_NODE_BIN/node" ]; then
        PATH="$CH3_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
CH3_NODE_SCRIPT_PATH=@@CH3_NODE_SCRIPT_PATH@@
if [ -n "$CH3_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$CH3_NODE_SCRIPT_PATH" "$@"
fi
if command -v ch3 >/dev/null 2>&1; then
  exec ch3 "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@CH3_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@CH3_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the ch3 CLI and could not install @@CH3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@CH3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.ch3/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.ch3"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-ch3.sh"
RUNNER_NEXT="$STATE_DIR/run-ch3.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@CH3_DEFAULT_REMOTE_PORT@@" "@@CH3_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@CH3_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@CH3_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@CH3_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@CH3_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@CH3_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@CH3_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env CH3CODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@CH3_READY_TIMEOUT_MS@@"; then
    printf 'Remote CH3 server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.ch3/ssh-launch/@@CH3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.ch3"
RUNNER_FILE="$STATE_DIR/run-ch3.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteCH3RunnerScript(input?: RemoteCH3RunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "ch3@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      CH3_PACKAGE_SPEC: packageSpec,
      CH3_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      CH3_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteCH3RunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      CH3_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      CH3_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteCH3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    CH3_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    CH3_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteCH3RunnerScript(input)),
    CH3_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    CH3_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    CH3_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    CH3_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    CH3_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    CH3_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    CH3_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteCH3RunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    CH3_STATE_KEY: remoteStateKey(target),
    CH3_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteCH3RunnerScript(input)),
  });
}
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteCH3RunnerOptions;
...
>()("@ch3tools/ssh/tunnel/SshEnvironmentManager") {
