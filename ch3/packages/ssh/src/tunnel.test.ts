...
describe("ssh tunnel scripts", () => {
  it("builds the remote ch3 runner with npx and npm fallbacks", () => {
    const script = buildRemoteCH3RunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "CH3_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec ch3 "$@"');
    assert.include(script, "exec npx --yes 'ch3@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'ch3@latest' -- \"$@\"");
    assert.include(script, "could not install 'ch3@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `CH3_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for CH3_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteCH3RunnerScript();

    assert.include(script, "CH3_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote ch3 runner", () => {
    const script = buildRemoteCH3RunnerScript({
      packageSpec: "ch3@nightly; touch /tmp/ch3-owned",
    });

    assert.include(script, "exec npx --yes 'ch3@nightly; touch /tmp/ch3-owned' \"$@\"");
    assert.include(script, "exec npm exec --yes 'ch3@nightly; touch /tmp/ch3-owned' -- \"$@\"");
    assert.notInclude(script, "exec npx --yes ch3@nightly; touch /tmp/ch3-owned");
  });

  it("builds the remote ch3 runner with a node script override", () => {
    const script = buildRemoteCH3RunnerScript({
    ...
    assert.include(
      script,
      "CH3_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$CH3_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote ch3 runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `CH3_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote CH3 server did not become ready");
    assert.include(buildRemoteLaunchScript({ packageSpec: "ch3@nightly" }), "ch3@nightly");
    assert.include(
    ...
    assert.include(buildRemotePairingScript(target, { packageSpec: "ch3@nightly" }), "ch3@nightly");
    assert.include(
