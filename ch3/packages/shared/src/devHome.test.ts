...
import { resolveGitWorktreePath, resolveWorktreeCH3Home } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ch3-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeCH3Home", () => {
  it.effect("answers with .ch3 before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeCH3Home(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".ch3"));
