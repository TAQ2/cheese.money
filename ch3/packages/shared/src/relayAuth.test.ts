...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.ch3.codes"))).toBe(
      "clerk.ch3.codes",
    );
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.ch3.codes"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.ch3.codes/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.ch3.codes", "clerk.ch3.codes")).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.ch3.codes")).toBe(false);
    expect(isAllowedClerkFrontendApiHostname("nested.clerk.ch3.codes", "clerk.ch3.codes")).toBe(
      false,
    );
  });
});
