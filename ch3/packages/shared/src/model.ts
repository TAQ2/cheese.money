...
/**
 * Provider option id carrying the Claude response style for a thread.
 *
 * Unlike `effort` or `fastMode`, this option has no descriptor: the list of
 * styles is whatever the CLI reports for the signed-in home directory, not a
 * static model capability. It rides the same `modelSelection.options` array so
 * it inherits that array's transport and per-thread persistence for free.
 */
export const OUTPUT_STYLE_OPTION_ID = "outputStyle";

/**
 * Option ids that survive a descriptor-driven rebuild of the selections array.
 *
 * `buildProviderOptionSelectionsFromDescriptors` reconstructs the array from
 * the descriptors alone, which drops any selection no descriptor declares.
 * That is the intended behavior for model-scoped traits — switching to a model
 * without an effort dial should forget the effort — but it would silently wipe
 * a descriptor-less option on the next rebuild, and the composer rebuilds on
 * every dispatch.
 */
const DESCRIPTORLESS_PROVIDER_OPTION_IDS: ReadonlySet<string> = new Set([OUTPUT_STYLE_OPTION_ID]);

/**
 * Re-apply descriptor-less selections onto a freshly rebuilt selections array.
 *
 * Existing entries always win, so a rebuild that legitimately produced a value
 * for one of these ids is never overwritten by the stale previous one.
 */
export function withDescriptorlessProviderOptionSelections(
  rebuilt: ReadonlyArray<ProviderOptionSelection> | undefined,
  previous: ReadonlyArray<ProviderOptionSelection> | null | undefined,
): Array<ProviderOptionSelection> | undefined {
  const preserved = (previous ?? []).filter(
    (selection) =>
      DESCRIPTORLESS_PROVIDER_OPTION_IDS.has(selection.id) &&
      !(rebuilt ?? []).some((candidate) => candidate.id === selection.id),
  );
  if (preserved.length === 0) {
    return rebuilt ? [...rebuilt] : undefined;
  }
  return [...(rebuilt ?? []), ...preserved];
}

/**
 * Read the response style a thread has pinned, if any.
 *
 * An absent value means "whatever the CLI resolves on its own" — the user
 * never picked one, so no `outputStyle` is forced onto the session.
 */
export function getOutputStyleSelection(
  selections: ReadonlyArray<ProviderOptionSelection> | null | undefined,
): string | undefined {
  const value = getProviderOptionStringSelectionValue(selections, OUTPUT_STYLE_OPTION_ID);
  return value && value.trim().length > 0 ? value : undefined;
}

/**
 * Write (or clear, with `null`) the response style on a selections array.
 *
 * Returns `undefined` when the result is empty so it matches the shape
 * `buildProviderOptionSelectionsFromDescriptors` produces for "no options".
 */
export function withOutputStyleSelection(
  selections: ReadonlyArray<ProviderOptionSelection> | null | undefined,
  outputStyle: string | null,
): Array<ProviderOptionSelection> | undefined {
  const withoutOutputStyle = (selections ?? []).filter(
    (selection) => selection.id !== OUTPUT_STYLE_OPTION_ID,
  );
  const trimmed = outputStyle?.trim();
  const next = trimmed
    ? [...withoutOutputStyle, { id: OUTPUT_STYLE_OPTION_ID, value: trimmed }]
    : withoutOutputStyle;
