...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit CH3CODE_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "CH3CODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "CH3CODE_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CH3CODE_HOME, undefined);
        assert.equal(env.CH3CODE_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CH3CODE_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CH3CODE_HOME, path.resolve("/tmp/custom-ch3"));
        assert.equal(env.CH3CODE_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.CH3CODE_NO_BROWSER, "1");
        assert.equal(env.CH3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.CH3CODE_LOG_WS_EVENTS, "1");
        assert.equal(env.CH3CODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            CH3CODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          ch3Home: undefined,
        ...
        assert.equal(env.CH3CODE_MODE, "web");
        assert.equal(env.CH3CODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CH3CODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom ch3Home when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.CH3CODE_HOME, path.resolve("/tmp/my-ch3"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            CH3CODE_PORT: "13773",
            CH3CODE_MODE: "web",
            CH3CODE_NO_BROWSER: "0",
            CH3CODE_HOST: "0.0.0.0",
        ...
        assert.equal(env.CH3CODE_MODE, undefined);
        assert.equal(env.CH3CODE_NO_BROWSER, undefined);
        assert.equal(env.CH3CODE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.CH3CODE_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.CH3CODE_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { CH3CODE_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          ch3Home: undefined,
          browser: undefined,
        ...
        assert.equal(env.CH3CODE_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (CH3CODE_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/CH3CODE_HOST has to be probed as well.
    ...
    // Only the backend honours --host/CH3CODE_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { CH3CODE_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["CH3CODE_PORT_OFFSET", "CH3CODE_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .ch3 > ambient CH3CODE_HOME)
    ...
    it.effect("ignores an ambient CH3CODE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { CH3CODE_HOME: "/home/user/.ch3" },
          serverOffset: 0,
          webOffset: 0,
          ch3Home: undefined,
          browser: undefined,
    ...
    describe("ch3 home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "ch3-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, ch3Home: input.ch3Home }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            Effect.provideService(HostProcessWorkingDirectory, input.cwd),
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { CH3CODE_HOME: input.ambientHome },
            ),
          );

          return captured?.CH3CODE_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            ch3Home: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.ch3",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".ch3"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .ch3 over an ambient CH3CODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient CH3CODE_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            ch3Home: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.ch3"));
        }),
      );
