...
import { resolveGitWorktreePath, resolveWorktreeCH3Home } from "@ch3tools/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@ch3tools/shared/hostProcess";
...
export const DEFAULT_CH3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".ch3"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@ch3tools/contracts",
    "--filter=@ch3tools/web",
    "--filter=ch3",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=ch3", "dev"],
  "dev:web": ["run", "--filter=@ch3tools/web", "dev"],
  "dev:desktop": ["run", "--filter=@ch3tools/desktop", "--filter=@ch3tools/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`CH3CODE_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("CH3CODE_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("CH3CODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("CH3CODE_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `CH3CODE_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric CH3CODE_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed CH3CODE_DEV_INSTANCE=${seed}` });
  }
...
interface CreateDevRunnerEnvInput {
  ...
  readonly ch3Home: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .ch3 > ambient CH3CODE_HOME) is resolved
    // by the caller; an unset ch3Home here genuinely means "use the default".
    const configuredBaseDir = ch3Home?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.CH3CODE_HOME = resolvedBaseDir;
    } else {
      delete output.CH3CODE_HOME;
    }

    if (!isDesktopMode) {
      output.CH3CODE_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.CH3CODE_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.CH3CODE_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.CH3CODE_PORT = String(serverPort);
      ...
      delete output.CH3CODE_SINGLE_ORIGIN_DEV;
      delete output.CH3CODE_MODE;
      delete output.CH3CODE_NO_BROWSER;
      delete output.CH3CODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.CH3CODE_HOST = host;
    }

    if (!isDesktopMode) {
      output.CH3CODE_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.CH3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.CH3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.CH3CODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.CH3CODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.CH3CODE_MODE = "web";
      delete output.CH3CODE_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`CH3CODE_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["CH3CODE_PORT_OFFSET", "CH3CODE_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.ch3` — see @ch3tools/shared/devHome for why this must
    // outrank an ambient CH3CODE_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeCH3Home(yield* HostProcessWorkingDirectory);
    ...
    const resolvedCH3Home =
      (input.ch3Home?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.CH3CODE_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      ch3Home: resolvedCH3Home,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.CH3CODE_HOME ?? (yield* DEFAULT_CH3_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.CH3CODE_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.CH3CODE_DEV_ALLOWED_ORIGINS = [
            env.CH3CODE_DEV_ALLOWED_ORIGINS,
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  ch3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit CH3 data directory; runtime state is stored under userdata (equivalent to CH3CODE_HOME). Inside a git worktree this defaults to that worktree's own .ch3 so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to CH3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("CH3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to CH3CODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("CH3CODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to CH3CODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("CH3CODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to CH3CODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("CH3CODE_PORT")),
  ),
