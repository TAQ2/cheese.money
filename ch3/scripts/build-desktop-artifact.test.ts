...
import { HostProcessArchitecture, HostProcessPlatform } from "@ch3tools/shared/hostProcess";
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                CH3CODE_DESKTOP_UPDATE_REPOSITORY: "pingdotgg/ch3",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "pingdotgg/ch3",
              },
            }),
          ),
        ),
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@ch3tools/contracts": "workspace:*",
          "@ch3tools/shared": "workspace:*",
          "@ch3tools/ssh": "workspace:*",
          "@ch3tools/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      CH3CODE_MACOS_PROVISIONING_PROFILE: "/tmp/ch3.provisionprofile",
      CH3CODE_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "com.ch3tools.ch3",
      teamId: "ABC1234567",
      rpDomains: ["example.clerk.accounts.dev"],
      provisioningProfilePath: "/tmp/ch3.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      CH3CODE_APPLE_TEAM_ID: "ABC1234567",
      CH3CODE_MACOS_PROVISIONING_PROFILE: "/tmp/ch3.provisionprofile",
      CH3CODE_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.ch3tools.ch3</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      CH3CODE_APPLE_TEAM_ID: "ABC1234567",
      CH3CODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "CH3CODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      CH3CODE_APPLE_TEAM_ID: "ABC1234567",
      CH3CODE_MACOS_PROVISIONING_PROFILE: "/tmp/ch3.provisionprofile",
      CH3CODE_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          CH3CODE_APPLE_TEAM_ID: "ABC1234567",
          CH3CODE_MACOS_PROVISIONING_PROFILE: "/tmp/ch3.provisionprofile",
          CH3CODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      CH3CODE_APPLE_TEAM_ID: "ABC1234567",
      CH3CODE_MACOS_PROVISIONING_PROFILE: "/tmp/ch3.provisionprofile",
      CH3CODE_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "CH3CODE_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.ch3tools.ch3");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/ch3.provisionprofile");
      assert.deepStrictEqual(mac.protocols, [{ name: "CH3", schemes: ["ch3", "ch3-dev"] }]);
    }).pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })))),
  );
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                CH3CODE_DESKTOP_SKIP_BUILD: "true",
                CH3CODE_DESKTOP_KEEP_STAGE: "true",
                CH3CODE_DESKTOP_SIGNED: "true",
                CH3CODE_DESKTOP_VERBOSE: "true",
                CH3CODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
