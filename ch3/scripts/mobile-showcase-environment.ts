// @effect-diagnostics nodeBuiltinImport:off globalDate:off - This host-side fixture creates an isolated local CH3 environment.
...
export const SHOWCASE_PROJECT_ID = "ch3";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  "\u001b[38;5;75m~/Code/ch3\u001b[0m \u001b[38;5;212mfeat/remote-command-center\u001b[0m",
  "$ vp test run --changed",
  "",
  "  \u001b[38;5;117mch3-mobile\u001b[0m       184 passed",
...
const PROJECT_FAVICONS = {
  ch3: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    title: "CH3",
    directory: "ch3",
    repositoryUrl: "https://github.com/pingdotgg/ch3.git",
    favicon: PROJECT_FAVICONS.ch3,
  },
  {
    id: "react",
    title: "React",
...
async function seedCH3Workspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "ch3", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.ch3);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the CH3 mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedCH3Workspace(workspaceRoot);
  }
  await Promise.all(
    projects
