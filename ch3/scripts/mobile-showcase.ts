...
const ANDROID_PACKAGE = "com.ch3tools.ch3.dev";
const APP_SCHEME = "ch3-dev";
const IOS_READY_FILENAME = "CH3ShowcaseReadyScene";
const SERVER_HOST = "0.0.0.0";
const IOS_SIMULATOR_ARCH = NodeProcess.arch === "arm64" ? "arm64" : "x86_64";
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/CH3Dev.app",
);
...
Usage:
  pnpm --filter @ch3tools/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/CH3Dev.xcworkspace"),
      "-scheme",
      "CH3Dev",
      "-configuration",
      "Debug",
...
async function captureIos(
  ...
  const scenePath = NodePath.join(
    ...
    "Library/Caches/CH3ShowcaseScene",
  );
  const readyPath = NodePath.join(
    await iosAppContainer(simulator.udid),
    "Library/Caches",
...
async function writeAndroidShowcaseScene(serial: string, scene: ShowcaseScene): Promise<void> {
  await runAdb(serial, [
    "shell",
    `run-as ${ANDROID_PACKAGE} sh -c 'mkdir -p files && rm -f files/ch3-showcase-ready && printf %s ${scene} > files/ch3-showcase-scene'`,
  ]);
}
...
async function main(): Promise<void> {
  ...
  const showcaseRootDir = await NodeFSP.mkdtemp(
    NodePath.join(NodeOS.tmpdir(), "ch3-mobile-showcase-"),
  );
