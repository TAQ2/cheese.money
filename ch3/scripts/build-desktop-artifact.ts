...
import { fromYaml } from "@ch3tools/shared/schemaYaml";
import { HostProcessPlatform } from "@ch3tools/shared/hostProcess";
import { clerkFrontendApiHostnameFromPublishableKey } from "@ch3tools/shared/relayAuth";
...
const DESKTOP_APP_ID = "com.ch3tools.ch3";
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // CH3 always passes the user's installed Claude executable to the SDK,
...
export class InvalidAppleTeamIdError extends Schema.TaggedErrorClass<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `CH3CODE_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedErrorClass<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "CH3CODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedErrorClass<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "CH3CODE_CLERK_PUBLISHABLE_KEY or CH3CODE_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedErrorClass<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "CH3CODE_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.CH3CODE_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.CH3CODE_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.CH3CODE_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.CH3CODE_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "CH3CODE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("CH3CODE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "CH3CODE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("CH3CODE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("CH3CODE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("CH3CODE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("CH3CODE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("CH3CODE_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("CH3CODE_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("CH3CODE_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("CH3CODE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
  ...
  wslPrebuild: Config.string("CH3CODE_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("CH3CODE_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "CH3 (Nightly)"
    : (desktopPackageJson.productName ?? "CH3");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "CH3-Code-${version}-${arch}.${ext}",
  ...
  if (platform === "mac") {
    ...
    buildConfig.mac = {
      ...
      // Apple. Without one, electron-builder leaves the app ad-hoc signed,
      // and macOS keys keychain permission to the exact binary — so every
      // reinstall is a stranger to the login keychain and the app asks for
      // the password again. Signing every build with the SAME identity makes
      // the grant survive rebuilds. Notarization is untouched: this only
      // applies when the build is not a signed release.
      ...(localSignIdentity !== undefined
        ? {
            identity: localSignIdentity,
            // electron-builder turns hardened runtime on as soon as it signs.
            // That is for notarized releases and needs matching entitlements;
            // without them Electron cannot allocate executable memory and the
            // app dies at launch. A local identity is only here to keep the
            // keychain grant stable, so leave it off.
...
/**
 * The name of a local, self-signed code signing identity to sign otherwise
 * unsigned builds with, from `CH3CODE_DESKTOP_LOCAL_SIGN_IDENTITY`.
 *
...
// node-pty's prebuilds/linux-<arch>/ with a ch3 marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / CH3CODE_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
  ...
  yield* fs.writeFileString(path.join(prebuildDir, "ch3-wsl-node-pty.json"), `${markerJson}\n`);
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `ch3-desktop-${options.platform}-stage-`,
  });
  ...
  if (!options.signed) {
    // A local identity still needs electron-builder to look in the keychain;
    // only a build with neither disables discovery outright.
    buildEnv.CSC_IDENTITY_AUTO_DISCOVERY = readLocalSignIdentity() === undefined ? "false" : "true";
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @ch3tools/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: CH3CODE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: CH3CODE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: CH3CODE_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: CH3CODE_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: CH3CODE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: CH3CODE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: CH3CODE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: CH3CODE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: CH3CODE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: CH3CODE_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: CH3CODE_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: CH3CODE_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for CH3."),
